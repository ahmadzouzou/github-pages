...
{$REGION 'Internal Declarations'}
	strict private
		procedure CommandChange(const Command:TUICommandDecimal;const Verb:TUICommandVerb;const Value:Double;
...
{$REGION 'Internal Declarations'}
	strict private
		...
		procedure CommandSelect(const Command:TUICommandRecentItems;const Verb:TUICommandVerb;const ItemIndex:Integer;
			const Properties:TUICommandExecutionProperties);
{$ENDREGION 'Internal Declarations'}
	protected
		procedure SetAction(Value:TBasicAction);override;
	...
	public
		...
		/// </summary>
		procedure PropertyUpdated(Sender:TObject;const PropKey:TUIPropertyKey;var NewValue:TPropVariant;
			out Handled:Boolean);
		/// <summary>
...
begin
	Result:=inherited and(FClient.UseImageFromAction or(TUIRibbonOption.roAssignImagesFromActionManager
		in TUIRibbon(FClient.Owner).Options));
end;
...
begin
	inherited;
	if Value is TCustomAction then
		with TCustomAction(Value) do begin
			if FClient.CommandType<>TUICommandType.ctAction then DisableIfNoHandler:=False;
...
begin
	if IsCaptionLinked and(Value<>'') then begin
		...
		if Value.Contains(cAmpersand) then begin
...
begin
	...
	if not Value then FClient.Enabled:=False;
	if Succeeded(UIInitPropertyFromBoolean(UI_PKEY_Viewable,TCustomAction(Action).Visible,lCurrentValue))and
		Succeeded(UIInitPropertyFromBoolean(UI_PKEY_Viewable,Value,lNewValue)) then
...
begin
	if Assigned(Self.Action) then Result:=inherited Update()
	else Result:=False;
end;

procedure TUICommandActionLink.SetHint(const Value:String);
begin
	if IsHintLinked and not Value.IsEmpty then begin
		...
		if (FClient.CommandType=TUICommandType.ctAction) then begin
		...
		if Assigned(FClient.OnUpdateHint) then FClient.OnUpdateHint(FClient,Value);
	end;
end;
...
begin
	...
	if (Value>=0)and IsImageIndexLinked and(TContainedAction(Self.Action).ActionList is TActionManager) then begin
		...
		if Assigned(lActionManager.Images) then FClient.SmallImage:=TUIImage.Create(lActionManager.Images,Value);
		if Assigned(lActionManager.LargeImages) then FClient.LargeImage:=TUIImage.Create(lActionManager.LargeImages,Value);
	end;
end;
...
procedure TUICommandActionActionLink.CommandExecute(const Args:TUICommandActionEventArgs);
begin
...
begin
	...
	if Args.Verb<>TUICommandVerb.cvExecute then Exit;
...
procedure TUICommandDecimalActionLink.CommandChange(const Command:TUICommandDecimal;const Verb:TUICommandVerb;
...
begin
	...
	if IsCheckedLinked and Args.Command.Checked<>TContainedAction(Action).Checked then begin
...
procedure TUICommandFontActionLink.CommandChanged(const Args:TUICommandFontEventArgs);
begin
...
begin
	...
	if (Action is TRibbonFontAction) then TRibbonFontAction(Action).UICommand:=(Client as TUICommandFont);
end;

{ TUICommandColorAnchorActionLink }

procedure TUICommandColorAnchorActionLink.CommandExecute(const Args:TUICommandColorEventArgs);
begin
	if Assigned(Action) then Action.Execute;
end;
...
begin
	...
	if (Action is TRibbonColorAction) then TRibbonColorAction(Action).UICommand:=(Client as TUICommandColorAnchor);
end;

{ TUICommandRecentItemsActionLink }

procedure TUICommandRecentItemsActionLink.CommandSelect(const Command:TUICommandRecentItems;const Verb:TUICommandVerb;
...
begin
	...
	Self.Selected:=TUIRecentItem.Create((lItem as TUIRecentItem).LabelText,(lItem as TUIRecentItem).Description,
		(lItem as TUIRecentItem).Path,(lItem as TUIRecentItem).Datex,(lItem as TUIRecentItem).Pinned);
	try
		if Assigned(Action) then Action.Execute;
	finally FreeAndNil(fSelected);
	end;
end;
...
begin
	...
	if (Action is TRecentItemAction) then TRecentItemAction(Action).UICommand:=(Client as TUICommandRecentItems);
end;
...
var
	...
	begin
		...
		for lCurrentCollectionItem in lCommandCollection.Categories do begin
			...
			else Result:=Max(Result,lGalleryCollectionItem.CategoryId);
			// Keep track of the highest used id. We may need it later to create a new category.
		end;
...
begin
	...
	if IsCurrentlyDisplayed then begin
	...
	try
		...
		for I:=0 to fActionList.Count-1 do begin
			...
			if not lAction.Visible then continue;

			lCategory:=fActionList[I].Category;
			if lCategory.IsEmpty then lTargetCategoryId:=-1
			else lTargetCategoryId:=FindOrCreateCategory(lCategory);
...
var
	lCommand:TUICommand;
begin
	if Assigned(UICommand) then begin
...
begin
	...
	if Assigned(UICommand) then UICommand.SelectedItem:=fSelectedItem
end;
...
begin
	...
	if Assigned(UICommand)and not(fSelectionInitialized) then begin
		UICommand.SelectedItem:=fSelectedItem;
		fSelectionInitialized:=True;
	end;
	if fRefreshWhenNotDisplayed and not IsCurrentlyDisplayed then RefreshCommandCollection;
end;

procedure TRibbonCollectionAction.PropertyUpdated(Sender:TObject;const PropKey:TUIPropertyKey;var NewValue:TPropVariant;
	out Handled:Boolean);
var
...
begin
	if PropKey=UI_PKEY_Viewable then begin
		...
		if not Succeeded(UIPropertyToBoolean(PropKey,NewValue,lActionVisible)) then Exit;
...
begin
	...
	if Length(pLabel)>Length(UICommand.RepresentativeString) then UICommand.RepresentativeString:=pLabel;
	if (UICommand.Items.Count=1)and(fSelectedItem=-1) then
		// Only one item exists and nothing has been selected yet -> Select it, so that we have a default selection.
			SelectedItem:=0;
end;
...
begin
	Dec(fUpdateCount);
	if (fUpdateCount=0) then RefreshCommandCollection;
end;
...
begin
	BeginUpdate;
	try
		for I:=fActionList.Count-1 downto 0 do Remove(fActionList[I]);
	finally EndUpdate; // This triggers RefreshCommandCollection()
	end;
end;
...
begin
	if Assigned(fOriginalOnMenuChange) then fOriginalOnMenuChange(Sender,Source,Rebuild);
	...
	try
		for I:=0 to Menu.Items.Count-1 do begin
			if not Assigned(Menu.Items[I].Action)and SameText(Menu.Items[I].Caption,cLineCaption) then begin
				...
				if lCategory=' ' then begin
					for J:=0 to lActionList.Count-1 do lActionList[J].Category:=lCategory;
				end;
		...
		AddRange(lActionList);
	finally FreeAndNil(lActionList);
	end;
end;
