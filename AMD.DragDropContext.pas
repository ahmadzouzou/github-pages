...
{$IF CompilerVersion >= 23.0}
	...
	System.Classes,
	System.Win.ComObj,
	System.Win.Registry,{$IF CompilerVersion >= 25.0}System.AnsiStrings,{$IFEND}
	WinApi.Windows,
	WinApi.Messages,
	WinApi.ActiveX,
	WinApi.ShlObj,
...
type
	...
	TDropContextMenu=class(TInterfacedComponent,IShellExtInit,IContextMenu,IContextMenu2,IContextMenu3)
	private
...
// Delphi XE2
function TDropContextMenu.GetCommandString(idCmd:UINT_PTR;uFlags:UINT;pwReserved:PUINT;pszName:LPSTR;cchMax:UINT)
	:HResult;stdcall;
{$ELSE}

function TDropContextMenu.GetCommandString(idCmd,uType:UINT;pwReserved:PUINT;pszName:LPSTR;cchMax:UINT):HResult;stdcall;
{$IFEND}
var
	ItemIndex:integer;
	MenuItem:TMenuItem;
...
{$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrLCopy(pszName,PAnsiChar(sAnsi),cchMax);
			...
			GCS_VERBA:PAnsiChar(pszName)^:=AnsiChar(0);

			GCS_VERBW:PWideChar(pszName)^:=WideChar(0);
...
{$ENDIF}
	...
	if (FContextMenu=nil)or(HiWord(integer(lpici.lpVerb))<>0) then Exit;
...
{$IF CompilerVersion >= 23.0}WinApi.{$IFEND}Windows.MessageBox(0,PChar(E.Message),'Error',MB_OK or MB_ICONEXCLAMATION or MB_SYSTEMMODAL);
					Result:=E_UNEXPECTED;
				end;
			end;
		finally FFiles.Clear;
		end;
	end
	else Result:=E_INVALIDARG;
end;

function TDropContextMenu.QueryContextMenu(Menu:HMenu;indexMenu,idCmdFirst,idCmdLast,uFlags:UINT):HResult;
	...
	begin
		if (MenuItem.Parent.Parent<>nil) then begin
...
{$ENDIF}
	...
	if (Assigned(FOnPrepareMenu)) then begin
		Continue:=True;
		try FOnPrepareMenu(Self,Continue);
		except
			Result:=E_UNEXPECTED;
			Exit;
		end;
	...
	*)
	...
	if (FContextMenu<>nil)and(uFlags and CMF_DEFAULTONLY=0) then begin
		FMenuOffset:=idCmdFirst;
		NextMenuID:=idCmdFirst;
		for i:=0 to FContextMenu.Items.Count-1 do
			if (FContextMenu.Items[i].Visible) then begin
				FillChar(MenuItemInfo,SizeOf(MenuItemInfo),0);
				MenuItemInfo.cbSize:=SizeOf(MenuItemInfo);
				...
				if (IsLine(FContextMenu.Items[i])) then begin
					...
					if (FContextMenu.Items[i].Count>0) then begin
					...
					//
					if (HasGlyph(FContextMenu.Items[i])) then begin
						if (SupportsOwnerDraw) then begin
					...
					if (not FContextMenu.Items[i].Enabled) then MenuItemInfo.fState:=MenuItemInfo.fState or MFS_DISABLED;

					if (FContextMenu.Items[i].Checked) then MenuItemInfo.fState:=MenuItemInfo.fState or MFS_CHECKED;
				end;
	...
	Result:=MakeResult(SEVERITY_SUCCESS,FACILITY_NULL,NextMenuID-FMenuOffset);
end;
...
begin
	Result:=(not Item.Bitmap.Empty)or((FContextMenu.Images<>nil)and(Item.ImageIndex<>-1));
end;

function TDropContextMenu.Initialize(pidlFolder:PItemIDList;lpdobj:IDataObject;hKeyProgID:HKEY):HResult;
begin
{$IFOPT D+}
...
{$ENDIF}
	...
	try
		...
		if (DataObject<>nil) then
			with TFileDataFormat.Create(dfdConsumer) do
				try
					if GetData(DataObject) then FFiles.Assign(Files);
				finally Free;
				end;
	...
	finally FDataObject:=nil;
	end;
end;

procedure TDropContextMenu.SetContextMenu(const Value:TPopupMenu);
begin
	if (Value<>FContextMenu) then begin
		if (FContextMenu<>nil) then FContextMenu.RemoveFreeNotification(Self);
		FContextMenu:=Value;
...
begin
	if (FFolderPIDL<>Folder) then begin
		if (FFolderPIDL<>nil) then coTaskMemFree(FFolderPIDL);

		FFolderPIDL:=nil;

		if (Folder<>nil) then FFolderPIDL:=ILClone(Folder);
	end;
end;

procedure TDropContextMenu.Notification(AComponent:TComponent;Operation:TOperation);
begin
	if (Operation=opRemove)and(AComponent=FContextMenu) then FContextMenu:=nil;
	inherited;
end;

function TDropContextMenu.ProcessMenuChar(Menu:HMenu;Shortcut:Char):integer;

	function DoProcessMenu(MenuHandle:HMenu;Menu:TMenuItem;var Index:integer;var FirstItem,SelectedItem,NextItem:integer;
		CheckAccelerators:boolean):integer;
	var
		i:integer;
		MenuItemInfo:TMenuItemInfo;
		ItemIndex:integer;
	...
	begin
		...
		for i:=0 to Menu.Count-1 do begin
			if (not Menu[i].Visible) then Continue;
			...
			if (not IsLine(Menu[i]))and(MenuItemInfo.fState and MF_DISABLED=0) then begin
				if (CheckAccelerators) then Hit:=(IsAccel(Ord(Shortcut),Menu[i].Caption))
				else Hit:=(AnsiSameText(Shortcut,Copy(Menu[i].Caption,1,1)))and(GetHotkey(Menu[i].Caption)='');

				if (Hit) then begin
					inc(Result);

					if (FirstItem=-1) then FirstItem:=Index;

					if (MenuItemInfo.fState and MFS_HILITE<>0) then begin
						// Item is already selected
						ASSERT(SelectedItem=-1);
						SelectedItem:=Index;
					end else if (SelectedItem<>-1) then begin
...
begin
	...
	while (Count>0) do begin
		...
		if (MenuItemInfo.wID>=FMenuOffset)and(MenuItemInfo.wID<=FLastMenuID)and(MenuItemInfo.dwItemData<>0) then begin
			// Found an item.
			...
			Count:=DoProcessMenu(Menu,TMenuItem(MenuItemInfo.dwItemData).Parent,i,FirstItem,SelectedItem,NextItem,True);

			// If no accelerator was found, try to match the first char of the caption
			if (Count=0) then begin
				i:=0;
				Count:=DoProcessMenu(Menu,TMenuItem(MenuItemInfo.dwItemData).Parent,i,FirstItem,SelectedItem,NextItem,False);
			end;
...
function TDropContextMenu.GetMenuItem(Index:integer):TMenuItem;
	...
	begin
		...
		Result:=nil;
		while (Result=nil)and(i<Menu.Count) do begin
			if (Menu.Items[i].Visible) then begin
				if (Index=0) then Result:=Menu.Items[i];
				dec(Index);
				if (Result=nil)and(Menu.Items[i].Count>0) then Result:=DoGetMenuItem(Menu.Items[i]);
			end;
			inc(i);
		end;
	end;
...
begin
	if (FContextMenu<>nil) then Result:=DoGetMenuItem(FContextMenu.Items)
	else Result:=nil;
end;
...
begin
	...
	if (FContextMenu=nil)or(DrawItemStruct.CtlType<>ODT_MENU) then Exit;
		...
		try
			...
			try
				...
				*)
				...
				end else if Win98Plus and(odInactive in State) then begin
				...
				if ((MenuItem.Parent<>nil)and(MenuItem.Parent.Parent=nil))and
					not((MenuItem.GetParentMenu<>nil)and(MenuItem.GetParentMenu.OwnerDraw or(MenuItem.GetParentMenu.Images<>nil))
					and(Assigned(MenuItem.OnAdvancedDrawItem)or Assigned(MenuItem.OnDrawItem))) then begin
...
{$IF CompilerVersion < 15.0}
					if (MenuItem.GetParentMenu.Images=nil)or(MenuItem.ImageIndex=-1) then inc(DrawItemStruct.rcItem.Left,8);
{$IFEND}
				end;
				...
				if ((MenuItem.Parent<>nil)and(MenuItem.Parent.Parent=nil)) then begin
					...
					try
						...
						if (not MenuItem.Bitmap.Empty) then begin
						...
						if (ImageList<>nil) then dec(r.Left,ImageList.Width);
						...
						if (not Bitmap.Empty) then begin
						...
						end else if (ImageList<>nil)and(ImageIndex<>-1) then begin
							if (ImageList.Width>16)or(ImageList.Height>16) then begin
							...
							else ImageList.Draw(Canvas,DrawItemStruct.rcItem.Left+(16-ImageList.Width)div 2,
									DrawItemStruct.rcItem.Top+(DrawItemStruct.rcItem.Bottom-DrawItemStruct.rcItem.Top-ImageList.Height)
									div 2,ImageIndex);
				...
				else TMenuItemCracker(MenuItem).AdvancedDrawItem(Canvas,DrawItemStruct.rcItem,State,False);
...
begin
	// Make sure context is valid.
	if (FContextMenu=nil)or(MeasureItemStruct.CtlType<>ODT_MENU) then Exit;
	...
	if (MenuItem<>nil) then begin
		...
		finally ReleaseDC(GetForegroundWindow,DC);
		end;

		// Make room for sub menu arrow
		if (MenuItem.Count>0) then inc(MeasureItemStruct.itemWidth,16);
	end else begin
