...
begin
	...
	try
		...
		if (BitmapInfo^.bmiHeader.biBitCount>8) then begin
			if ((BitmapInfo^.bmiHeader.biCompression and BI_BITFIELDS)<>0) then Inc(InfoSize,12);
		end
		else Inc(InfoSize,sizeof(TRGBQuad)*(1 shl BitmapInfo^.bmiHeader.biBitCount));
...
begin
	...
	try
		...
		if DIB=nil then begin
			...
			if (Stream.Read(DIB^,DIBSize)<>DIBSize) then begin
				GlobalUnlock(Result);
				GlobalFree(Result);
				Result:=0;
			end
...
begin
	if (AMedium.hBitmap<>0) then begin
...
function TPaletteClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var Medium:TStgMedium):boolean;
begin
	Result:=False;

	try Medium.hBitmap:=CopyPalette(FPalette);
	except exit;
	end;

	if (Medium.hBitmap<>0) then begin
...
begin
	Result:=False;
	if (AMedium.hBitmap=0) then exit;
	...
	finally Palette.Free;
	end;
	Result:=True;
end;

function TBitmapClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
var
	Palette:hPalette;
	Format:Word;
	hBitmap:THandle;
...
function TDIBClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
begin
	AMedium.hBitmap:=GetHGlobalDIBFromBitmap(FBitmap);
	Result:=(AMedium.hBitmap<>0);
	if (Result) then AMedium.tymed:=TYMED_HGLOBAL;
end;
...
begin
	...
	if (Length=0) then _RaiseLastWin32Error;
	GetMem(Bits,Length);
	try
		if (GetMetaFileBitsEx(MetaFile.hMF,Length,Bits)<Length) then _RaiseLastWin32Error;
		RefDC:=GetDC(0);
		try Result:=SetWinMetaFileBits(Length,Bits,RefDC,MetaFile);
		finally ReleaseDC(0,RefDC);
		end;
		if (Result=0) then _RaiseLastWin32Error;
	finally FreeMem(Bits);
	end;
end;
...
begin
	...
	try
		...
		if (Result) then FMetaFile.Handle:=WMF2EMF(pMetaFile^);
	finally GlobalUnlock(AMedium.hMetaFilePict);
	end;
end;
...
begin
	...
	if (Result) then FMetaFile.Handle:=CopyEnhMetafile(AMedium.hEnhMetaFile,nil);
end;
...
begin
	...
	if (Source is TDIBClipboardFormat) then FBitmap.Assign(TDIBClipboardFormat(Source).Bitmap)

	else if (Source is TBitmapClipboardFormat) then FBitmap.Assign(TBitmapClipboardFormat(Source).Bitmap)

		// TODO -oanme : Is this nescessary? Palette is extracted in TBitmapClipboardFormat GetData.
	else if (Source is TPaletteClipboardFormat) then FBitmap.Palette:=CopyPalette(TPaletteClipboardFormat(Source).Palette)
...
begin
	...
	if (Dest is TDIBClipboardFormat) then TDIBClipboardFormat(Dest).Bitmap.Assign(FBitmap)

	else if (Dest is TBitmapClipboardFormat) then TBitmapClipboardFormat(Dest).Bitmap.Assign(FBitmap)

	else if (Dest is TPaletteClipboardFormat) then TPaletteClipboardFormat(Dest).Palette:=CopyPalette(FBitmap.Palette)
...
begin
	...
	if (Source is TMetaFileClipboardFormat) then FMetaFile.Assign(TMetaFileClipboardFormat(Source).MetaFile)

	else if (Source is TEnhMetaFileClipboardFormat) then FMetaFile.Assign(TEnhMetaFileClipboardFormat(Source).MetaFile)
...
begin
	...
	if (Result) then begin
		if (FBitmapFormat.HasData) then FPicture.Assign(FBitmapFormat.Bitmap)
		else if (FMetaFileFormat.HasData) then FPicture.Assign(FMetaFileFormat.MetaFile)
		else Result:=False;
	end;
end;
