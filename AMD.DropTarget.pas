...
type
	...
	TDropTargetScrollEvent=procedure(Sender:TObject;Point:TPoint;var Scroll:TScrollDirections;var Interval:integer)
		of object;

	TScrollBars=set of TScrollBarKind;

	TDropTargetEvent=procedure(Sender:TObject;ShiftState:TShiftState;APoint:TPoint;var Effect:Longint) of object;
...
{$ENDIF}
	...
	protected
		...
		procedure DoScroll(Point:TPoint;var Scroll:TScrollDirections;var Interval:integer);virtual;
		...
		function GetValidDropEffect(ShiftState:TShiftState;pt:TPoint;dwEffect:Longint):Longint;virtual;
	...
	published
		...
		property OnGetDropEffect:TDropTargetEvent read FOnGetDropEffect write FOnGetDropEffect;
		property OnScroll:TDropTargetScrollEvent read FOnScroll write FOnScroll;
		property OnStartAsyncTransfer:TNotifyEvent read FOnStartAsyncTransfer write FOnStartAsyncTransfer;
		property OnEndAsyncTransfer:TNotifyEvent read FOnEndAsyncTransfer write FOnEndAsyncTransfer;
		// Drag Images...
		...
		property ActiveDragOver:boolean read FActiveDragOver write FActiveDragOver default True;
		property DropPoint:TPOINT read FPOINT write FPOINT;
	end;
	...
	end {$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0}'Use TCustomDropTarget instead'{$IFEND}{$IFEND};
	...
	/// /////////////////////////////////////////////////////////////////////////////
	TAcceptFormatEvent=procedure(Sender:TObject;const DataFormat:TCustomDataFormat;var Accept:boolean) of object;
	...
	protected
		procedure ClearData;override;
		function DoGetData:boolean;override;
		procedure DoAcceptFormat(const DataFormat:TCustomDataFormat;var Accept:boolean);virtual;
		property OnAcceptFormat:TAcceptFormatEvent read FOnAcceptFormat write FOnAcceptFormat;
	public
		constructor Create(AOwner:TComponent);override;
		destructor Destroy;override;
		function HasValidFormats(const ADataObject:IDataObject):boolean;override;
...
function TCustomDropTarget.DragEnter(const DataObj:IDataObject;grfKeyState:Longint;pt:TPoint;
...
begin
	try
		...
		*)
		if (FTarget=nil)and(FWinTarget=0) then begin
			ShowImage:=False;
			AutoScroll:=False;
		end else begin
			if FTarget=nil then ClientPt:=pt // transfer screen coordinates
			else ClientPt:=FTarget.ScreenToClient(pt);
			FLastPoint:=ClientPt;
		end;
		...
		*)
		if (Enabled)and(HasValidFormats(FDataObject)) then begin
			...
			if FGetDataOnEnter then
				if (not GetData(dwEffect)) then begin
					FDataObject:=nil;
			...
			if ((dwEffect and not(DROPEFFECT_SCROLL))=DROPEFFECT_NONE) then FDataObject:=nil;
		...
		if ShowImage then begin
			...
			if (Succeeded(CoCreateInstance(CLSID_DragDropHelper,nil,CLSCTX_INPROC_SERVER,IDropTargetHelper,FDropTargetHelper))
				)and(FDropTargetHelper<>nil) then begin
				...
				if FTarget=nil then begin
					if (Failed(DropTargetHelper.DragEnter(FWinTarget,DataObj,pt,dwEffect))) then FDropTargetHelper:=nil;
				end else if (Failed(DropTargetHelper.DragEnter(FTarget.Handle,DataObj,pt,dwEffect))) then
						FDropTargetHelper:=nil;
			end;

			if (FDropTargetHelper=nil) then begin
				FDragImageHandle:=ImageList_GetDragImage(nil,@FImageHotSpot);
				if (FDragImageHandle<>0) then begin
					...
					if FTarget=nil then begin
						with ClientPtToWindowPt(FWinTarget,ClientPt) do ImageList_DragEnter(FWinTarget,x,y);
					end
					else
						with ClientPtToWindowPt(FTarget.Handle,ClientPt) do ImageList_DragEnter(FTarget.Handle,x,y);
				end;
			end;
		end
		else FDragImageHandle:=0;
	except
...
begin
	if Assigned(FOnEnter) then FOnEnter(Self,ShiftState,Point,Effect);
end;

function TCustomDropTarget.DragOver(grfKeyState:Longint;pt:TPoint;var dwEffect:Longint):HRESULT;
var
	ShiftState:TShiftState;
	ClientPt:TPoint;
	CanScroll:boolean;
begin
	...
	try
		...
		if (not CustomScrollZone) then ResetScrollZone; // Updates FScrollBars and FNoScrollZone

		// Create a default drop effect based on the shift state and allowed
		// drop effects (or an OnGetDropEffect event if implemented).
		dwEffect:=GetValidDropEffect(ShiftState,ClientPt,dwEffect);

		if (Enabled)and(FDataObject<>nil)and FActiveDragOver then begin
		...
		if (dwEffect and DROPEFFECT_SCROLL<>0) then begin
			if (not FScrollTimer.Enabled) then FScrollTimer.Interval:=DragDropScrollDelay;
			// hardcoded to 100 in previous versions.
		...
		// Move drag image
		if (DropTargetHelper<>nil) then begin
			OleCheck(DropTargetHelper.DragOver(pt,dwEffect));
		end else if (FDragImageHandle<>0) then begin
			if ((FLastPoint.x<>ClientPt.x)or(FLastPoint.y<>ClientPt.y)) then
				with ClientPtToWindowPt(FTarget.Handle,ClientPt) do ImageList_DragMove(x,y);
		end;
...
begin
	if Assigned(FOnDragOver) then FOnDragOver(Self,ShiftState,Point,Effect);
end;
...
begin
	try
		...
		if (not AsyncTransfer) then begin
			ClearData;
			FDataObject:=nil;
		end;
		...
		end else if (FDragImageHandle<>0) then ImageList_DragLeave(FTarget.Handle);
...
function TCustomDropTarget.Drop(const DataObj:IDataObject;grfKeyState:Longint;pt:TPoint;var dwEffect:Longint):HRESULT;
var
	ShiftState:TShiftState;
	ClientPt:TPoint;
begin
	...
	try
		...
		if (not Enabled)or(FDataObject=nil) then begin
			dwEffect:=DROPEFFECT_NONE;
			Result:=E_UNEXPECTED;
		end else begin
			...
			// Create a default drop effect based on the shift state and allowed
			// drop effects (or an OnGetDropEffect event if implemented).
			if (FTarget<>nil) then ClientPt:=FTarget.ScreenToClient(pt)
			else ClientPt:=pt;
			dwEffect:=GetValidDropEffect(ShiftState,ClientPt,dwEffect);
			...
			// get data.
			try
				if (FGetDataOnEnter or GetData(dwEffect)) then begin
					if (not AsyncTransfer) then DoDrop(ShiftState,ClientPt,dwEffect);
				end
		...
		if (DropTargetHelper<>nil) then DropTargetHelper.Drop(DataObj,pt,dwEffect)
		else if (FDragImageHandle<>0)and(FTarget<>nil) then ImageList_DragLeave(FTarget.Handle);
	finally
		// clean up!
		if (not AsyncTransfer) then begin
			ClearData;
...
begin
	...
	if Assigned(FOnDrop) then FOnDrop(Self,ShiftState,Point,Effect);
	...
	*)
	...
	if (Effect<>DROPEFFECT_NONE) then begin
		...
		if (OptimizedMove)and(Effect=DROPEFFECT_MOVE) then DesiredEffect:=DROPEFFECT_COPY
		else DesiredEffect:=Effect;

		if (SetPerformedDropEffect(DesiredEffect)) then Effect:=DesiredEffect;
	end;
end;
...
type
	...
	public
		constructor Create(ADropTarget:TCustomDropTarget;const ADataObject:IDataObject;AEffect:Longint);
		destructor Destroy;override;
...
constructor TDropTargetTransferThread.Create(ADropTarget:TCustomDropTarget;const ADataObject:IDataObject;
	AEffect:Longint);
begin
	...
	OleCheck(CoMarshalInterThreadInterfaceInStream(IDataObjectAsyncCapability,FDataObject,
...
var
	Res:HRESULT;
	FDataObject:IDataObject;
	FAsyncOperation:IDataObjectAsyncCapability;
begin
	CoInitialize(nil);
	try
		try
			OleCheck(CoGetInterfaceAndReleaseStream(IStream(DataObjectStream),IDataObject,FDataObject));
			OleCheck(CoGetInterfaceAndReleaseStream(IStream(AsyncOperationStream),IDataObjectAsyncCapability,
				FAsyncOperation));
			...
			on E:EOleSysError do Res:=E.ErrorCode
			else
				Res:=E_UNEXPECTED;
		end;
...
begin
	...
	// ...Fire event.
	if Assigned(FOnEndAsyncTransfer) then FOnEndAsyncTransfer(Self);
end;

procedure TCustomDropTarget.GetCompatibleClipboardFormats(const DataFormatClass:TDataFormatClass;
...
var
	DoAsync:Bool;
	AsyncOperation:IDataObjectAsyncCapability;
	// h: HResult;
begin
	ClearData;
	...
	*)
	if (not AllowAsyncTransfer)or(not Supports(DataObject,IDataObjectAsyncCapability,AsyncOperation))or
		(Failed(AsyncOperation.GetAsyncMode(DoAsync))) then DoAsync:=False;

	// Start an async data transfer.
	if (DoAsync)and
	// Notify drop source that an async data transfer is starting.
		Succeeded(AsyncOperation.StartOperation(nil)) then begin
		try
			// Fire event.
			if Assigned(FOnStartAsyncTransfer) then FOnStartAsyncTransfer(Self);
			FIsAsync:=True;
			// Create the data transfer thread and launch it.
			with TDropTargetTransferThread.Create(Self,DataObject,Effect) do Start;
...
procedure TCustomDropTarget.Notification(AComponent:TComponent;Operation:TOperation);
begin
	inherited Notification(AComponent,Operation);
	if (Operation=opRemove)and(AComponent is TWinControl) then begin
		if (csDesigning in ComponentState)and(AComponent=FTarget) then FTarget:=nil;
		if (FTargets.IndexOf(TWinControl(AComponent))<>-1) then Unregister(TWinControl(AComponent));
	end;
end;
...
begin
	if (Parent.HandleAllocated) then RevokeDragDrop(Parent.Handle);
...
begin
	if (AutoRegister) then begin
		...
		with TWinControlProxy.Create(Self) do Parent:=ATarget;
	end
	else OleCheck(RegisterDragDrop(ATarget.Handle,Self));
end;
...
procedure TCustomDropTarget.Register(ATarget:TWinControl);
	...
	var
		i:integer;
	begin
		if (Child.Parent<>Parent) then begin
			Result:=False;
			for i:=0 to Parent.ControlCount-1 do
				if (Parent.Controls[i] is TWinControl)and Contains(TWinControl(Parent.Controls[i]),Child) then begin
					Result:=True;
					break;
				end;
		end
...
begin
	...
	if (FTargets.IndexOf(ATarget)<>-1) then exit;

	// Unregister previous target unless MultiTarget is enabled (for backwards
	// compatibility).
	if (not FMultiTarget)and not(csLoading in ComponentState) then Unregister;
	...
	for i:=FTargets.Count-1 downto 0 do
		if Contains(FTargets[i],ATarget) then begin
	...
	if (ATarget is TCustomRichEdit) then RevokeDragDrop(ATarget.Handle);
...
begin
	...
	if (ATarget=nil) then begin
		for i:=FTargets.Count-1 downto 0 do Unregister(FTargets[i]);
		exit;
	end;
	...
	if (ATarget=FTarget) then FTarget:=nil;
...
begin
	...
	if (AutoScroll)and(FTarget<>nil) then begin
		// Determine if the target control has scroll bars (and which).
		TargetStyles:=GetWindowLong(FTarget.Handle,GWL_STYLE);
		if (TargetStyles and WS_HSCROLL<>0) then include(FScrollBars,sbHorizontal);
		if (TargetStyles and WS_VSCROLL<>0) then include(FScrollBars,sbVertical);
		...
		if (FScrollBars<>[]) then begin
			FNoScrollZone:=FTarget.ClientRect;
			if (sbVertical in FScrollBars) then InflateRect(FNoScrollZone,0,-2*GetSystemMetrics(SM_CYHSCROLL));
			// InflateRect(FNoScrollZone, 0, -abs(TDummyWinControl(FTarget).Font.Height));
			if (sbHorizontal in FScrollBars) then InflateRect(FNoScrollZone,-2*GetSystemMetrics(SM_CXHSCROLL),0);
...
begin
	...
	if (AutoRegister) then begin
		...
		for i:=ATarget.ControlCount-1 downto 0 do
			if (ATarget.Controls[i] is TWinControlProxy)and(TWinControlProxy(ATarget.Controls[i]).Owner=Self) then
				with TWinControlProxy(ATarget.Controls[i]) do begin
					Parent:=nil;
					Free;
					break;
				end;
	end else if (ATarget.HandleAllocated) then
...
begin
	...
	while (Result<>nil)and(Targets.IndexOf(Result)=-1) do begin
...
var
	...
	var
		dx,dy:integer;
	begin
		if (p.x<r.Left) then dx:=r.Left-p.x
		else if (p.x>r.Right) then dx:=r.Right-p.x
		else dx:=0;
		if (p.y<r.Top) then dy:=r.Top-p.y
		else if (p.y>r.Bottom) then dy:=r.Bottom-p.y
		else dy:=0;
		Result:=dx*dx+dy*dy;
	end;
...
resourcestring
	sRichEditWarning='It is strongly recommended that you set the AutoRegister'+#13+
...
begin
	...
	if (csDesigning in ComponentState) then begin
		if (Value is TCustomRichEdit)and(AutoRegister) then begin
			Res:=MessageDlg(sRichEditWarning,mtWarning,[mbOK,mbIgnore,mbCancel],0);
			case Res of
				mrCancel:exit;
				mrOK:AutoRegister:=False;
			end;
		end;
		FTarget:=Value;
	end else begin
...
begin
	FShowImage:=Show;
	if (DropTargetHelper<>nil) then DropTargetHelper.Show(Show)
	else if (FDataObject<>nil) then ImageList_DragShowNolock(FShowImage);
end;

function TCustomDropTarget.GetValidDropEffect(ShiftState:TShiftState;pt:TPoint;dwEffect:Longint):Longint;
begin
	...
	if (not Assigned(FOnGetDropEffect)) then Result:=ShiftStateToDropEffect(ShiftState,Result,True);

	// Add scroll effect if necessary.
	if (FAutoScroll)and(FScrollBars<>[]) then begin
		// If the cursor is inside the no-scroll zone, clear the drag scroll flag,
		// otherwise set it.
		if (PtInRect(FNoScrollZone,pt)) then Result:=Result AND NOT integer(DROPEFFECT_SCROLL)
		else Result:=Result OR integer(DROPEFFECT_SCROLL);
	end;

	// Finally give the user a chance to override the 'Default' behaviour with an
	// OnGetDropEffect event handler.
	if Assigned(FOnGetDropEffect) then FOnGetDropEffect(Self,ShiftState,pt,Result);
end;
...
begin
	with TPreferredDropEffectClipboardFormat.Create do
		try
			if GetData(DataObject) then Result:=Value
...
begin
	...
	with TPasteSucceededClipboardFormat.Create do
		try
			...
			if Result then ReleaseStgMedium(Medium);
			Free;
		end;
end;
...
begin
	...
	try
		...
		if (GetData(Effect)) then Result:=Effect
		else Result:=DROPEFFECT_NONE;
...
begin
	if (Source is TClipboard) then PasteFromClipboard
	else if (Source.GetInterface(IDataObject,FDataObject)) then begin
		try
			// Get data from the IDataObject
			if (not GetData(DROPEFFECT_COPY)) then inherited Assign(Source);
		finally
			// Clean up
				FDataObject:=nil;
		end;
...
begin
	...
	if (not First) then begin
		...
		if (dTime>=DWORD(DragDropScrollVelocitySample)) then begin
			...
			if (dTime<>0) then Velocity:=MulDiv(Distance.x*Distance.x+Distance.y*Distance.y,1000*1000,dTime*dTime)
			else Velocity:=0;
			Result:=(Velocity<=DragDropScrollMaxVelocity*DragDropScrollMaxVelocity);
			if (FMouseNextSample.Time=0) then FMouseNextSample:=Sample;
		end;
	end;
	...
	end else if (FMouseNextSample.Time<>0)and(Tick-FMouseNextSample.Time>=DWORD(DragDropScrollVelocitySample)) then begin
...
begin
	...
	// auto-scroll zone).
	if (not PtInRect(FNoScrollZone,FLastPoint)) then begin
		GetCursorPos(pt);
		// Only scroll if mouse velocity is below limit.
		if (SampleMouse(pt)) then
			with FLastPoint do begin
				// Don't scroll unless scroll bars are visible.
				if (sbVertical in FScrollBars) then begin
					...
					if (y<FNoScrollZone.Top) then begin
						if (si.nPos>si.nMin) then include(Scroll,sdUp)
					end else if (y>FNoScrollZone.Bottom) then begin
						if (si.nPos+integer(si.nPage)<=si.nMax) then include(Scroll,sdDown);
					end;
				end;

				if (sbHorizontal in FScrollBars) then begin
					...
					if (x<FNoScrollZone.Left) then begin
						if (si.nPos>si.nMin) then include(Scroll,sdLeft)
					end else if (x>FNoScrollZone.Right) then begin
						if (si.nPos+integer(si.nPage)<=si.nMax) then include(Scroll,sdRight);
					end;
				end;
			end;
	end;

	if (Scroll<>[]) then DoScroll(FLastPoint,Scroll,Interval);
	...
	if (Scroll<>[]) then begin
		...
		if (DropTargetHelper<>nil) then DropTargetHelper.Show(False)
		else if (FDragImageHandle<>0) then ImageList_DragLeave(FTarget.Handle);
		try

			if (sdUp in Scroll) then FTarget.Perform(WM_VSCROLL,SB_LINEUP,0)
			else if (sdDown in Scroll) then FTarget.Perform(WM_VSCROLL,SB_LINEDOWN,0);

			if (sdLeft in Scroll) then FTarget.Perform(WM_HSCROLL,SB_LINEUP,0)
			else if (sdRight in Scroll) then FTarget.Perform(WM_HSCROLL,SB_LINEDOWN,0);

		finally
			// Restore drag image
			if (DropTargetHelper<>nil) then DropTargetHelper.Show(True)
			else if (FDragImageHandle<>0) then
				with ClientPtToWindowPt(FTarget.Handle,FLastPoint) do ImageList_DragEnter(FTarget.Handle,x,y);
		end;
...
procedure TCustomDropTarget.DoScroll(Point:TPoint;var Scroll:TScrollDirections;var Interval:integer);
begin
	if Assigned(FOnScroll) then FOnScroll(Self,FLastPoint,Scroll,Interval);
end;
...
begin
	...
	for i:=FDataFormats.Count-1 downto 0 do FDataFormats[i].Free;
	FDataFormats.Free;
	inherited Destroy;
end;
...
begin
	...
	if (ADataObject.EnumFormatEtc(DATADIR_GET,FormatEnumerator)<>S_OK)or(FormatEnumerator.Reset<>S_OK) then exit;
	...
	while (not Result)and(FormatEnumerator.Next(GetNum,SourceFormatEtc,@GotNum)=S_OK)and(GetNum=GotNum) do begin
		// Determine if any of the associated clipboard formats can
		// read the current data format.
		for i:=0 to FDataFormats.Count-1 do
			if (FDataFormats[i].AcceptFormat(SourceFormatEtc))and(FDataFormats[i].HasValidFormats(ADataObject)) then begin
				Result:=True;
...
var
	i:integer;
begin
	if (AsyncTransfer) then raise Exception.Create(sTargetAsyncBusy);
	for i:=0 to DataFormats.Count-1 do DataFormats[i].Clear;
end;
...
begin
	...
	for i:=0 to DataFormats.Count-1 do begin
		...
		if not(FDataFormats[i].HasValidFormats(DataObject)) then continue;
...
procedure TCustomDropMultiTarget.DoAcceptFormat(const DataFormat:TCustomDataFormat;var Accept:boolean);
begin
	if Assigned(FOnAcceptFormat) then FOnAcceptFormat(Self,DataFormat,Accept);
end;
