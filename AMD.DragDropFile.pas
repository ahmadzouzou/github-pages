...
type
	...
	TFilenameWClipboardFormat=TUnicodeFilenameClipboardFormat {$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0}'Use TUnicodeFilenameClipboardFormat instead'{$IFEND}{$IFEND};
...
{$ENDIF}
	...
	TFilenameMapWClipboardFormat=TUnicodeFilenameMapClipboardFormat {$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0}'Use TUnicodeFilenameMapClipboardFormat instead'{$IFEND}{$IFEND};
...
{$ENDIF}
	...
	public
		...
		function HasData:boolean;override;
		function NeedsData:boolean;override;
		property Files:TUnicodeStrings read FFiles write FFiles;
	end;
...
{$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0}'Use TUnicodeFileGroupDescriptorClipboardFormat instead'{$IFEND}{$IFEND};
...
{$ENDIF}
	...
	TOnGetStreamEvent=procedure(Sender:TFileContentsStreamOnDemandClipboardFormat;Index:Integer;out AStream:IStream)
		of object;
	...
	public
		...
		property FileContentsClipboardFormat:TFileContentsStreamOnDemandClipboardFormat read FFileContentsClipboardFormat;
...
{$ENDIF}
	...
	public
		...
		// E.g. dragging from 'Recycle Bin'.Sourcehandle
...
function ReadFilesFromZeroList(const Data:pointer;Size:Integer;Wide:boolean;Files:TUnicodeStrings):boolean;overload;
function ReadFilesFromZeroList(const Data:pointer;Size:Integer;Wide:boolean;Files:TStrings):boolean;overload;

function WriteFilesToZeroList(Data:pointer;Size:Integer;Wide:boolean;const Files:TUnicodeStrings):boolean;overload;
function WriteFilesToZeroList(Data:pointer;Size:Integer;Wide:boolean;const Files:TStrings):boolean;overload;
...
begin
	if (Dest is TStrings) then begin
		...
		finally TStrings(Dest).EndUpdate;
		end;
	end
...
var
	Temp:AnsiString;
begin
	if (CurIndex<0)or(CurIndex>=Count) then raise Exception.CreateFmt(SListIndexError,[CurIndex]);
	if (NewIndex<0)or(NewIndex>=Count) then raise Exception.CreateFmt(SListIndexError,[NewIndex]);
...
begin
	...
	try Result:=ReadFilesFromData(DropFiles,GlobalSize(HGlob),Files)
...
begin
	...
	try Result:=ReadFilesFromHGlobal(HGlob,Adapter);
...
begin
	Adapter:=TUnicodeStringsAdapter.Create(Files);
	try Result:=ReadFilesFromData(Data,Size,Adapter);
	finally Adapter.Free;
	end;
end;

function ReadFilesFromZeroList(const Data:pointer;Size:Integer;Wide:boolean;Files:TUnicodeStrings):boolean;
var
	p:PAnsiChar;
	StringSize:Integer;
begin
...
function ReadFilesFromZeroList(const Data:pointer;Size:Integer;Wide:boolean;Files:TStrings):boolean;
var
	Adapter:TUnicodeStringsAdapter;
begin
	Adapter:=TUnicodeStringsAdapter.Create(Files);
	try Result:=ReadFilesFromZeroList(Data,Size,Wide,Adapter);
	finally Adapter.Free;
	end;
end;

function WriteFilesToZeroList(Data:pointer;Size:Integer;Wide:boolean;const Files:TUnicodeStrings):boolean;
var
	j:Integer;
	ws:UnicodeString;
	pws:PWideChar;
...
function WriteFilesToZeroList(Data:pointer;Size:Integer;Wide:boolean;const Files:TStrings):boolean;
var
	Adapter:TUnicodeStringsAdapter;
begin
	Adapter:=TUnicodeStringsAdapter.Create(Files);
	try Result:=WriteFilesToZeroList(Data,Size,Wide,Adapter);
	finally Adapter.Free;
	end;
end;
...
begin
	ASSERT(pointer(@(PFileGroupDescriptorA(nil)^.cItems))=pointer(@(PFileGroupDescriptorW(nil)^.cItems)));
	if (FileGroupDescriptor<>nil) then Result:=@(PFileGroupDescriptor(FileGroupDescriptor)^.fgd[Index])
	else Result:=nil;
end;
...
function TFileGroupDescriptorCustomClipboardFormat.WriteData(Value:pointer;Size:Integer):boolean;
var
	BufferSize:Integer;
begin
	ASSERT(pointer(@(PFileGroupDescriptorA(nil)^.cItems))=pointer(@(PFileGroupDescriptorW(nil)^.cItems)));
...
begin
	if (CF_FILEGROUPDESCRIPTOR=0) then CF_FILEGROUPDESCRIPTOR:=RegisterClipboardFormat(CFSTR_FILEDESCRIPTORA);
	Result:=CF_FILEGROUPDESCRIPTOR;
end;

function TAnsiFileGroupDescriptorClipboardFormat.GetFileDescriptor(Index:Integer):PFileDescriptorA;
begin
...
procedure TAnsiFileGroupDescriptorClipboardFormat.SetAnsiFilename(Index:Integer;const Value:AnsiString);
begin
	if (Index>=Count) then raise Exception.CreateFmt('Filename index out of bounds (%d)',[Index]);
...
begin
	if (CF_FILEGROUPDESCRIPTORW=0) then CF_FILEGROUPDESCRIPTORW:=RegisterClipboardFormat(CFSTR_FILEDESCRIPTORW);
	Result:=CF_FILEGROUPDESCRIPTORW;
end;

function TUnicodeFileGroupDescriptorClipboardFormat.GetFileDescriptor(Index:Integer):PFileDescriptorW;
begin
...
procedure TUnicodeFileGroupDescriptorClipboardFormat.SetUnicodeFilename(Index:Integer;const Value:UnicodeString);
begin
	if (Index>=Count) then raise Exception.CreateFmt('Filename index out of bounds (%d)',[Index]);
...
procedure TUnicodeFileGroupDescriptorClipboardFormat.SetFilename(Index:Integer;const Value:string);
begin
...
begin
	...
	*)
	...
	if (Assigned(FOnGetStream))and(AFormatEtcIn.tymed and FormatEtc.tymed<>0)and(Index>=0) then begin
				...
				if (AMedium.HGlobal<>0) then
					try
						try
							...
							try
								OleCheck(Stream.Seek(0,STREAM_SEEK_SET,PUInt64(nil)^));
...
begin
	...
	if (Succeeded((DataFormat.Owner as TCustomDroptarget).DataObject.GetData(AFormatEtc,Medium))) then
		try
			case Medium.tymed of
				...
				TYMED_HGLOBAL: if (Succeeded(CreateStreamOnHGlobal(Medium.HGlobal,(Medium.unkForRelease=nil),Result))) then
...
function TFileContentsStorageClipboardFormat.DoSetData(const AFormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
var
	Index:Integer;
begin
	Index:=AFormatEtcIn.lindex;
		...
		if (Source is TAnsiFileGroupDescriptorClipboardFormat) then begin
			...
			for i:=0 to TAnsiFileGroupDescriptorClipboardFormat(Source).Count-1 do begin
				...
				try
					...
					MultiByteToWideChar(DefaultSystemCodePage,0,FDA^.cFileName,SizeOf(FDA^.cFileName),FDW^.cFileName,
						SizeOf(FDW^.cFileName));
			...
			*)
			if (Source is TUnicodeFileGroupDescriptorClipboardFormat) then begin
				...
				for i:=0 to TUnicodeFileGroupDescriptorClipboardFormat(Source).Count-1 do begin
					GetMem(FDW,SizeOf(TFileDescriptorW));
					try
						Move(TUnicodeFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.fgd[i],FDW^,
							SizeOf(TFileDescriptorW));
						FFileDescriptors.Add(FDW);
					except
						FreeMem(FDW);
						raise;
			...
			if (FFileDescriptors.Count>0) then begin
				...
				try
					...
					for i:=0 to FFileDescriptors.Count-1 do begin
						...
						WideCharToMultiByte(DefaultSystemCodePage,0,PFileDescriptorW(FFileDescriptors[i])^.cFileName,
							SizeOf(PFileDescriptorW(FFileDescriptors[i])^.cFileName),FGDA.fgd[i].cFileName,
							SizeOf(FGDA.fgd[i].cFileName),nil,nil);
					end;
					TAnsiFileGroupDescriptorClipboardFormat(Dest).CopyFrom(FGDA);
				finally FreeMem(FGDA);
				end;
				Result:=True;
			end
			else Result:=False;
			...
			if (Dest is TUnicodeFileGroupDescriptorClipboardFormat) then begin
				if (FFileDescriptors.Count>0) then begin
					...
					try
						...
						for i:=0 to FFileDescriptors.Count-1 do Move(FFileDescriptors[i]^,FGDW.fgd[i],SizeOf(TFileDescriptorW));
						TUnicodeFileGroupDescriptorClipboardFormat(Dest).CopyFrom(FGDW);
					finally FreeMem(FGDW);
					end;
					Result:=True;
				end
				else Result:=False;
...
begin
	Result:=(FFileDescriptors.Count>0)and((FHasContents)or Assigned(FFileContentsClipboardFormat.OnGetStream));
end;
...
begin
	...
	if (Source is TAnsiFilenameMapClipboardFormat) then FFileMaps.Assign(TAnsiFilenameMapClipboardFormat(Source).FileMaps)
...
begin
	...
	if (Dest is TAnsiFilenameMapClipboardFormat) then TAnsiFilenameMapClipboardFormat(Dest).FileMaps.Assign(FFileMaps)
...
begin
	...
	else if (Source is TPIDLClipboardFormat) then FFiles.Assign(TPIDLClipboardFormat(Source).Filenames)
...
function TDropFileTarget.FDGetFiles:TStringList;
begin
	Result:=FFdfiles;
end;

procedure TDropFileTarget.SetFiles(AFiles:TUnicodeStrings);
begin
	FFileFormat.Files.Assign(AFiles);
end;

procedure TDropFileTarget.FDSetFiles(AFiles:TStringList);
begin
