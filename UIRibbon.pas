...
type
	...
	public
		...
		constructor Create(pActionName:string;pActionID:integer;pLabelTitleResourceID:integer=-1;
			pLabelDescriptionResourceID:integer=-1;pTooltipTitleResourceID:integer=-1;
...
type
	...
	TUILoadResourceStringEvent=procedure(const Sender:TUIRibbon;const pCommand:TUICommand;pResourceID:integer;
		var pString:string) of object;
	...
	{ TEventLogger=class(TInterfacedObject,IUIEventLogger)
		FUIRIBBON:TUIRibbon;
		function OnUIEvent(pEventParams:UI_EVENTPARAMS):HRESULT;stdcall;
		published
		property UIRibbon:TUIRibbon READ FUIRIBBON write FUIRIBBON;
		end; }

	TUIRibbon=class(TWinControl,IUIApplication) // ,IUIEventLogger
{$REGION 'Internal Declarations'}
	...
	strict private
		...
		procedure SetQuickAccessToolbarPosition(const Value:TUIQuickAccessToolbarPosition);
...
{$ENDREGION 'Internal Declarations'}
	public
		...
		procedure InvalidateUICommand(const Command:TUICommand;const Aspects:TUICommandInvalidations);overload;
		...
		procedure InvalidateUICommand(const Command:TUICommand;const Prop:TUIProperty);overload;
		...
		procedure AddToRecentItems(const pCaption:string;const pPath:string='';const pDescription:string='');
			deprecated 'Use RecentItems.Add() instead';
		...
		{ Where the Quick Access Toolbar is docked relative to the ribbon (above
			or below }
		property QuickAccessToolbarPosition:TUIQuickAccessToolbarPosition read GetQuickAccessToolbarPosition
			write SetQuickAccessToolbarPosition;
	...
	published
		...
		/// the Ribbon Editor with the Ribbon XML file. IF the value is not set, a file open dialog will be shown.
...
constructor TRibbonMarkupElement.Create(pActionName:string;pActionID:integer;pLabelTitleResourceID:integer=-1;
	pLabelDescriptionResourceID:integer=-1;pTooltipTitleResourceID:integer=-1;pTooltipDescriptionResourceID:integer=-1);
begin
...
begin
	...
	if not Assigned(fContainer) then fContainer:=TObjectList<TRibbonMarkupElementList>.Create(True);
	fContainer.Add(Self);
end;
...
begin
	if not Assigned(fContainer) then Exit(nil);
	for lElement in fContainer do begin
		if SameText(pResourceName,lElement.ResourceName) then Exit(lElement);
	end;
...
begin
	...
	if Command.CommandId=0 then Command.CommandId:=CreateUnusedCommandId
	else fMaxCommandId:=Max(fMaxCommandId,Command.CommandId);
	// Keep track of the highest command id that was used until now.
...
begin
	...
	if not Assigned(ParentForm) then begin
		if csDesigning in ComponentState then MessageDlg(cControlErrorMsg,mtError,[mbOK],0)
		else raise Exception.Create(cControlErrorMsg);
...
begin
	...
	if Visible then begin
		Load();
		if TUIRibbonState.PaintInitialized in fRibbonState then begin
			// Check pending states
			if TUIRibbonState.MinimizePending in fRibbonState then begin
				Minimized:=True;
...
begin
	inherited;
	if Assigned(FRibbon) {$IF CompilerVersion >= 31}and isDpiChange{$IFEND} then Height:=FRibbon.GetHeight;
end;
...
begin
	...
	if roAutoPreserveState in Options then SaveRibbonSettings(); // Save quick toolbar, etc.

	if Assigned(FFramework)and FAvailable then FFramework.Destroy;
...
const
	sNoMappingFound=
		'Action "%s" that maps to Ribbon command with ID %d was not found! Please ensure to update your Ribbon.Markup.xml file if you have renamed an Action.';
var
	lMarkupItem:TRibbonMarkupElement;
	lAction:TCustomAction;
begin
	if pCommand.CommandType=TUICommandType.ctRecentItems then fRecentItems:=(pCommand as TUICommandRecentItems);
	...
	if Assigned(Self.RibbonMapper)and Self.RibbonMapper.TryGetItem(pCommand.CommandId,lMarkupItem) then begin
		// Get the corresponding TAction for the given Ribbon command
		lAction:=Self.GetActionForCommand(pCommand);
		if Assigned(lAction) then pCommand.Assign(lAction)
		else begin
...
{$ENDIF}
	...
	if Assigned(FOnCommandCreate) then FOnCommandCreate(Self,pCommand);
end;
...
begin
	...
	if lResultCharCount<>0 then Result:=lBuffer;
	if Assigned(Self.OnLoadResourceString) then OnLoadResourceString(Self,pCommand,pResourceID,Result);
end;
...
begin
	Result:=0;
	if Assigned(FFramework)and Supports(FFramework,IPropertyStore,PropertyStore) then begin
...
begin
	fUseDarkMode:=pValue;
	if Self.IsLoaded then UpdateDarkModeSetting;
end;
...
var
	...
	begin
		...
		finally lRegistry.Free;
		end;
	end;

begin
	if fUseDarkMode=TDarkMode.Always then lEnable:=True
	else if fUseDarkMode=TDarkMode.Never then lEnable:=FALSE
	else
		try lEnable:=IsDarkThemeEnabled; // fUseDarkMode = TDarkMode.Auto
		except
...
var
	lElement:TRibbonMarkupElement;
begin
	if (not Self.Visible) then Exit(nil);
...
begin
	Result:=True;
	if Assigned(FRibbon)and Supports(FRibbon,IPropertyStore,PropertyStore) then begin
...
begin
	Result:=qpTop;
	if Assigned(FRibbon)and Supports(FRibbon,IPropertyStore,PropertyStore) then begin
		if Succeeded(PropertyStore.GetValue(TPropertyKey(UI_PKEY_QuickAccessToolbarDock),PropValue)) then begin
			UIPropertyToUInt32(UI_PKEY_QuickAccessToolbarDock,PropValue,Dock);
			if (integer(Dock)=Ord(UIControlDockBottom)) then Result:=qpBottom;
		end;
	end;
end;
...
var
	...
	PropValue:TPropVariant;
begin
	if not Available then Exit(FALSE);
...
begin
	for Command in FCommands.Values do
		if (Command.HandleShortCut(ShortCut)) then Exit(True);
	Result:=FALSE;
end;
...
begin
	if not(roAssignImagesFromActionManager in Self.Options) then Exit;
	// ActionManager's images are not actually used -> Nothing todo

	// Update all commands
	for lCommand in Self do begin
		// Check if this command has an action. If yes, use the action's image index.
		if Assigned(lCommand.ActionLink)and Assigned(lCommand.ActionLink.Action)and
			Assigned(TCustomAction(lCommand.ActionLink.Action).ActionList) then begin
...
begin
	...
	for Command in FCommands.Values do begin
		if Assigned(Command.ActionLink.Action) then begin
			if not(Command.CommandType in [TUICommandType.ctRecentItems]) then
				// Recently used items will be updated when the File menu is opened, done in TUICommandRecentItems.DoUpdate()
			...
			if Command.ActionLink.Action is TContainedAction then
					Command.Enabled:=(Command.ActionLink.Action as TContainedAction).Enabled and
					(Command.ActionLink.Action as TContainedAction).Visible;
		end;
	end;
end;

procedure TUIRibbon.InvalidateUICommand(const Command:TUICommand;const Prop:TUIProperty);
var
	CommandId:Cardinal;
	Key:PUIPropertyKey;
begin
	if (FAvailable) then begin
		...
		if Assigned(Key) then begin
			if Assigned(Command) then CommandId:=Command.CommandId
			else CommandId:=UIAllCommands;
...
begin
	if not Assigned(fRecentItems) then fRecentItems:=TUICommandRecentItems.Create(Self,0);
	Exit(fRecentItems);
end;
...
begin
	...
	// ShowMessage(fRibbonSettingsFilePath);
...
begin
	...
	try
		if (Stream.Size>0) then LoadSettings(Stream);
	finally Stream.Free;
	end;
end;
...
begin
	...
	if (not Available) then Height:=0;

	if (Available)and(inherited Visible)and not(FLoaded) then begin
		...
		if (FResourceInstance=0) then Inst:=HInstance
		else Inst:=FResourceInstance;
		try FFramework.LoadUI(Inst,PChar(fResourceName+'_RIBBON'));
		except
		...
		if roAutoPreserveState in Options then LoadRibbonSettings();
		if roAssignImagesFromActionManager in Options then RegisterForImageChanges();
		// If we use images from the action manager, register for changes to the image list, so that icons are refreshed whenever a change occurs.
		...
		if fApplicationModes<>[] then Set_ApplicationModes(fApplicationModes);
		...
		if (lForm.Color=clBtnFace)or(lForm.Color=clWindow) then begin
			...
			else lForm.Color:=Vcl.GraphUtil.GetHighlightColor(Self.BackgroundColor);
			// TColorHelper.IncreaseRgbValues(FRibbon.BackgroundColor, 17, 12, 10)
		end; // if clBtnFace
	end;
end;
...
begin
	...
	if Assigned(fActionManager) then begin
		if not Assigned(fImageChangeLink)and Assigned(fActionManager.Images) then begin
			fImageChangeLink:=TChangeLink.Create;
			fImageChangeLink.OnChange:=ImageListChange;
			fActionManager.Images.RegisterChanges(fImageChangeLink);
		end;

		if not Assigned(fLargeImageChangeLink)and(fActionManager is TActionManager)and
			Assigned((fActionManager as TActionManager).LargeImages) then begin
...
begin
	...
	FAvailable:=Succeeded(CoCreateInstance(CLSID_UIRibbonFramework,nil,CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER,
		IUnknown,Intf));
...
begin
	...
	if fStoredContextTabAvailability.Count>0 then RestoreContextTabAvailability;
end;

procedure TUIRibbon.DestroyWnd;
begin
	inherited;
	if (csRecreating in ControlState) then StoreContextTabAvailability;
	if Assigned(FFramework)and FAvailable then FFramework.Destroy;
	FFramework:=nil;
	if (csRecreating in ControlState) then LoadFramework;
	FCommands.Clear;
	fRecentItems:=nil;
	// Destroying the window handle does not necessarily call OnDestroyUICommand for the recent items command, so we might have to set the reference to nil here.
	FLoaded:=FALSE;
end;
...
var
	lCommand:TUICommand;
begin
	for lCommand in FCommands.Values do begin
...
var
	...
	lCommand:TUICommand;
begin
	for lCommandID in fStoredContextTabAvailability.Keys do begin
...
begin
	...
	if not Self.Visible then Exit(FALSE);
	...
	if not FileExists(RibbonSettingsFilePath) then Exit(FALSE);
	// Otherwise, try to load the file.
	try Self.LoadSettings(lSettingsFileFullPath);
	except
		on E:EOleError do
...
var
	ComStream:IStream;
begin
	if not Assigned(FRibbon) then raise EOleError.Create('Ribbon control was not properly initialzed');
...
begin
	...
	if (pMarkupItem.LabelDescriptionResourceID<>-1)and(pCommand.CommandType=TUICommandType.ctAction) then
		(pCommand as TUICommandAction).LabelDescription:=
			DoLoadResourceString(pCommand,pMarkupItem.LabelDescriptionResourceID);
	// ToolTip title
...
procedure TUIRibbon.InvalidateUICommand(const Command:TUICommand;const Aspects:TUICommandInvalidations);
var
	CommandId:Cardinal;
begin
	if (FAvailable) then begin
...
begin
	...
	try
		...
		if (CommandClass=nil) then Exit(E_FAIL);

		if (not FCommands.TryGetValue(CommandId,Command)) then begin
			...
			else Command:=CommandClass.Create(Self,CommandId);
			DoCommandCreated(Command);
		end;

		CommandHandler:=Command;
		TUICommandAccess(Command).Alive:=True;
	except Application.HandleException(Self); // Process otherwise completely unhandled exceptions in this scenario.
	end;
end;
...
begin
	...
	if TUICommandType(TypeId)=TUICommandType.ctRecentItems then fRecentItems:=nil;
	if Assigned(FOnCommandDestroy)and TryGetCommand(CommandId,lCommand) then FOnCommandDestroy(Self,lCommand);
	Result:=S_OK;
end;

function TUIRibbon.OnViewChanged(ViewId:UInt32;TypeId:_UIViewType;const View:IInterface;Verb:_UIViewVerb;
	ReasonCode:Int32):HRESULT;
var
	NewHeight:integer;
begin
	Result:=E_NOTIMPL;
	try
		...
		if (TypeId=UIViewTypeRibbon) then begin
			case Verb of
				{ The view was newly created. }
				UIViewVerbCreate:begin
						FRibbon:=View as IUIRibbon;
						Result:=S_OK;
					end;
				{ The view has been resized. For the Ribbon view, the application should
					call GetHeight to determine the height of the ribbon. }
				UIViewVerbSize:begin
						FRibbon:=View as IUIRibbon;
...
begin
	...
	for Mode in Modes do
		if (Mode>=0)and(Mode<=31) then AppModes:=AppModes or(1 shl Mode);
...
begin
	...
	try Result:=SaveSettings(Stream);
	finally Stream.Free;
	end;
end;
...
begin
	...
	try
		...
		on E:EFileStreamError do begin
			OutputDebugString(PChar('An EFileStreamError error occurred while trying to save the ribbon settings: '+
				E.Message));
{$IFDEF DEBUG}
			raise E;
{$ENDIF}
...
begin
	...
	if (Result)and FLoaded then begin
		ComStream:=TStreamAdapter.Create(Stream,soReference);
...
begin
	if (FAvailable) then begin
		Load();
		if (Modes=0) then FFramework.SetModes(1)
		else FFramework.SetModes(Modes);
	end;
end;
...
procedure TUIRibbon.SetColor(const PropKey:TUIPropertyKey;const Value:TUIHsbColor);
var
	PropertyStore:IPropertyStore;
	PropValue:TPropVariant;
begin
...
procedure TUIRibbon.SetContextTabAvailability(const pCommandId:integer;
	const pContextAvailability:TUIContextAvailability);
var
	lCommand:TUICommand;
begin
	// If Ribbons are disabled, exit here.
...
begin
	...
	if (lCommand.CommandType=TUICommandType.ctContext) then Result:=(lCommand as TUICommandContext).Availability;
end;
...
var
	...
	PropValue:TPropVariant;
begin
	if Assigned(FRibbon)and Supports(FRibbon,IPropertyStore,PropertyStore)and
		(TUIRibbonState.PaintInitialized in fRibbonState) then begin
		...
		PropertyStore.Commit;
	end else if Value then fRibbonState:=fRibbonState+[TUIRibbonState.MinimizePending];
	// Ribbon hasn't received a paint message yet -> Postpone until WMPaint is called.
end;

procedure TUIRibbon.SetQuickAccessToolbarPosition(const Value:TUIQuickAccessToolbarPosition);
var
	PropertyStore:IPropertyStore;
	PropValue:TPropVariant;
	Dock:Cardinal;
begin
	if Assigned(FRibbon)and Supports(FRibbon,IPropertyStore,PropertyStore) then begin
		if (Value=qpBottom) then Dock:=Ord(UIControlDockBottom)
		else Dock:=Ord(UIControlDockTop);
...
procedure TUIRibbon.AddToRecentItems(const pCaption:string;const pPath:string='';const pDescription:string='');
begin
	RecentItems.Add(pCaption,pPath,pDescription);
end;
...
var
	...
	PropValue:TPropVariant;
begin
	if Value then Self.Load();
	if Assigned(FRibbon)and Supports(FRibbon,IPropertyStore,PropertyStore)and FLoaded then begin
...
procedure TUIRibbon.ShowContextPopup(const PopupId:Cardinal;const ScreenPos:TPoint);
var
	Intf:IInterface;
