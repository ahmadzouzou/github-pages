...
begin
	...
	S:=(S*255)div 240;
	LD:=L/240;
	if (LD>0.9821) then B:=255
	else if (LD<0.1793) then B:=0
	else B:=Trunc(257.7+149.9*Ln(LD)+0.5);
	Result:=UIHsb(H,S,B);
end;
...
begin
	SetString(Result,Buffer,LoadString(HInstance,ResourceId,Buffer,Length(Buffer)));
end;
