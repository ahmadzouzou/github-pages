...
{$IF CompilerVersion >= 23.0}
	System.SysUtils,
	System.Classes,
	System.Win.ComObj,{$IF CompilerVersion >= 25.0}System.AnsiStrings,{$IFEND}
	WinApi.Windows,
	WinApi.ShlObj,
	WinApi.ActiveX,
{$ELSE}
...
type
	...
	TURLWClipboardFormat=TUnicodeURLClipboardFormat {$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0}'Use TURLWClipboardFormat instead'{$IFEND}{$IFEND};
...
type
	...
	TOpenIMsgOnIStg=function(lpMsgSess:pointer;lpAllocateBuffer:pointer;lpAllocateMore:pointer;lpFreeBuffer:pointer;
		lpMalloc:IMalloc;lpMapiSup:pointer;lpStg:IStorage;lpfMsgCallRelease:pointer;ulCallerData:ULONG;ulFlags:ULONG;
		out lppMsg:IUnknown):SCODE;stdcall;
...
begin
	...
	try Result:=GetURLFromStream(Stream,URL);
	finally Stream.Free;
	end;
end;
...
begin
	...
	try
		...
		i:=0;
		while (not Result)and(i<URLfile.Count-1) do begin
			if (CompareText(URLfile[i],sInternetShortcut)=0) then begin
				inc(i);
				while (not Result)and(i<URLfile.Count) do begin
					s:=AnsiString(URLfile[i]);
					p:=PAnsiChar(s);
					if ({$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrLIComp(p,'URL=',Length('URL='))=0) then begin
						inc(p,Length('URL='));
						URL:=p;
						Result:=True;
					end else if (p^='[') then exit;
					inc(i);
				end;
			end;
			inc(i);
		end;
	finally URLfile.Free;
	end;
end;
...
const
	...
	Protocols: array [0..3] of string=('http://','ftp://','mailto:','file:');
var
	i:integer;
	LastInvalid:boolean;
	LowerUrl:string;
begin
	...
	for i:=Low(Protocols) to High(Protocols) do
		if (StrLIComp(PChar(LowerUrl),PChar(Protocols[i]),Length(Protocols[i]))=0) then begin
			Delete(Result,1,Length(Protocols[i]));
			break;
		end;

	if (Length(Result)>120) then SetLength(Result,120);
	...
	for i:=Length(Result) downto 1 do
		if (Result[i] in Invalids) then begin
			if (not LastInvalid) then begin
				Result[i]:=' ';
				LastInvalid:=True;
			end
			else
		...
		else LastInvalid:=False;

	if Result='' then Result:='untitled';
...
function MakeHTML(const s:string):AnsiString;
const
	Header:string='Version:0.9'+#13#10+'StartHTML:%.00008d'+#13#10+'EndHTML:%.00008d'+#13#10+'StartFragment:%.00008d'+
		#13#10+'EndFragment:%.00008d'+#13#10;
	WrapperStart:string='<HTML>'+#13#10+'<BODY>'+#13#10+'<!--StartFragment-->';
	WrapperEnd:string='<!--EndFragment-->'+#13#10+'</BODY>'+#13#10+'</HTML>';
var
	Fragment:string;
	StartHTML:integer;
	EndHTML:integer;
...
begin
	...
	*)
	...
	else Result:=AnsiString(s);
end;

function MakeTextFromHTML(const s:AnsiString;FullHTML:boolean):string;
const
	Sections: array [boolean,0..1] of AnsiString=(('StartFragment:','EndFragment'),('StartHTML','EndHTML'));
var
	n1,n2:integer;
	p:PAnsiChar;
begin
...
begin
	...
	if (CF_URL=0) then CF_URL:=RegisterClipboardFormat(CFSTR_SHELLURL);
	Result:=CF_URL;
end;
...
begin
	if (CF_INETURLW=0) then CF_INETURLW:=RegisterClipboardFormat('UniformResourceLocatorW'); // *** DO NOT LOCALIZE ***
	Result:=CF_INETURLW;
end;
...
begin
	if (CF_NETSCAPEBOOKMARK=0) then CF_NETSCAPEBOOKMARK:=RegisterClipboardFormat('Netscape Bookmark');
	// *** DO NOT LOCALIZE ***
...
begin
	...
	if (FURL<>'') then begin
		inc(Result,1024);
		if (FTitle<>'') then inc(Result,1024);
	end;
end;

function TNetscapeBookmarkClipboardFormat.ReadData(Value:pointer;Size:integer):boolean;
begin
...
function TNetscapeBookmarkClipboardFormat.WriteData(Value:pointer;Size:integer):boolean;
begin
{$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrLCopy(Value,PAnsiChar(FURL),Size);
	dec(Size,1024);
	if (Size>0)and(FTitle<>'') then begin
		inc(PAnsiChar(Value),1024);
...
begin
	if (CF_NETSCAPEIMAGE=0) then CF_NETSCAPEIMAGE:=RegisterClipboardFormat('Netscape Image Format');
	Result:=CF_NETSCAPEIMAGE;
end;

type
	TNetscapeImageRec=record
		Size,_Unknown1,Width,Height,HorMargin,VerMargin,Border,OfsLowRes,OfsTitle,OfsURL,OfsExtra:DWORD end;
		...
		begin
			...
			if (FLowRes<>'') then inc(Result,Length(FLowRes)+1);
			if (FTitle<>'') then inc(Result,Length(FTitle)+1);
			if (FURL<>'') then inc(Result,Length(FURL)+1);
			if (FExtra<>'') then inc(Result,Length(FExtra)+1);
		end;

		function TNetscapeImageClipboardFormat.ReadData(Value:pointer;Size:integer):boolean;
		begin
			...
			if (Result) then begin
				...
				if (PNetscapeImageRec(Value)^.OfsLowRes<>0) then FLowRes:=PAnsiChar(Value)+PNetscapeImageRec(Value)^.OfsLowRes;
				if (PNetscapeImageRec(Value)^.OfsTitle<>0) then FTitle:=PAnsiChar(Value)+PNetscapeImageRec(Value)^.OfsTitle;
				if (PNetscapeImageRec(Value)^.OfsURL<>0) then FURL:=PAnsiChar(Value)+PNetscapeImageRec(Value)^.OfsURL;
				if (PNetscapeImageRec(Value)^.OfsExtra<>0) then FExtra:=PAnsiChar(Value)+PNetscapeImageRec(Value)^.OfsExtra;
			end;
		end;

		function TNetscapeImageClipboardFormat.WriteData(Value:pointer;Size:integer):boolean;
...
{$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrLCopy(Value,PAnsiChar(FExtra),Size);
	...
	var
		...
		begin
			if (CF_VCARD=0) then CF_VCARD:=RegisterClipboardFormat('+//ISBN 1-887687-00-9::versit::PDI//vCard');
			// *** DO NOT LOCALIZE ***
			Result:=CF_VCARD;
		end;
	...
	begin
		if (Items.Count>0) then begin
			...
			for i:=0 to Items.Count-1 do inc(Result,Length(Items[i])+1);
		end
		else Result:=0;
	end;
	...
	begin
		...
		if (Result) then begin
			...
			if (Items.Count>0)and(CompareText(Items[0],'begin:vcard')=0) then Items.Delete(0);
			if (Items.Count>0)and(CompareText(Items[Items.Count-1],'end:vcard')=0) then Items.Delete(Items.Count-1);
...
{$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrLCopy(Value,PAnsiChar(s),Size);
	...
	var
		...
		begin
			if (CF_HTML=0) then CF_HTML:=RegisterClipboardFormat('HTML Format');
			Result:=CF_HTML;
		end;
		...
		begin
			Result:=True;
			if (Source is TTextDataFormat) then begin
				if IsHTML(TTextDataFormat(Source).Text) then HTML.Text:=TTextDataFormat(Source).Text
				else HTML.Text:=String(MakeHTML(TTextDataFormat(Source).Text));
			end
		...
		begin
			Result:=True;
			if (Dest is TTextDataFormat) then begin
				...
				if (TTextDataFormat(Dest).Text='') then TTextDataFormat(Dest).Text:=HTML.Text;
			end
	...
	var
		...
		begin
			if (CF_RFC822=0) then CF_RFC822:=RegisterClipboardFormat('Internet Message (rfc822/rfc1522)');
			// *** DO NOT LOCALIZE ***
			Result:=CF_RFC822;
		end;
		...
		begin
			Result:=True;
			if (Source is TTextDataFormat) then Text.Text:=TTextDataFormat(Source).Text
			else Result:=inherited Assign(Source);
		end;
		...
		begin
			Result:=True;
			if (Dest is TTextDataFormat) then TTextDataFormat(Dest).Text:=Text.Text
			else Result:=inherited AssignTo(Dest);
		end;
	...
	begin
		...
		(*
			** TAnsiURLClipboardFormat
		*)
		if (Source is TAnsiURLClipboardFormat) then begin
			if (FURL='') then FURL:=TAnsiURLClipboardFormat(Source).URL;
			Result:=True;
		end
		else
			(*
			...
			*)
			if (Source is TAnsiTextClipboardFormat) then begin
				...
				*)
				if (Source is TFileClipboardFormat) then begin
					if (FURL='') then begin
						s:=TFileClipboardFormat(Source).Files[0];
						// Convert from Internet Shortcut file format.
						if (SameText(ExtractFileExt(s),sInternetShortcutExt))and(GetURLFromFile(s,NewUrl)) then begin
							FURL:=NewUrl;
							if (FTitle='') then FTitle:=ChangeFileExt(ExtractFileName(s),'');
							Result:=True;
						end;
					end;
				end
					...
					*)
					if (Source is TFileContentsClipboardFormat) then begin
						...
						*)
						if (Source is TAnsiFileGroupDescriptorClipboardFormat) then begin
							if (FTitle='') then begin
								if (TAnsiFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.cItems>0) then begin
									...
									if (CompareText(ExtractFileExt(s),sInternetShortcutExt)=0) then begin
							...
							*)
							if (Source is TUnicodeFileGroupDescriptorClipboardFormat) then begin
								if (FTitle='') then begin
									if (TUnicodeFileGroupDescriptorClipboardFormat(Source).FileGroupDescriptor^.cItems>0) then begin
										// Extract the title of an Internet Shortcut
	...
	var
		...
		FGDW:AMD.DragDropFile.TFileGroupDescriptorW;
		Filename:AnsiString;
	begin
		Result:=True;
		(*
			** TAnsiURLClipboardFormat
		*)
		if (Dest is TAnsiURLClipboardFormat) then begin
			...
			(*
				** TAnsiTextClipboardFormat
			*)
			if (Dest is TAnsiTextClipboardFormat) then begin
				...
				(*
					** TFileContentsClipboardFormat
				*)
				if (Dest is TFileContentsClipboardFormat) then begin
					TFileContentsClipboardFormat(Dest).Data:=sInternetShortcut+#13#10+'URL='+URL+#13#10;
				end
				else
					(*
						** TAnsiFileGroupDescriptorClipboardFormat
					*)
					if (Dest is TAnsiFileGroupDescriptorClipboardFormat) then begin
						...
						FGDA.cItems:=1;
						if (Title<>'') then Filename:=AnsiString(Title)
						else Filename:=URL;
{$IF CompilerVersion >= 25.0}System.AnsiStrings.{$IFEND}StrPLCopy(@FGDA.fgd[0].cFileName[0],ConvertURLToFilename(Filename),SizeOf(FGDA.fgd[0].cFileName));
						...
						*)
						if (Dest is TUnicodeFileGroupDescriptorClipboardFormat) then begin
							...
							WStrPLCopy(@FGDW.fgd[0].cFileName[0],WideString(ConvertURLToFilename(Filename)),
	...
	begin
		...
		if (Source is THTMLClipboardFormat) then FHTML.Assign(THTMLClipboardFormat(Source).HTML)
	...
	begin
		...
		if (Dest is THTMLClipboardFormat) then THTMLClipboardFormat(Dest).HTML.Assign(FHTML)
	...
	begin
		Changing;
		if (FStorages<>nil) then FStorages.Clear;
	end;
	...
	begin
		...
		if (Source is TFileContentsStorageClipboardFormat) then begin
			FStorages.Assign(TFileContentsStorageClipboardFormat(Source).Storages);
			Result:=True;
		end else if (Source is TFileGroupDescriptorCustomClipboardFormat) then begin
			// Can only assign name if interface has already been assigned
			if (TFileGroupDescriptorCustomClipboardFormat(Source).Count=FStorages.Count) then begin
				Result:=True;
	...
	begin
		...
		if (Dest is TFileContentsStorageClipboardFormat) then begin
			TFileContentsStorageClipboardFormat(Dest).Storages.Assign(FStorages);
			Result:=True;
		end else

			if (Dest is TFileGroupDescriptorCustomClipboardFormat) then begin
			// Only handle FGD if we actually have filenames
			for i:=0 to FStorages.Count-1 do
				if (FStorages.Names[i]<>'') then begin
					Result:=True;
					break;
				end;
			if (Result) then begin
	...
	const
		...
		begin
			if (MAPI=0) then begin
				MAPI:=SafeLoadLibrary(MAPIDLL);
				if (MAPI<=HINSTANCE_ERROR) then raise Exception.CreateFmt('%s: %s',[SysErrorMessage(GetLastError),MAPIDLL]);
{$IFDEF WIN32}
...
{$ENDIF}
	...
	begin
		...
		if (FSessionCount>0) then begin
			if (FSessionLock=0) then dec(FSessionCount);
			if (FSessionCount=0) then begin
	...
	var
		i:integer;
	begin
		if (FStorages.Count<>FMessages.Count) then begin
			FMessages.Capacity:=FStorages.Count;
			for i:=0 to FStorages.Count-1 do FMessages.Add(nil);
		end;

		if (FMessages[Index]=nil) then begin
			LoadMAPI;
			BeginSession;
...
{$IF CompilerVersion >= 25.0}
				pointer(@MAPIAllocateBuffer),pointer(@MAPIAllocateMore),pointer(@MAPIFreeBuffer),
{$ELSE}
				@MAPIAllocateBuffer,@MAPIAllocateMore,@MAPIFreeBuffer,
{$IFEND}
				IMalloc(MAPIGetDefaultMalloc),nil,FStorages[Index],nil,0,0,Result));

			FMessages[Index]:=Result;
		end
		else Result:=FMessages[Index];
	end;
	...
	begin
		dec(FSessionLock);
		if (FSessionLock=0) then EndSession;
	end;
	...
	begin
		...
		if (Result=DROPEFFECT_NONE) then Result:=DROPEFFECT_LINK;
	end;
