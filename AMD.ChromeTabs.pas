...
type
	...
	TOnButtonBeforCloseTabClick=procedure(Sender:TObject;ATab:TChromeTab;var Close:Boolean) of object;
	TOnButtonAfterCloseTabClick=procedure(Sender:TObject;ATab:TChromeTab;counts:integer) of object;
	...
	TOnBeforeDrawItem=procedure(Sender:TObject;TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;
		TabIndex:integer;var Handled:Boolean) of object;
	TOnAfterDrawItem=procedure(Sender:TObject;const TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;
		TabIndex:integer) of object;
	TOnGetControlPolygons=procedure(Sender:TObject;ChromeTabsControl:TObject;ItemRect:TRect;ItemType:TChromeTabItemType;
		Orientation:TTabOrientation;var Polygons:IChromeTabPolygons) of object;
	TOnShowHint=procedure(Sender:TObject;HitTestResult:THitTestResult;var HintText:String;var HintTimeout:integer)
		of object;
	TOnTabDblClick=procedure(Sender:TObject;ATab:TChromeTab) of object;
	TOnTabClick=procedure(Sender:TObject;ATab:TChromeTab) of object;
	TOnTabDragStart=procedure(Sender:TObject;ATab:TChromeTab;var Allow:Boolean) of object;
	TOnTabDragOver=procedure(Sender:TObject;X,Y:integer;State:TDragState;DragTabObject:IDragTabObject;var Accept:Boolean)
		of object;
	TOnTabDragDrop=procedure(Sender:TObject;X,Y:integer;DragTabObject:IDragTabObject;Cancelled:Boolean;
		var TabDropOptions:TTabDropOptions) of object;
	TOnTabDragDropped=procedure(Sender:TObject;DragTabObject:IDragTabObject;NewTab:TChromeTab) of object;
	TOnTabDragEnd=procedure(Sender:TObject;MouseX,MouseY:integer;DragTabObject:IDragTabObject;Cancelled:Boolean)
		of object;
	TOnAnimateStyle=procedure(Sender:TObject;ChromeTabsControl:TBaseChromeTabsControl;NewDrawState:TDrawState;
		var AnimationTimeMS:Cardinal;var EaseType:TChromeTabsEaseType) of object;
	TOnAnimateMovement=procedure(Sender:TObject;ChromeTabsControl:TBaseChromeTabsControl;var AnimationTimeMS:Cardinal;
		var EaseType:TChromeTabsEaseType) of object;
	...
	private
		...
		procedure TabDragDrop(Sender:TObject;X,Y:integer;DragTabObject:IDragTabObject;Cancelled:Boolean;
			var TabDropOptions:TTabDropOptions);
		procedure DragCompleted(MouseX,MouseY:integer;DragTabObject:IDragTabObject;Cancelled:Boolean);
	...
	private
		...
		FOnButtonBeforCloseTabClick:TOnButtonBeforCloseTabClick;
		FOnButtonAfterCloseTabClick:TOnButtonAfterCloseTabClick;
		...
		procedure SetControlDrawState(ChromeTabsControl:TBaseChromeTabsControl;NewDrawState:TDrawState;
	...
	protected
		...
		procedure WMLButtonClk(var Message:TWMLButtonUp);message WM_LBUTTONUP;
...
{$ENDIF}
		...
		procedure DoOnButtonBeforCloseTabClick(ATab:TChromeTab;var AllowClose:Boolean);virtual;
		procedure DoOnButtonAfterCloseTabClick(ATab:TChromeTab;counts:integer);virtual;
		...
		procedure DoOnTabClick(ATab:TChromeTab);virtual;
		...
		procedure DoOnTabDragDrop(X,Y:integer;DragTabObject:IDragTabObject;Cancelled:Boolean;
			var TabDropOptions:TTabDropOptions);virtual;
		procedure DoOnTabDragDropped(DragTabObject:IDragTabObject;NewTab:TChromeTab);virtual;
		procedure DoOnTabDragEnd(MouseX,MouseY:integer;DragTabObject:IDragTabObject;Cancelled:Boolean);virtual;
		...
		procedure DoOnAnimateStyle(ChromeTabsControl:TBaseChromeTabsControl;NewDrawState:TDrawState;
			var AnimationTimeMS:Cardinal;var EaseType:TChromeTabsEaseType);virtual;
		procedure DoOnAnimateMovement(ChromeTabsControl:TBaseChromeTabsControl;var AnimationTimeMS:Cardinal;
			var EaseType:TChromeTabsEaseType);virtual;
		...
		// Virtual (IChromeTabs)
		procedure DoOnBeforeDrawItem(TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;TabIndex:integer;
			var Handled:Boolean);virtual;
		procedure DoOnAfterDrawItem(const TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;
			TabIndex:integer);virtual;
		procedure DoOnGetControlPolygons(ChromeTabsControl:TObject;ItemRect:TRect;ItemType:TChromeTabItemType;
			Orientation:TTabOrientation;var Polygons:IChromeTabPolygons);virtual;
	...
	protected
		...
		property OnButtonBeforCloseTabClick:TOnButtonBeforCloseTabClick read FOnButtonBeforCloseTabClick
			write FOnButtonBeforCloseTabClick;
		property OnButtonAfterCloseTabClick:TOnButtonAfterCloseTabClick read FOnButtonAfterCloseTabClick
			write FOnButtonAfterCloseTabClick;
	...
	published
		...
		property OnButtonBeforCloseTabClick;
		property OnButtonAfterCloseTabClick;
...
begin
	...
	finally FreeAndNil(WindowFromPointFixThread);
	end;
end;
...
function TCustomChromeTabs.GetPreviousTabPolygons(Index:integer):IChromeTabPolygons;
var
	PreviousIndex:integer;
begin
	PreviousIndex:=GetLastVisibleTabIndex(Index-1);

	if PreviousIndex=-1 then Result:=nil
	else Result:=TabControls[PreviousIndex].GetPolygons;
end;
...
begin
	...
	for i:=0 to pred(GetPinnedTabCount) do
		if (Tabs[i].Visible)and((IncludeMarkedForDelete)or(not Tabs[i].MarkedForDeletion)) then Inc(Result);
end;
...
begin
	...
	if HitTestResult.TabIndex=-1 then EndDrag(MouseX,MouseY,TRUE)
	else begin
		...
		if not Allow then EndDrag(MouseX,MouseY,TRUE)
		else begin
			...
			FDragTabObject.DragCursorOffset:=Point(MouseX-BidiXPos(TabControls[HitTestResult.TabIndex].ControlRect.Left)+
				BidiScrollOffset,MouseY-TabControls[HitTestResult.TabIndex].ControlRect.Top);
...
begin
	if FUpdateCount=0 then FPausedUpdateCount:=0;
...
begin
	if ScrollingActive then Result:=tdScrolling
	else if (GetVisibleTabCount=0)or
		((TabControls[GetLastVisibleTabIndex(Tabs.Count-1)].EndRect.Right<TabContainerRect.Right)) then Result:=tdNormal
	else Result:=tdCompressed;
end;
...
begin
	...
	if FActiveDragTabObject<>nil then begin
		BeginUpdate;
		try
			// Move the tab between the controls
			AddState(stsAddingDroppedTab);
			try Result:=Tabs.Add;
			finally RemoveState(stsAddingDroppedTab);
			end;
			...
			if FDragTabControl<>nil then begin
				SetControlPosition(TabControls[FActiveDragTabObject.DropTabIndex],FDragTabControl.ControlRect,FALSE);
			end;
...
{$IFDEF LCL}
	...
	// Fix the draw states
	if FAddButtonControl.DrawState=dsUnknown then FAddButtonControl.SetDrawState(dsNotActive,0,ttNone,TRUE);

	if FScrollButtonLeftControl.DrawState=dsUnknown then FScrollButtonLeftControl.SetDrawState(dsNotActive,0,ttNone,TRUE);
...
procedure TCustomChromeTabs.DoOnAnimateMovement(ChromeTabsControl:TBaseChromeTabsControl;var AnimationTimeMS:Cardinal;
	var EaseType:TChromeTabsEaseType);
begin
	if Assigned(FOnAnimateMovement) then FOnAnimateMovement(Self,ChromeTabsControl,AnimationTimeMS,EaseType);
end;

procedure TCustomChromeTabs.DoOnAnimateStyle(ChromeTabsControl:TBaseChromeTabsControl;NewDrawState:TDrawState;
	var AnimationTimeMS:Cardinal;var EaseType:TChromeTabsEaseType);
begin
	if Assigned(FOnAnimateStyle) then FOnAnimateStyle(Self,ChromeTabsControl,NewDrawState,AnimationTimeMS,EaseType);
end;

procedure TCustomChromeTabs.SetControlDrawState(ChromeTabsControl:TBaseChromeTabsControl;NewDrawState:TDrawState;
	ForceUpdate:Boolean);
var
	AnimationTimeMS:Cardinal;
	EaseType:TChromeTabsEaseType;
begin
	if (ChromeTabsControl.DrawState<>NewDrawState)or(ForceUpdate) then begin
		if (not(csDesigning in ComponentState))and
			(((ChromeTabsControl.DrawState=dsHot)and((NewDrawState<>dsActive))or(not(ChromeTabsControl is TChromeTabControl)))
			or((NewDrawState=dsHot)and(ChromeTabsControl.DrawState<>dsActive))or(not(ChromeTabsControl is TChromeTabControl)))
		then EaseType:=FOptions.Animation.DefaultMovementEaseType
		else EaseType:=ttNone;
...
begin
	...
	if FActiveDragTabObject=nil then begin
		...
		try
			...
			if (ForceUpdate)or((not HasState(stsPendingUpdates))and
				((FLastHitTestResult.HitTestArea<>HitTestResult.HitTestArea)or
				(FLastHitTestResult.TabIndex<>HitTestResult.TabIndex))) then begin
				FLastHitTestResult:=HitTestResult;

				if (BiDiMode in BidiLeftToRightTabModes)and(ScrollOffset=0)or(BiDiMode in BidiRightToLeftTabModes)and
					(ScrollOffset=GetMaxScrollOffset) then SetControlDrawState(FScrollButtonLeftControl,dsDisabled,ForceUpdate)
				else SetControlDrawState(FScrollButtonLeftControl,dsActive,ForceUpdate);
				...
				else SetControlDrawState(FScrollButtonRightControl,dsActive,ForceUpdate);

				// Clear new button hot state
				SetControlDrawState(FAddButtonControl,dsNotActive,ForceUpdate);

				if HitTestResult.HitTestArea=htAddButton then begin
					if HasState(stsMouseDown) then SetControlDrawState(FAddButtonControl,dsDown,ForceUpdate)
					else SetControlDrawState(FAddButtonControl,dsHot,ForceUpdate);
				end else

					if HitTestResult.HitTestArea=htScrollButtonLeft then begin
					if ScrollOffset>0 then begin
						if HasState(stsMouseDown) then SetControlDrawState(FScrollButtonLeftControl,dsDown,ForceUpdate)
						else SetControlDrawState(FScrollButtonLeftControl,dsHot,ForceUpdate);
					end;
				end else

					if HitTestResult.HitTestArea=htScrollButtonRight then begin
					if ScrollOffset<GetMaxScrollOffset then begin
						if HasState(stsMouseDown) then SetControlDrawState(FScrollButtonRightControl,dsDown,ForceUpdate)
						else SetControlDrawState(FScrollButtonRightControl,dsHot,ForceUpdate);
					end;
				end else begin
					// Clear hot tabs
					for i:=0 to pred(Tabs.Count) do begin
						if (HitTestResult.TabIndex=i)and(not HasState(stsDragging)) then begin
							if not Tabs[i].Active then SetControlDrawState(TabControls[i],dsHot,ForceUpdate);

							if HitTestResult.HitTestArea=htCloseButton then begin
								if HasState(stsMouseDown) then TabControls[i].CloseButtonState:=dsDown
								else TabControls[i].CloseButtonState:=dsHot
							end else begin
								if FTabs[i].Active then TabControls[i].CloseButtonState:=dsActive
								else TabControls[i].CloseButtonState:=dsNotActive;
							end;
						end else begin
							if not Tabs[i].Active then SetControlDrawState(TabControls[i],dsNotActive,ForceUpdate);
...
begin
	...
	if (Tabs.Count>1)and(FScrollWidth>0) then Result:=Result+ScaledPixels(FOptions.Display.Tabs.TabOverlap);
...
begin
	if (FOptions.Scrolling.Enabled)and(Value<>FScrollOffset) then begin
		FScrollOffset:=Value;

		if (not IgnoreContraints)and(Tabs.Count>0)and(FScrollOffset>GetMaxScrollOffset) then
...
procedure TCustomChromeTabs.SetImagesSpinnerDownload(const Value:TCustomImageList);
begin
	FImagesSpinnerDownload:=Value;
end;

procedure TCustomChromeTabs.SetImagesSpinnerUpload(const Value:TCustomImageList);
begin
...
begin
 ...
 {	if FMouseDownHitTest.TabIndex<>-1 then begin
		if OperationID=0 then Tabs[FMouseDownHitTest.TabIndex].Pinned:=TRUE
		else

			if OperationID=1 then Tabs[FMouseDownHitTest.TabIndex].Pinned:=FALSE;

		if OperationID=2 then DeleteTab(FMouseDownHitTest.TabIndex);
	end;

	if OperationID=3 then DoOnButtonAddClick
	else if OperationID>=10 then Tabs.ActiveTab:=Tabs[OperationID-10];  }
...
begin
	try
		if FDragTabObject<>nil then begin
			NewTab:=nil;

			Screen.Cursor:=FDragTabObject.OriginalCursor;

			if FDragTabObject.DockControl<>nil then begin
				try
					if FDragTabObject.DockControl=FDragTabObject.SourceControl then TabDropOptions:=[tdMoveTab]
					else TabDropOptions:=[tdDeleteDraggedTab,tdCreateDroppedTab];

					FDragTabObject.DockControl.TabDragDrop(Self,MouseX,MouseY,FDragTabObject,Cancelled,TabDropOptions);

					if (Cancelled)or(FDragTabObject.DockControl.GetControl=FDragTabObject.SourceControl.GetControl) then begin
						// We've just moved the tab within this control
						if (Cancelled)or(FDragTabObject.DropTabIndex=-1)or(FDragTabObject.DragTab.Index=FDragTabObject.DropTabIndex)
						then begin
							...
							if tdMoveTab in TabDropOptions then begin
						...
						if tdCreateDroppedTab in TabDropOptions then NewTab:=FDragTabObject.DockControl.InsertDroppedTab;
					end
				finally
					FDragTabObject.DockControl.TabDragOver(Self,MouseX,MouseY,dsDragLeave,FDragTabObject,DummyAccept);

					if NewTab<>nil then DoOnTabDragDropped(FDragTabObject,NewTab);
				end;
			end else begin
		...
		if FDragTabObject<>nil then begin
			if DockControl<>nil then DockControl.DragCompleted(MouseX,MouseY,FDragTabObject,Cancelled);

			SourceControl.DragCompleted(MouseX,MouseY,FDragTabObject,Cancelled);
		end;
		...
		if (tdDeleteDraggedTab in TabDropOptions)and(ActiveTabIndex<>-1) then begin
...
procedure TCustomChromeTabs.DoOnTabDragEnd(MouseX,MouseY:integer;DragTabObject:IDragTabObject;Cancelled:Boolean);
begin
	if Assigned(FOnTabDragEnd) then FOnTabDragEnd(Self,MouseX,MouseY,DragTabObject,Cancelled);
end;
...
begin
	if FUpdateCount>0 then begin
		...
		if (FUpdateCount=0)and(FPausedUpdateCount>0) then begin
...
begin
	...
	if (HasState(stsDragging))and(FOptions.DragDrop.DragType<>dtNone) then begin
...
begin
	Result:=([csDestroying,csLoading]*ComponentState=[])and(FControlConstructed)and(not(csCreating in ControlState));
	(HandleAllocated);
end;
...
begin
	...
	begin
		if (ControlReady)and(TabChangeType=tcPropertyUpdated) then begin
			UpdateProperties;
		...
		if ATab<>nil then begin
			if ATab.TabControl=nil then begin
				...
				begin
					...
					if LastVisibleTabIndex<>-1 then
							NewTabLeft:=TabControls[LastVisibleTabIndex].ControlRect.Right-
							ScaledPixels(FOptions.Display.Tabs.TabOverlap)
					else NewTabLeft:=ScaledPixels(FOptions.Display.Tabs.OffsetLeft+FOptions.Display.TabContainer.PaddingLeft);

					SetMovementAnimation(FOptions.Animation.MovementAnimations.TabAdd);

					SetControlPosition(NewTabControl,Rect(NewTabLeft,ScaledPixels(FOptions.Display.Tabs.OffsetTop),
						NewTabLeft+ScaledPixels(FOptions.Animation.MinimumTabAnimationWidth),
						ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom)),FALSE);
				end;

				// Fire the added event here before we activate the tab
				if Assigned(FOnChange) then FOnChange(Self,ATab,TabChangeType);

				// If no tabs are active, activate this one
				if (FOptions.Behaviour.ActivateNewTab)and
					((ActiveTabIndex=-1)or((ControlReady)and(not HasState(stsAddingDroppedTab)))) then begin
					ATab.Active:=TRUE;
			...
			begin
				case TabChangeType of
					tcActivated:begin
							...
							if FOptions.Scrolling.Enabled then ScrollIntoView(ATab);
					...
					tcDeleted:begin
							...
							if (FOptions.Animation.GetMovementAnimationEaseType(FOptions.Animation.MovementAnimations.TabDelete)<>
								ttNone)and(ATab<>nil) then begin
								// If the tabs are compressed and this is the last tab, don't animate
								if (GetTabDisplayState=tdCompressed)and(ATab.Index=Tabs.Count-1) then begin
					...
					tcDeleting:begin
							...
							if FOptions.Behaviour.TabSmartDeleteResizing then begin
								if ATab.Index=Tabs.Count-1 then AddState(stsEndTabDeleted)
								else AddState(stsTabDeleted);
							end;
						end;

					tcControlState:begin
							if ATab<>nil then TabControls[ATab.Index].Invalidate;
						end;
				end;
...
begin
	if (FOptions.Scrolling.Enabled)and(FOptions.Scrolling.MouseWheelScroll)and(Visible)and(Msg.WheelDelta<>0) then begin
...
begin
	if Assigned(FOnActiveTabChanged) then FOnActiveTabChanged(Self,ATab);
end;

procedure TCustomChromeTabs.DoOnActiveTabChanging(AOldTab,ANewTab:TChromeTab;var Allow:Boolean);
begin
	if Assigned(FOnActiveTabChanging) then FOnActiveTabChanging(Self,AOldTab,ANewTab,Allow);
end;
...
var
	...
	HitTestResult:THitTestResult;
begin
	if FTabPopupMenu<>nil then FreeAndNil(FTabPopupMenu);
	...
	if FMouseDownHitTest.TabIndex<>-1 then begin
		...
		if not Tabs[FMouseDownHitTest.TabIndex].Pinned then AddMenuItem(StrPinThisTab,0)
		else AddMenuItem(StrUnpinThisTab,1);

		AddMenuItem('-',-1);
	end;

	for i:=0 to Tabs.Count-1 do AddMenuItem(GetTabDescription(i),i+10);

	HitTestResult:=HitTest(Point(FLastMouseX,FLastMouseY));

	if HitTestResult.TabIndex=-1 then begin
		ATab:=nil;
	end else begin
...
begin
	if Assigned(FOnTabPopupMenu) then FOnTabPopupMenu(Self,ATab,PopupMenu);
end;
...
begin
	if GetTabDisplayState=tdScrolling then ScrollTabs(FScrollDirection);
end;
...
begin
	...
	finally FreeAndNil(TabsOptions);
	end;
end;
...
var
	IncValue:integer;
begin
	if GetTabDisplayState=tdScrolling then begin
		FScrollDirection:=ScrollDirection;

		IncValue:=ScaledPixels(FOptions.Scrolling.ScrollStep);

		if BiDiMode in BidiRightToLeftTabModes then IncValue:=-IncValue;
		...
		if StartTimer then FScrollTimer.Enabled:=TRUE;
	end;
end;
...
function IsWindowCloaked(hwnd:hwnd):Boolean;
var
	isCloaked:BOOL;
const
	DWMWA_CLOAKED=14;
begin
	...
	if DwmGetWindowAttribute(hwnd,DWMWA_CLOAKED,@isCloaked,sizeof(isCloaked))<>S_OK then exit;

	Result:=isCloaked;
end;

function IsMyFormCovered(const AComponent:TWinControl):Boolean;
var
	MyRect:TRect;
	MyRgn,TempRgn:HRGN;
	RType:integer;
	hw:hwnd;
begin
	MyRect:=AComponent.BoundsRect; // screen coordinates
	...
	while (hw<>0)and(hw<>AComponent.handle)and(RType<>NULLREGION) do begin
...
function IsWindowVisibleOnScreen(AComponent:TWinControl):Boolean;
begin
	Result:=IsWindowVisible(AComponent.handle)and(not IsWindowCloaked(AComponent.handle))and
		(not IsMyFormCovered(AComponent));
end;
...
procedure TCustomChromeTabs.OnAnimateTimer(Sender:TObject);
	...
	begin
		if ButtonControl.AnimateMovement then AddState(stsAnimatingMovement);

		if ButtonControl.AnimateStyle then AddState(stsAnimatingStyle);
	end;
...
begin
	if not(csDestroying in ComponentState) then begin
		...
		for i:=pred(Tabs.Count) downto 0 do begin
			if (AnimateModified)and(TabControls[i].AnimateModified) then AddState(stsAnimatingStyle);

			if (AnimateSpinner)and(TabControls[i].AnimateSpinner) then AddState(stsAnimatingStyle);

			if TabControls[i].AnimateStyle then AddState(stsAnimatingStyle);

			if TabControls[i].AnimateMovement then begin
				AddState(stsAnimatingMovement);

				if (i=ActiveTabIndex)and(HasState(stsAnimatingNewTab)) then ScrollIntoView(Tabs[i]);
			end else begin
				if (i=ActiveTabIndex)and(HasState(stsAnimatingNewTab)) then begin
			...
			if Tabs[i].MarkedForDeletion then begin
				AddState(stsControlPositionsInvalidated);
				AddState(stsAnimatingMovement);

				if RectWidth(TabControls[i].ControlRect)<=ScaledPixels(FOptions.Animation.MinimumTabAnimationWidth) then begin
					DeleteTab(i);
		...
		if (HasState(stsAnimatingMovement))or(HasState(stsAnimatingStyle)) then begin
			if (HasState(stsDeletingUnPinnedTabs))and(not HasState(stsDragging)) then SetControlDrawStates;
		...
		{ TODO :
			...
			is not processed. }
		if (FDragTabObject<>nil)and(FDragTabObject.DockControl<>nil)and
			(FDragTabObject.SourceControl<>FDragTabObject.DockControl) then FDragTabObject.DockControl.FireScrollTimer;
	end;
end;
...
begin
	if Tabs.ActiveTab=nil then Result:=-1
	else Result:=Tabs.ActiveTab.Index;
end;
...
begin
	if BiDiMode in BidiLeftToRightTabModes then Result:=FScrollOffset
	else Result:=-FScrollOffset;
end;
...
begin
	if (Tabs.Count=0)or(not Tabs[0].Pinned) then Result:=-1
	else begin
		Result:=0;

		while (Result<FTabs.Count-1)and(FTabs[Result+1].Pinned) do Inc(Result);
	end;
end;
...
begin
	...
	else Result:=TChromeTabControl(Tabs[Index].TabControl);
end;

function TCustomChromeTabs.GetTabDescription(Index:integer):String;
begin
	if (Index>=0)and(Index<Tabs.Count) then begin
		Result:=Tabs[Index].Caption;

		if Result='' then Result:=StrNoCaption;
	end
	else Result:='';
end;
...
begin
	...
	if HitTestResult.TabIndex=-1 then Result:=nil
	else Result:=Tabs[HitTestResult.TabIndex];
end;
...
begin
	...
	if not PtInRect(ControlRect,Pt) then Result.HitTestArea:=htNoWhere
	else

		// Add button?
		if FAddButtonControl.ContainsPoint(Pt) then Result.HitTestArea:=htAddButton
		else begin
			// Scroll buttons?
			if (ScrollButtonLeftVisible)and((not IgnoreDisabledControls)or(FScrollButtonLeftControl.DrawState<>dsDisabled))and
				(FScrollButtonLeftControl.ContainsPoint(Pt)) then Result.HitTestArea:=htScrollButtonLeft
			else begin
				if (ScrollButtonRightVisible)and((not IgnoreDisabledControls)or(FScrollButtonRightControl.DrawState<>dsDisabled)
					)and(FScrollButtonRightControl.ContainsPoint(Pt)) then Result.HitTestArea:=htScrollButtonRight
				else begin
					// If we're not overlaying buttons, we need to make sure
					// we're in the tab container
					if (FOptions.Display.TabContainer.OverlayButtons)or(PtInRect(TabContainerRect,Pt)) then begin
						// Check the active tab first as it will always be in front
						if ActiveTabIndex<>-1 then begin
							HitTestArea:=TabControls[ActiveTabIndex].GetHitTestArea(Pt.X,Pt.Y);

							if HitTestArea<>htBackground then begin
						...
						for i:=0 to pred(Tabs.Count) do begin
							if (not Tabs[i].Visible)or(i=ActiveTabIndex) then Continue;
...
begin
	...
	if not HasState(stsMouseDown) then begin
		FMouseDownHitTest:=HitTest(Point(X,Y));
		FMouseButton:=Button;
		FMouseDownPoint:=Point(X,Y);

		if (FOptions.Behaviour.BackgroundDragMovesForm)and(FMouseDownHitTest.HitTestArea=htBackground)and
			(GetParentForm(Self)<>nil) then begin
			ReleaseCapture;
		...
		begin
			...
			if FMouseDownHitTest.HitTestArea=htScrollButtonLeft then begin
				ScrollTabs(mdsLeft);
			end else

				if FMouseDownHitTest.HitTestArea=htScrollButtonRight then begin
				ScrollTabs(mdsRight);
			end else
			...
			begin
				...
				try
					...
					if (Button=mbLeft)or((Button=mbRight)and(FOptions.Behaviour.TabRightClickSelect)) then begin
						// Are activating a tab?
						if ((FMouseDownHitTest.HitTestArea=htTab)or((FMouseDownHitTest.HitTestArea=htCloseButton)and(Button=mbRight)
							)or(FMouseDownHitTest.HitTestArea=htTabImage))and(FMouseDownHitTest.TabIndex<>ActiveTabIndex) then begin
							Tabs[FMouseDownHitTest.TabIndex].Active:=TRUE;
						end;
					end else if (Button=
{$IFDEF EXPLICIT_DRAW_STATE}TMouseButton.{$ENDIF}mbMiddle)and(FMouseDownHitTest.HitTestArea=htTab) then begin
						HitTestResult:=HitTest(Point(X,Y));
						AllowClose:=TRUE;
						if Assigned(FOnButtonBeforCloseTabClick) then
								FOnButtonBeforCloseTabClick(Self,Tabs[FMouseDownHitTest.TabIndex],AllowClose);
						if (AllowClose)and(not Tabs[HitTestResult.TabIndex].Pinned)and(FOptions.Behaviour.CloseOnWheel) then begin
							...
							AddState(stsControlPositionsInvalidated);
							if Assigned(FOnButtonAfterCloseTabClick) then
									FOnButtonAfterCloseTabClick(Self,Tabs[FMouseDownHitTest.TabIndex],Tabs.Count);
						end;
					end;
				finally EndUpdate;
				end;
...
begin
	...
	if HasState(stsDragging) then inherited
	else begin
		...
		if HintText='' then inherited
		else begin
			if HitTestResult.TabIndex<>-1 then HintRect:=TabControls[HitTestResult.TabIndex].ControlRect
			else HintRect:=ControlRect;
...
begin
	if BiDiMode in BidiRightToLeftTabModes then Result:=HorzFlipRect(ControlRect,ARect)
	else Result:=ARect;
end;

function TCustomChromeTabs.BidiXPos(X:integer):integer;
begin
	if BiDiMode in BidiRightToLeftTabModes then Result:=ClientWidth-X
	else Result:=X;
end;
...
begin
	...
	if (FLastMouseX<>X)or(FLastMouseY<>Y) then begin
		FLastMouseX:=X;
		FLastMouseY:=Y;
		...
		try
			...
			if (not(HasState(stsDragging)))and(FOptions.DragDrop.DragType in [dtWithinContainer,dtBetweenContainers])and
				(HasState(stsMouseDown))and(FMouseDownHitTest.HitTestArea=htTab)and(FMouseButton=mbLeft)and
				(HitTestResult.HitTestArea=htTab)and((FMouseDownPoint.X>=X+ScaledPixels(FOptions.DragDrop.DragStartPixels))or
				...
				(FMouseDownPoint.Y<=Y-ScaledPixels(FOptions.DragDrop.DragStartPixels))) then begin
				AllowDrag:=TRUE;

				DoOnBeginTabDrag(FTabs[HitTestResult.TabIndex],AllowDrag);

				if AllowDrag then BeginDrag(FMouseDownPoint.X,FMouseDownPoint.Y);
			end;

			if FDragTabObject=nil then SetControlDrawStates;

			// Are we dragging?
			if (FDragTabObject<>nil)and(FDragTabObject.SourceControl.GetControl=Self) then begin
				FDragTabObject.DragOverType:=dotNone;

				if FOptions.DragDrop.DragType=dtWithinContainer then begin
					FDragTabObject.DragOverType:=dotHomeContainer;
					...
					CursorWinControl:=FindChromeTabsControlWithinRange(ScreenPoint,
						ScaledPixels(FOptions.DragDrop.DragOutsideDistancePixels));

					Accept:=FALSE;

					if CursorWinControl=nil then DoOnDebugLog('No ChromeTabs found at X: %d, Y: %d',[ScreenPoint.X,ScreenPoint.Y])
					else DoOnDebugLog('ChromeTabs (%s) found at X: %d, Y: %d',[CursorWinControl.Name,ScreenPoint.X,
							ScreenPoint.Y]);

					// Does it support IChromeTabDockControl?
					if Supports(CursorWinControl,IChromeTabDockControl,TabDockControl) then begin
						Accept:=TRUE;

						ControlPoint:=TabDockControl.GetControl.ScreenToClient(ScreenPoint);

						// Tell the dock control that we are dragging over it
						if (FDragTabObject.DockControl=nil)or(FDragTabObject.DockControl.GetControl<>Self) then
						...
						if Accept then begin
							...
							if (PreviousDockControl<>nil)and(PreviousDockControl<>TabDockControl) then
									PreviousDockControl.TabDragOver(Self,ControlPoint.X,ControlPoint.Y,dsDragLeave,FDragTabObject,
									DummyAccept);

							// Set the current dock control
							FDragTabObject.DockControl:=TabDockControl;

							if FDragTabObject.DockControl=nil then FDragTabObject.DragOverType:=dotNone
							else if FDragTabObject.DockControl.GetControl=Self then FDragTabObject.DragOverType:=dotHomeContainer
							else FDragTabObject.DragOverType:=dotRemoteContainer;
							...
							if (FDragTabObject.DockControl<>nil)and
								(FDragTabObject.DockControl.GetControl=FDragTabObject.SourceControl.GetControl) then Redraw;
						end;
					end else begin
						...
						if FDragTabObject.DockControl<>nil then begin
							...
							FDragTabObject.DockControl.TabDragOver(Self,ControlPoint.X,ControlPoint.Y,dsDragLeave,FDragTabObject,
								DummyAccept);
						end;
				...
				if (FOptions.Display.TabMouseGlow.Visible)and(HitTestResult.HitTestArea in [htTab,htCloseButton,htTabImage])
				then Redraw;
			end;
		finally EndUpdate;
		end;
...
begin
	...
	if FOptions.DragDrop.DragDisplay in [ddTab,ddControl,ddTabAndControl] then begin
		ActualDragDisplay:=FOptions.DragDrop.DragDisplay;

		DoOnCreateDragForm(ATab,Result);

		if Result=nil then begin
			if ActualDragDisplay in [ddControl,ddTabAndControl] then begin
				DoOnNeedDragImageControl(ATab,DragControl);

				if DragControl=nil then ActualDragDisplay:=ddTab;
			end;

			if BiDiMode in BidiRightToLeftTabModes then BiDiX:=ControlRect.Right+FDragTabObject.DragCursorOffset.X
			else BiDiX:=FDragTabObject.DragCursorOffset.X;

			if ActualDragDisplay=ddTab then FDragTabObject.DragFormOffset:=Point(BiDiX,FDragTabObject.DragCursorOffset.Y)
			else FDragTabObject.DragFormOffset:=Point(Round(BiDiX*FOptions.DragDrop.DragControlImageResizeFactor),
			...
			try
						...
						case FOptions.Display.Tabs.Orientation of
							...
							toBottom:begin
									...
									FDragTabObject.DragFormOffset:=Point(FDragTabObject.DragFormOffset.X,
										FDragTabObject.DragFormOffset.Y+
										Round(DragControl.Height*FOptions.DragDrop.DragControlImageResizeFactor));
								end;
						end;
					end;
				end;
				...
				if ActualDragDisplay in [ddControl,ddTabAndControl] then CopyControlToBitmap(DragControl,Bitmap,0,ControlTop);

				DragCanvas:=TGPGraphics.Create(Bitmap.Canvas.handle);
				try
					if ActualDragDisplay in [ddTab,ddTabAndControl] then begin
						...
						case FOptions.Display.Tabs.Orientation of
							...
							toBottom:Bitmap.Canvas.FillRect(Rect(0,Bitmap.Height,Bitmap.Width,
									Bitmap.Height-RectHeight(TabControls[ATab.Index].ControlRect)));
						end;
						...
						try
							if (ActualDragDisplay in [ddControl,ddTabAndControl])and
								(RectWidth(TabControls[ATab.Index].ControlRect)>DragControl.Width-10) then
									TabControls[ATab.Index].SetWidth(DragControl.Width-10,0,ttNone);

							SetControlPosition(TabControls[ATab.Index],
								Rect(0,TabTop,TabControls[ATab.Index].ControlRect.Right-TabControls[ATab.Index].ControlRect.Left,
								TabTop+RectHeight(TabControls[ATab.Index].ControlRect)),FALSE);

							TabControls[ATab.Index].DrawTo(DragCanvas,FLastMouseX,FLastMouseY);
						finally
							TabControls[ATab.Index].ScrollableControl:=TRUE;

							SetControlPosition(TabControls[ATab.Index],TempRect,FALSE);
						end;
					end;

					if ActualDragDisplay in [ddControl,ddTabAndControl] then begin
						BorderPen:=TGPPen.Create(MakeGDIPColor(FOptions.DragDrop.DragFormBorderColor,255),
							ScaledPixels(FOptions.DragDrop.DragFormBorderWidth));
						try
							...
							case FOptions.Display.Tabs.Orientation of
								toTop:begin
										...
										DragCanvas.DrawLine(BorderPen,Bitmap.Width-BorderOffset,ControlTop,Bitmap.Width-BorderOffset,
											Bitmap.Height-BorderOffset);
										DragCanvas.DrawLine(BorderPen,Bitmap.Width-BorderOffset,Bitmap.Height-BorderOffset,0,
											Bitmap.Height-BorderOffset);
										DragCanvas.DrawLine(BorderPen,0,Bitmap.Height-BorderOffset,0,ControlTop);
									end;

								toBottom:begin { TODO : Needs fixing for bottom tabs }
										DragCanvas.DrawLine(BorderPen,TabEndX,ControlTop,Bitmap.Width-BorderOffset,ControlTop);
										DragCanvas.DrawLine(BorderPen,Bitmap.Width-BorderOffset,ControlTop,Bitmap.Width-BorderOffset,
											Bitmap.Height-BorderOffset);
										DragCanvas.DrawLine(BorderPen,Bitmap.Width-BorderOffset,Bitmap.Height-BorderOffset,0,
						...
						finally FreeAndNil(BorderPen);
						end;
					end;
				finally FreeAndNil(DragCanvas);
				end;

				ScaledBitmap:= {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap.Create;
				try
					// Scale the image
					if ActualDragDisplay=ddTab then begin
				...
				finally FreeAndNil(ScaledBitmap);
				end;
...
begin
	if Assigned(FOnTabImageClick) then FOnTabImageClick(Self,ATab);
end;
...
begin
	...
	if Assigned(FOnButtonAddClick) then FOnButtonAddClick(Self,Handled);

	if not Handled then Tabs.Add;
end;

procedure TCustomChromeTabs.DoOnCreateDragForm(ATab:TChromeTab;var DragForm:TForm);
begin
	if Assigned(FOnCreateDragForm) then FOnCreateDragForm(Self,ATab,DragForm);
end;

procedure TCustomChromeTabs.DoOnDebugLog(const Text:String;Args: Array of const);
begin
	if (FOptions.Behaviour.DebugMode)and(Assigned(FOnDebugLog)) then FOnDebugLog(Self,format(Text,Args));
end;

procedure TCustomChromeTabs.DoOnGetControlPolygons(ChromeTabsControl:TObject;ItemRect:TRect;ItemType:TChromeTabItemType;
...
procedure TCustomChromeTabs.DoOnNeedDragImageControl(ATab:TChromeTab;var DragControl:TWinControl);
begin
	if Assigned(FOnNeedDragImageControl) then FOnNeedDragImageControl(Self,ATab,DragControl);
end;
...
procedure TCustomChromeTabs.DoOnShowHint(HitTestResult:THitTestResult;var HintText:String;var HintTimeout:integer);
begin
	if Assigned(FOnShowHint) then FOnShowHint(Self,HitTestResult,HintText,HintTimeout);
end;

procedure TCustomChromeTabs.DoOnStateChange(PreviousState,CurrentState:TChromeTabStates);
begin
	if Assigned(FOnStateChange) then FOnStateChange(Self,PreviousState,CurrentState);
end;

procedure TCustomChromeTabs.DoOnScrollWidthChanged;
begin
	if Assigned(FOnScrollWidthChanged) then FOnScrollWidthChanged(Self);
end;

procedure TCustomChromeTabs.DoOnSetTabWidth(ATabControl:TChromeTabControl;var TabWidth:integer);
begin
	if Assigned(FOnSetTabWidth) then FOnSetTabWidth(Self,ATabControl,TabWidth);
end;

procedure TCustomChromeTabs.DoOnTabDblClick(ATab:TChromeTab);
begin
	if Assigned(FOnTabDblClick) then FOnTabDblClick(Self,ATab);
end;

procedure TCustomChromeTabs.DoOnTabClick(ATab:TChromeTab);
begin
	if Assigned(FOnTabClick) then FOnTabClick(Self,ATab);
end;

procedure TCustomChromeTabs.DoOnTabDragDrop(X,Y:integer;DragTabObject:IDragTabObject;Cancelled:Boolean;
	var TabDropOptions:TTabDropOptions);
begin
	if Assigned(FOnTabDragDrop) then FOnTabDragDrop(Self,X,Y,DragTabObject,Cancelled,TabDropOptions);
end;

procedure TCustomChromeTabs.DoOnTabDragDropped(DragTabObject:IDragTabObject;NewTab:TChromeTab);
begin
	if Assigned(FOnTabDragDropped) then FOnTabDragDropped(Self,DragTabObject,NewTab);
end;

procedure TCustomChromeTabs.DoOnTabDragOver(X,Y:integer;State:TDragState;DragTabObject:IDragTabObject;
	var Accept:Boolean);
begin
	if Assigned(FOnTabDragOver) then FOnTabDragOver(Self,X,Y,State,DragTabObject,Accept);
end;

procedure TCustomChromeTabs.DoOnTabDragStart(ATab:TChromeTab;var Allow:Boolean);
begin
	if Assigned(FOnTabDragStart) then FOnTabDragStart(Self,ATab,Allow);
end;
...
var
	...
	HitTestResult:THitTestResult;
begin
	if (Button<>mbRight)or(FDragTabObject=nil) then inherited;
	...
	try
		...
		if HasState(stsDragging) then begin
			EndDrag(X,Y,FDragCancelled);
		end;

		if (FMouseButton=mbLeft)and(HitTestResult.HitTestArea=htAddButton)and(FMouseDownHitTest.HitTestArea=htAddButton)
		then begin
			DoOnButtonAddClick;
			...
			{ if (HitTestResult.HitTestArea=htTab)and(FMouseDownHitTest.HitTestArea=htTab)and
				(HitTestResult.TabIndex=FMouseDownHitTest.TabIndex) then begin
				DoOnTabClick(Tabs[HitTestResult.TabIndex]);

				AddState(stsControlPositionsInvalidated);
				end
				else }
			// Have we clicked the TabImage?
			if (FMouseButton=mbLeft)and(HitTestResult.HitTestArea=htTabImage)and(FMouseDownHitTest.HitTestArea=htTabImage)and
				(HitTestResult.TabIndex=FMouseDownHitTest.TabIndex) then begin
				DoOnTabImageClick(Tabs[HitTestResult.TabIndex]);
			end
			else
				// Have we clicked the Close button?
				if (FMouseButton=mbLeft)and(HitTestResult.HitTestArea=htCloseButton)and
					(FMouseDownHitTest.HitTestArea=htCloseButton)and(HitTestResult.TabIndex=FMouseDownHitTest.TabIndex) then begin
					AllowClose:=TRUE;

					DoOnButtonBeforCloseTabClick(Tabs[HitTestResult.TabIndex],AllowClose);
					...
					if AllowClose then begin
						...
						if not Tabs[HitTestResult.TabIndex].Pinned then begin
							// Reset the tab closing state
							ClearTabClosingStates;
						...
						AddState(stsControlPositionsInvalidated);
						DoOnButtonAfterCloseTabClick(Tabs[HitTestResult.TabIndex],Tabs.Count);
					end;
...
begin
	if Assigned(FOnAfterDragImageCreated) then FOnAfterDragImageCreated(Self);
end;

procedure TCustomChromeTabs.DoOnAfterDrawItem(const TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;
	TabIndex:integer);
begin
	if Assigned(FOnAfterDrawItem) then FOnAfterDrawItem(Self,TargetCanvas,ItemRect,ItemType,TabIndex);
end;

procedure TCustomChromeTabs.DoOnBeforeDrawItem(TargetCanvas:TGPGraphics;ItemRect:TRect;ItemType:TChromeTabItemType;
	TabIndex:integer;var Handled:Boolean);
begin
	Handled:=FALSE;

	if Assigned(FOnBeforeDrawItem) then FOnBeforeDrawItem(Self,TargetCanvas,ItemRect,ItemType,TabIndex,Handled);
end;

procedure TCustomChromeTabs.DoOnBeginTabDrag(ATab:TChromeTab;var Allow:Boolean);
begin
	if Assigned(FOnBeginTabDrag) then FOnBeginTabDrag(Self,ATab,Allow);
end;

procedure TCustomChromeTabs.DoOnButtonBeforCloseTabClick(ATab:TChromeTab;var AllowClose:Boolean);
begin
	if Assigned(FOnButtonBeforCloseTabClick) then FOnButtonBeforCloseTabClick(Self,ATab,AllowClose);
end;

procedure TCustomChromeTabs.DoOnButtonAfterCloseTabClick(ATab:TChromeTab;counts:integer);
begin
	if Assigned(FOnButtonAfterCloseTabClick) then FOnButtonAfterCloseTabClick(Self,ATab,Tabs.Count);
end;

procedure TCustomChromeTabs.Notification(AComponent:TComponent;Operation:TOperation);
begin
	...
	if (Operation=opRemove) then begin
		if AComponent=FImages then SetImages(nil)
			...
			if AComponent=FImagesOverlay then SetImagesOverlay(nil);

		if AComponent=FImagesSpinnerUpload then SetImagesSpinnerUpload(nil);

		if AComponent=FImagesSpinnerDownload then SetImagesSpinnerDownload(nil);
	end;
end;
...
begin
	if (not FOptions.Behaviour.IgnoreDoubleClicksWhileAnimatingMovement)or(not HasState(stsAnimatingMovement)) then begin
		HitTestResult:=HitTest(Point(Message.XPos,Message.YPos));

		if HitTestResult.TabIndex<>-1 then DoOnTabDblClick(FTabs[HitTestResult.TabIndex]);

		if FOptions.Behaviour.BackgroundDblClickMaximiseRestoreForm then begin
			if HitTestResult.HitTestArea=htBackground then begin
				ParentForm:=GetParentForm(Self);

				if ParentForm<>nil then begin
					if ParentForm.WindowState=wsNormal then ParentForm.WindowState:=wsMaximized
					else ParentForm.WindowState:=wsNormal;
				end;
			end;
		end;
	end;
...
procedure TCustomChromeTabs.WMLButtonClk(var Message:TWMLButtonUp);
var
	HitTestResult:THitTestResult;
	ParentForm:TCustomForm;
begin
	...
	if HitTestResult.TabIndex<>-1 then DoOnTabClick(FTabs[HitTestResult.TabIndex]);
...
begin
	...
	finally FreeAndNil(TabsLookAndFeel);
	end;
end;
...
{$IFDEF UNIT_SCOPE_NAMES}System.{$ENDIF}SysUtils.DeleteFile(Filename);
	...
	try SaveLookAndFeel(FileStream);
	finally FreeAndNil(FileStream);
	end;
end;
...
begin
	Result:=((ScrollingActive)or(not FOptions.Scrolling.AutoHideButtons))and(FOptions.Scrolling.Enabled);
end;
...
begin
	...
	if (ScrollButtonLeftVisible)and(FOptions.Scrolling.ScrollButtons in [csbLeft,csbLeftAndRight]) then begin
		SetControlPosition(FScrollButtonLeftControl,
			...
			LeftOffset+ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Horizontal+
			FOptions.Display.ScrollButtonLeft.Width),ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Vertical+
			FOptions.Display.ScrollButtonLeft.Height)),FALSE);

		Inc(LeftOffset,RectWidth(FScrollButtonLeftControl.ControlRect)+
			ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Horizontal+1));
	end;

	// If the scroll Right button is visible on the left, calculate it's rect
	if (ScrollButtonRightVisible)and(FOptions.Scrolling.ScrollButtons in [csbLeft]) then begin
		SetControlPosition(FScrollButtonRightControl,
			...
			LeftOffset+ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Horizontal+
			FOptions.Display.ScrollButtonRight.Width),ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Vertical+
			FOptions.Display.ScrollButtonRight.Height)),FALSE);

		Inc(LeftOffset,RectWidth(FScrollButtonRightControl.ControlRect)+
			ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Horizontal+1));
	end;

	if FOptions.Display.AddButton.Visibility in [avLeft] then begin
		SetControlPosition(FAddButtonControl,Rect(LeftOffset+ScaledPixels(FOptions.Display.AddButton.Offsets.Horizontal),
			...
			ScaledPixels(FOptions.Display.AddButton.Height+FOptions.Display.AddButton.Offsets.Vertical)),FALSE);

		Inc(LeftOffset,ScaledPixels(FOptions.Display.AddButton.Width+FOptions.Display.AddButton.Offsets.Horizontal+1));
	end;

	// Calculate the right offset
	if (ScrollButtonRightVisible)and(FOptions.Scrolling.ScrollButtons in [csbRight,csbLeftAndRight]) then begin
		SetControlPosition(FScrollButtonRightControl,Rect(RightOffset-ScaledPixels(FOptions.Display.ScrollButtonRight.Width)
			-ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Horizontal),
			ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Vertical),
			RightOffset-ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Horizontal),
			ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Vertical+
			FOptions.Display.ScrollButtonRight.Height)),FALSE);

		Dec(RightOffset,RectWidth(FScrollButtonRightControl.ControlRect)+
			ScaledPixels(FOptions.Display.ScrollButtonRight.Offsets.Horizontal+1));
	end;

	if (ScrollButtonLeftVisible)and(FOptions.Scrolling.ScrollButtons in [csbRight]) then begin
		SetControlPosition(FScrollButtonLeftControl,Rect(RightOffset-ScaledPixels(FOptions.Display.ScrollButtonLeft.Width)-
			ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Horizontal),
			ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Vertical),
			RightOffset-ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Horizontal),
			ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Vertical+FOptions.Display.ScrollButtonLeft.Height)),FALSE);

		Dec(RightOffset,RectWidth(FScrollButtonLeftControl.ControlRect)+
			ScaledPixels(FOptions.Display.ScrollButtonLeft.Offsets.Horizontal+1));
	end;

	if (FOptions.Display.AddButton.Visibility=avRightFixed)or(FOptions.Display.AddButton.Visibility=avRightFloating) then
	begin
		FMaxAddButtonRight:=RightOffset-ScaledPixels(FOptions.Display.AddButton.Width)-
			ScaledPixels(FOptions.Display.AddButton.Offsets.Horizontal);

		if FOptions.Display.AddButton.Visibility<>avRightFloating then
				SetControlPosition(FAddButtonControl,Rect(FMaxAddButtonRight,
				ScaledPixels(FOptions.Display.AddButton.Offsets.Vertical),
	...
	if (FOptions.Behaviour.TabSmartDeleteResizing)and(HasState(stsDeletingUnPinnedTabs))and(HasState(stsEndTabDeleted))
	then begin
		...
		FTabContainerRect:=Rect(LeftOffset+ScaledPixels(FOptions.Display.Tabs.OffsetLeft),
			ScaledPixels(FOptions.Display.Tabs.OffsetTop),FClosedTabRect.Right-ScrollDelta,
			ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom));

		if ScrollDelta>0 then RemoveState(stsEndTabDeleted);
	end else begin
		FTabContainerRect:=Rect(LeftOffset+ScaledPixels(FOptions.Display.Tabs.OffsetLeft),
			ScaledPixels(FOptions.Display.Tabs.OffsetTop),RightOffset-ScaledPixels(FOptions.Display.Tabs.OffsetRight),
			ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom));
	end;
end;
...
begin
	Result:=((BidiMode1 in BidiRightToLeftTabModes)and(BiDiMode2 in BidiRightToLeftTabModes))or
		((BidiMode1 in BidiLeftToRightTabModes)and(BiDiMode2 in BidiLeftToRightTabModes));
end;

procedure TCustomChromeTabs.SetControlPosition(ChromeTabsControl:TBaseChromeTabsControl;ControlRect:TRect;
	Animate:Boolean);
var
	EaseType:TChromeTabsEaseType;
	AnimationTime:Cardinal;
begin
	if (Animate)and(not(csDesigning in ComponentState)) then begin
	...
	if not(csDesigning in ComponentState) then DoOnAnimateMovement(ChromeTabsControl,AnimationTime,EaseType);

	ChromeTabsControl.SetPosition(ControlRect,AnimationTime,EaseType);
end;

procedure TCustomChromeTabs.SetControlLeft(ChromeTabsControl:TBaseChromeTabsControl;ALeft:integer;Animate:Boolean);
begin
	SetControlPosition(ChromeTabsControl,Rect(ALeft,ChromeTabsControl.ControlRect.Top,
		RectWidth(ChromeTabsControl.ControlRect)+ALeft,ChromeTabsControl.ControlRect.Bottom),Animate);
end;

procedure TCustomChromeTabs.SetMovementAnimation(MovementAnimationTypes:TChromeTabsMovementAnimationTypes);
begin
	if MovementAnimationTypes=nil then begin
...
begin
	...
	if Result>ScaledPixels(FOptions.Display.Tabs.MaxWidth) then Result:=ScaledPixels(FOptions.Display.Tabs.MaxWidth)
	else if Result<ScaledPixels(FOptions.Display.Tabs.MinWidth) then Result:=ScaledPixels(FOptions.Display.Tabs.MinWidth);
...
procedure TCustomChromeTabs.CalculateTabRects;
	...
	begin
		...
		if (FDragTabControl<>nil)and(FActiveDragTabObject.SourceControl.GetControl<>Self)and
			(FActiveDragTabObject.DragTab.Pinned) then VisiblePinnedTabCount:=VisiblePinnedTabCount+1;

		VisibleTabCount:=GetVisibleNonPinnedTabCount;

		if (FDragTabControl<>nil)and(FActiveDragTabObject.SourceControl.GetControl<>Self)and
			(not FActiveDragTabObject.DragTab.Pinned) then begin
			VisibleTabCount:=VisibleTabCount+1;
		end;

		if FOptions.Display.Tabs.ShowPinnedTabText then begin
		...
		if FDragTabControl<>nil then TabClientWidth:=TabClientWidth+ScaledPixels(FOptions.Display.Tabs.TabOverlap);

		TabWidth:=0;

		if FOptions.Display.Tabs.ShowPinnedTabText then begin
			if GetVisibleTabCount+GetVisiblePinnedTabCount>0 then begin
		...
		end else if VisibleTabCount>0 then begin
			TabWidth:=TabClientWidth div VisibleTabCount;
			TabEndSpace:=TabClientWidth mod VisibleTabCount;
		end;

		if TabWidth<ScaledPixels(FOptions.Display.Tabs.MinWidth) then begin
			TabWidth:=ScaledPixels(FOptions.Display.Tabs.MinWidth);
			TabEndSpace:=0;
		end;

		if TabWidth>ScaledPixels(FOptions.Display.Tabs.MaxWidth) then begin
	...
	begin
		for i:=0 to pred(Tabs.Count) do begin
			if (not FTabs[i].Visible)or(i<FClosedTabIndex) then Continue;
			...
			if (FOptions.Animation.GetMovementAnimationEaseType(FOptions.Animation.MovementAnimations.TabDelete)<>ttNone)and
				(HasState(stsAnimatingCloseTab)) then Inc(RealClosedTabIndex);

			if i=RealClosedTabIndex then begin
			...
			else TabWidth:=RectWidth(TabControls[i].ControlRect);

			if not Tabs[i].MarkedForDeletion then begin
				// Set the position of the tabs
				SetControlPosition(TabControl,Rect(TabLeft,ScaledPixels(FOptions.Display.Tabs.OffsetTop),TabLeft+TabWidth,
					ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom)),TRUE);
			end;
	...
	begin
		...
		if (FActiveDragTabObject<>nil)and(FActiveDragTabObject.DragTab.Pinned=PinnedTabs) then begin
			if FOptions.Display.Tabs.TabWidthFromContent then begin
		...
		if DragTabControl<>nil then begin
			...
			if FOptions.DragDrop.ContrainDraggedTabWithinContainer then begin
				MaxRight:=TabContainerRect.Right;

				if FOptions.Display.AddButton.Visibility=avRightFloating then
						MaxRight:=MaxRight+ScaledPixels(FOptions.Display.AddButton.Width+
						FOptions.Display.AddButton.Offsets.Horizontal);

				if DragTabX+RectWidth(DragTabControl.ControlRect)>MaxRight then
						DragTabX:=MaxRight-RectWidth(DragTabControl.ControlRect)
				else if DragTabX<ScaledPixels(FOptions.Display.Tabs.OffsetLeft+FOptions.Display.TabContainer.PaddingLeft) then
						DragTabX:=ScaledPixels(FOptions.Display.Tabs.OffsetLeft+FOptions.Display.TabContainer.PaddingLeft);
			end;

			SetControlPosition(DragTabControl,Rect(DragTabX,ControlRect.Top+ScaledPixels(FOptions.Display.Tabs.OffsetTop),
				DragTabX+DragTabWidth,ControlRect.Bottom-ScaledPixels(FOptions.Display.Tabs.OffsetBottom)),FALSE);
		end;

		// Step through the tabs
		for i:=StartTabIndex to EndTabIndex do begin
			// Skip this tab if it being dragged with it's own container
			if (not FTabs[i].Visible)or
				((FDragTabObject<>nil)and((FDragTabObject.DockControl=nil)or(FDragTabObject.DockControl.GetControl<>Self))and
				(i=ActiveTabIndex)) then Continue;
			...
			if (DragTabControl<>nil)and(DragTabControl.ControlRect.Left+ScrollOffset+
				(RectWidth(DragTabControl.ControlRect)div 2)+(ScaledPixels(FOptions.Display.Tabs.TabOverlap)div 2)<TabLeft+
				RectWidth(DragTabControl.ControlRect)) then begin
				// If the Dock control is this tab set, insert the drag tab into the control
				if (ActiveDragTabObject.DockControl<>nil)and(ActiveDragTabObject.DockControl.GetControl=Self) then begin
					...
					FScrollWidth:=FScrollWidth+RectWidth(DragTabControl.ControlRect)-
						ScaledPixels(FOptions.Display.Tabs.TabOverlap);
				end;

				// Do we need to hide the New Button?
				if (not PinnedTabs)and(FActiveDragTabObject.DragTab.Index=pred(Tabs.Count)) then
			...
			if (not HasState(stsDragging))or((FDragTabObject<>nil)and(i<>FDragTabObject.DragTab.Index)) then begin
				// We add a sigle pixel to some of the tabs in order to align the
				// right hand side perfectly
				if ((not PinnedTabs)or(FOptions.Display.Tabs.ShowPinnedTabText))and
					(not FOptions.Display.Tabs.TabWidthFromContent)and(not Tabs[i].MarkedForDeletion)and(TabEndSpace>0) then
						TabWidthAddition:=1
				else TabWidthAddition:=0;
				...
				if not Tabs[i].MarkedForDeletion then begin
					...
					if (HasState(stsResizing))or(ExtraTabWidth<>0) then SetMovementAnimation(nil);

					if (FOptions.Display.Tabs.TabWidthFromContent)and((not PinnedTabs)or(FOptions.Display.Tabs.ShowPinnedTabText))
					then TabWidth:=GetTabWidthByContent(TabControl);
					...
					SetControlPosition(TabControl,Rect(TabLeft-ExtraTabWidth,ScaledPixels(FOptions.Display.Tabs.OffsetTop),
						TabRight,ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom)),TRUE);
		...
		if DragTabControl<>nil then begin
			if not DraggingInOwnContainer then EndTabIndex:=EndTabIndex+1;

			// If we're dragging a pinned tab, we need to leave a space at the end of the pinned tabs
			if (PinnedTabs)and(ActiveDragTabObject.DragTab.Pinned) then begin
				TabLeft:=TabLeft+RectWidth(DragTabControl.ControlRect)-ScaledPixels(FOptions.Display.Tabs.TabOverlap);
			...
			if Tabs.Count>0 then FScrollWidth:=FScrollWidth+RectWidth(TabControls[Tabs.Count-1].ControlRect);
		...
		if FActiveDragTabObject<>nil then begin
			if FActiveDragTabObject.DropTabIndex<0 then FActiveDragTabObject.DropTabIndex:=0;
		end;
	end;
...
begin
	try
		...
		if (HasState(stsDeletingUnPinnedTabs))and(HasState(stsTabDeleted)) then begin
			SetTabPositionsClosing(TabLeft);
		end else begin
			if (not DraggingInOwnContainer)and(not FOptions.Display.Tabs.TabWidthFromContent) then
		...
		if FOptions.Display.AddButton.Visibility=avRightFloating then begin
			if GetVisibleTabCount(TRUE)=0 then AddButtonLeft:=ScaledPixels(FOptions.Display.TabContainer.PaddingLeft)
			else begin
				...
				else AddButtonLeft:=TabControls[GetLastVisibleTabIndex(pred(FTabs.Count))].ControlRect.Right+
						ScaledPixels(FOptions.Display.AddButton.HorizontalOffsetFloating);
			end;

			if ((GetTabDisplayState<>tdNormal)and(not HasState(stsEndTabDeleted)))or
				(GetTabDisplayState in [tdCompressed,tdScrolling])or(AddButtonLeft>FMaxAddButtonRight) then
					AddButtonLeft:=FMaxAddButtonRight;

			SetControlPosition(FAddButtonControl,Rect(AddButtonLeft,ScaledPixels(FOptions.Display.AddButton.Offsets.Vertical),
				AddButtonLeft+ScaledPixels(FOptions.Display.AddButton.Width),
				ScaledPixels(FOptions.Display.AddButton.Height+FOptions.Display.AddButton.Offsets.Vertical)),FALSE);
		end;

		if FScrollWidth<>FPreviousScrollWidth then DoOnScrollWidthChanged;

		FPreviousScrollWidth:=FScrollWidth;

		// Calculate the position of the next control
		if Tabs.Count=0 then TabLeft:=0
		else TabLeft:=TabControls[Tabs.Count-1].ControlRect.Right;

		TabLeft:=FScrollWidth+ScaledPixels(FOptions.Display.Tabs.TabOverlap+FOptions.Display.AddButton.Offsets.Horizontal+
			FOptions.Display.Tabs.OffsetLeft);
	finally
		// Reset the drag status now we're drawn everything
		if (HasState(stsCancellingDrag))or(HasState(stsCompletingDrag)) then begin
...
begin
	if (FOptions.Scrolling.Enabled)and(ScrollOffset>0)and((AlwaysCorrect)or(FLastClientWidth<CorrectedClientWidth)) then
	begin
		// Make sure we're showing as many tabs as possible
		LastTabIndex:=GetLastVisibleTabIndex(Tabs.Count-1);

		if LastTabIndex<>-1 then begin
			if TabControls[LastTabIndex].ControlRect.Right<ScrollOffset+ClientWidth then ScrollOffset:=GetMaxScrollOffset;
		end;
	end;
...
var
	i:integer;
begin
	if (ControlReady)and(not FInvalidatingControls) then begin
		FInvalidatingControls:=TRUE;
		try
			for i:=0 to pred(Tabs.Count) do SetControlDrawState(TabControls[i],TabControls[i].DrawState,TRUE);
			...
			if not HasState(stsDeletingUnPinnedTabs) then AddState(stsControlPositionsInvalidated);

			Redraw;
		finally FInvalidatingControls:=FALSE;
		end;
	end;
end;
...
begin
	...
	if ActiveTab<>nil then begin
		TabControls[ActiveTabIndex].SetDrawState({$IFDEF EXPLICIT_DRAW_STATE}TDrawState.{$ENDIF}dsActive,0,
{$IFDEF EXPLICIT_DRAW_STATE}TChromeTabsEaseType.{$ENDIF}ttNone,TRUE);
	end;
end;
...
begin
	...
	try LoadLookAndFeel(FileStream);
	finally FreeAndNil(FileStream);
	end;
end;
...
var
	i:integer;
begin
	for i:=0 to pred(FTabs.Count) do SetControlDrawState(TabControls[i],TabControls[i].DrawState,TRUE);
end;

procedure TCustomChromeTabs.SetActiveTab(const Value:TChromeTab);
begin
	if Value<>nil then FTabs.ActiveTab:=Value;
end;

procedure TCustomChromeTabs.SetActiveTabIndex(const Value:integer);
begin
	if Value=-1 then Tabs.ActiveTab:=nil
	else if IsValidTabIndex(Value) then Tabs.ActiveTab:=FTabs[Value];
end;
...
procedure TCustomChromeTabs.TabDragDrop(Sender:TObject;X,Y:integer;DragTabObject:IDragTabObject;Cancelled:Boolean;
	var TabDropOptions:TTabDropOptions);
begin
	...
	if FDragTabControl<>nil then SetControlLeft(FDragTabControl,FDragTabControl.ControlRect.Left+ScrollOffset,FALSE);

	DoOnTabDragDrop(X,Y,DragTabObject,Cancelled,TabDropOptions);
end;

procedure TCustomChromeTabs.TabDragOver(Sender:TObject;X,Y:integer;State:TDragState;DragTabObject:IDragTabObject;
	var Accept:Boolean);
begin
	...
	if (Accept)or(State=dsDragLeave) then begin
		FActiveDragTabObject:=DragTabObject;

		// Do we need to scroll the container?
		if (State<>dsDragLeave)and(FOptions.Scrolling.Enabled)and(FOptions.Scrolling.DragScroll) then begin
			DoOnDebugLog('Drag - X: %d, Bidi Left: %d, Offset: %d',[X,BidiRect(FTabContainerRect).Left,
				ScaledPixels(FOptions.Scrolling.DragScrollOffset)]);

			if // (X <= BidiRect(FTabContainerRect).Right) and
				(X>=BidiRect(FTabContainerRect).Right-ScaledPixels(FOptions.Scrolling.DragScrollOffset)) then begin
				ScrollTabs(mdsRight);
			end else

				if // (X >= BidiRect(FTabContainerRect).Left) and
				(X<=BidiRect(FTabContainerRect).Left+ScaledPixels(FOptions.Scrolling.DragScrollOffset)) then begin
				ScrollTabs(mdsLeft);
			end
		...
		case State of
			dsDragEnter:begin
					...
					if DragTabObject.SourceControl.GetControl=Self then FDragTabControl:=TabControls[ActiveTabIndex]
					else
						...
						if FDragTabControl=nil then begin
							...
							if ActiveTabIndex<>-1 then SetControlDrawState(TabControls[ActiveTabIndex],dsNotActive,TRUE);
						end;
			...
			dsDragLeave:begin
					...
					if FDragTabControl<>nil then FDragTabControl.ScrollableControl:=TRUE;

					FScrollTimer.Enabled:=FALSE;

					if DraggingInOwnContainer then begin
						FDragTabControl:=nil;
					end else begin
...
begin
	...
	if FDragTabObject.DragForm<>nil then begin
		// Move the drag form off the screen. This stops flicker on multi monitor systems
		if not FDragTabObject.DragForm.Visible then begin
...
begin
	...
	if Assigned(PopupMenu) then begin
		...
		if (FOptions.Behaviour.UseBuiltInPopupMenu)and(FDragTabObject=nil) then DoPopup(Self,ScreenPoint);
	end;
end;
...
begin
	if (FDragTabObject<>nil)and(Message.Msg=CN_KEYDOWN)and(Message.WParam=VK_ESCAPE) then begin
		FDragCancelled:=TRUE;
...
procedure TCustomChromeTabs.DragCompleted(MouseX,MouseY:integer;DragTabObject:IDragTabObject;Cancelled:Boolean);
begin
	DoOnTabDragEnd(MouseX,MouseY,DragTabObject,Cancelled);
...
begin
	Result:=(ActiveDragTabObject<>nil)and(ActiveDragTabObject.DockControl<>nil)and
		(ActiveDragTabObject.DockControl.GetControl=Self);
end;

function TCustomChromeTabs.DraggingInOwnContainer:Boolean;
begin
	Result:=(FDragTabObject<>nil)and((FDragTabObject.DockControl<>nil)and(FDragTabObject.DockControl.GetControl=Self));
end;

function TCustomChromeTabs.ActiveTabVisible:Boolean;
begin
	Result:=(FDragTabObject=nil)or((FDragTabObject.DockControl<>nil)and((FDragTabObject.SourceControl=nil)or
		(FDragTabObject.SourceControl.GetControl=Self)));

	// Split for clarity
	if Result then begin
		if (FDragTabObject<>nil)and(FDragTabObject.SourceControl<>nil)and(FDragTabObject.SourceControl.GetControl=Self)and
			((FDragTabObject.DockControl=nil)or(FDragTabObject.DockControl.GetControl<>Self)) then Result:=FALSE;
	end;
end;
...
var
	...
	begin
		...
		if FOptions.Display.TabContainer.OverlayButtons then
				...
				ClientHeight-ScaledPixels(FOptions.Display.Tabs.OffsetBottom)))))
		else TabCanvas.SetClip(RectToGPRectF(BidiRect(TabContainerRect)));
	end;
...
begin
	...
	if ControlReady then
		try
			...
			try
				if (HasState(stsControlPositionsInvalidated))or((HasState(stsAnimatingMovement))and(GetTabDisplayState=tdNormal))
				then begin
				...
				if HasState(stsFirstPaint) then begin
				...
				try
					...
					for i:=pred(FTabs.Count) downto 0 do begin
						if (Tabs[i].Visible)and(i<>ActiveTabIndex)and(TabControls[i].ControlRect.Right>=ScrollOffset)and
							(TabControls[i].ControlRect.Left<=CorrectedClientWidth+ScrollOffset) then begin
							SetTabClipRegion;

							ClipPolygons:=nil;

							// Save the current clip region of the GPGraphics
							if (not FOptions.Display.Tabs.SeeThroughTabs)and((FDragTabObject=nil)or(ActiveTabIndex<>i)) then begin
								...
								if (PrevVisibleIndex<>-1)and((FDragTabObject=nil)or(PrevVisibleIndex<>ActiveTabIndex)) then
								...
								if (ActiveTab<>nil)and(i+1=ActiveTabIndex)and(FDragTabObject=nil) then
					...
					if FOptions.Display.Tabs.BaseLineTabRegionOnly then SetTabClipRegion
					else TabCanvas.ResetClip;
					...
					// Draw the active tab
					if (ActiveTabIndex<>-1)and(ActiveTabVisible)and(TabControls[ActiveTabIndex].ControlRect.Right>=ScrollOffset)
						and(TabControls[ActiveTabIndex].ControlRect.Left<=CorrectedClientWidth+ScrollOffset) then
					...
					if (FOptions.Display.AddButton.Visibility<>avNone)and
						((not DraggingDockedTab)or(not FActiveDragTabObject.HideAddButton))and
					...
					if FOptions.Scrolling.Enabled then begin
						if ScrollButtonLeftVisible then FScrollButtonLeftControl.DrawTo(TabCanvas,FLastMouseX,FLastMouseY);

						if ScrollButtonRightVisible then FScrollButtonRightControl.DrawTo(TabCanvas,FLastMouseX,FLastMouseY);
					end;
			...
			finally DoOnDebugLog('Canvas Draw: %d ms',[GetTickCount-DrawTicks]);
			end;
...
begin
	Result:=Rect(ARect.Left-BidiScrollOffset,ARect.Top,ARect.Right-BidiScrollOffset,ARect.Bottom);
end;

procedure TCustomChromeTabs.ScrollIntoView(Tab:TChromeTab);
begin
	if TabControls[Tab.Index].EndRect.Left<ScrollOffset then ScrollOffset:=TabControls[Tab.Index].EndRect.Left
	else

		if TabControls[Tab.Index].EndRect.Right>TabContainerRect.Right+ScrollOffset then
			ScrollOffset:=TabControls[Tab.Index].EndRect.Right-RectWidth(TabContainerRect)+
			ScaledPixels(FOptions.Display.Tabs.TabOverlap);
end;

function TCustomChromeTabs.ScrollRect(ALeft,ATop,ARight,ABottom:integer):TRect;
begin
	Result:=Rect(ALeft-ScrollOffset,ATop,ARight-ScrollOffset,ABottom);
end;
	...
	try
		...
		try
			for i:=low(Targets) to high(Targets) do begin
				...
				if not FOptions.Display.TabContainer.TransparentBackground then begin
		...
		finally FreeAndNil(GPImage);
		end;
	finally FreeAndNil(MemStream);
	end;
end;
...
begin
	...
	if Result then ChromeTabs:=TCustomChromeTabs(WinControl)
	else ChromeTabs:=nil;
end;
...
begin
	...
	while i>=0 do begin
		if i=0 then FindChromeTabsControlAt(FPoint.X,FPoint.Y,FChromeTabs)
		else begin
			if (FindChromeTabsControlAt(FPoint.X-i,FPoint.Y-i,FChromeTabs))or
				...
				(FindChromeTabsControlAt(FPoint.X+i,FPoint.Y+i,FChromeTabs))or
				(FindChromeTabsControlAt(FPoint.X-i,FPoint.Y+i,FChromeTabs)) then Break;
		end;
