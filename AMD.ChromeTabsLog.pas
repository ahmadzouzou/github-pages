...
begin
	if Self<>nil then Log([format(Text,Args)],Row);
end;
...
procedure TChromeTabsLog.DoGetText(Node:PVirtualNode;Column:TColumnIndex;TextType:TVSTTextType;var Text:UnicodeString);
begin
	inherited;
...
begin
	if AColumn<0 then AColumn:=0;

	if (ARow<FLogStrings.Count)and(AColumn<TStringList(FLogStrings[ARow]).Count) then
...
begin
	if Self<>nil then begin
		BeginUpdate;
		try
			if Row=-1 then Row:=FLogStrings.Count;

			while Row>=FLogStrings.Count do FLogStrings.Add(TStringList.Create);

			for i:=Low(ColumnText) to High(ColumnText) do begin
				if i>=Header.Columns.Count then Header.Columns.Add;

				if i>=TStringList(FLogStrings[Row]).Count then TStringList(FLogStrings[Row]).Add('');

				TStringList(FLogStrings[Row])[i]:=ColumnText[i];

				if i<=10 then Header.AutoFitColumns(FALSE);
			end;

			RootNodeCount:=FLogStrings.Count;

			if (not Focused)and(FAutoScroll) then ScrollIntoView(GetLast,FALSE);
...
begin
	BeginUpdate;
	try
		if Row=-1 then Row:=Items.Count;

		while Row>=Items.Count do Items.Add;

		for i:=Low(ColumnText) to High(ColumnText) do begin
			if i>=Columns.Count then Columns.Add;

			if Items[Row].SubItems.Count<pred(length(ColumnText)) then Items[Row].SubItems.Add('');

			if i=0 then Items[Row].Caption:=ColumnText[i]
			else Items[Row].SubItems[i-1]:=ColumnText[i]
		end;

		if (Items.Count>0)and(not Focused)and(FAutoScroll) then Items[Items.Count-1].MakeVisible(FALSE);
	finally EndUpdate;
	end;
end;
...
function TChromeTabsLog.GetColumnText(AColumn,ARow:Integer):String;
begin
	if (ARow<Items.Count)and((AColumn=0)or(AColumn-1<Items[ARow].SubItems.Count)) then begin
		if AColumn=0 then Result:=Items[ARow].Caption
		else Result:=Items[ARow].SubItems[AColumn-1];
	end
	else Result:='';
end;
...
begin
	...
	if ColumnWidth=0 then Column.Width:=ClientWidth
