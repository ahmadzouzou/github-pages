...
begin
	...
	if (not IsRTF(s)) then begin
		Result:=sRtfHeader;
		p:=PChar(s);
		while (ord(p^)<>0) do begin
			c:=p^;
			cAnsi:=AnsiChar(c);
			...
			begin
				case cAnsi of
					'\','{','}':Result:=Result+'\'+cAnsi;
					Chr(9):Result:=Result+'\tab ';
					Chr(13):Result:=Result+'\par ';
				else if (ord(cAnsi)>31)and(ord(cAnsi)<127) then Result:=Result+cAnsi
					else Result:=Result+AnsiString(Format('\''%.2x',[ord(c)]));
				end;
			end
			else // Char is Unicode
			begin
				if (cAnsi<' ')or(cAnsi in ['0'..'9','\','{','}']) then cAnsi:='?';
...
begin
	...
	if (ord(Data[Result])<>0) then inc(Result);
end;
...
function TCustomUnicodeTextClipboardFormat.ReadData(Value:pointer;Size:integer):boolean;
begin
...
function TCustomUnicodeTextClipboardFormat.WriteData(Value:pointer;Size:integer):boolean;
begin
	Result:=(Size<=(Length(FText)+1)*2);
	if (Result) then Move(PWideChar(FText)^,Value^,Size);
end;
...
begin
	...
	// the string's implicit terminating zero.
	if (FText[Result]<>#0) then inc(Result);
...
begin
	...
	// unit as CF_RTF
	if (CF_RTF=0) then CF_RTF:=RegisterClipboardFormat('Rich Text Format'); // *** DO NOT LOCALIZE ***
	Result:=CF_RTF;
end;
...
begin
	if (CF_CSV=0) then CF_CSV:=RegisterClipboardFormat('CSV'); // *** DO NOT LOCALIZE ***
	Result:=CF_CSV;
end;
...
begin
	...
	else if (Source is TUnicodeTextClipboardFormat) then begin
		FUnicodeText:=TUnicodeTextClipboardFormat(Source).Text;
		FNeedUnicode:=False;
		FNeedAnsi:=FNeedAnsi or((FUnicodeText<>'')and(FAnsiText=''));
	end

	else if (Source is TLocaleClipboardFormat) then Locale:=TLocaleClipboardFormat(Source).Locale

{$IFDEF DROPSOURCE_TEXTSCRAP}
	else if (Source is TFileContentsClipboardFormat) then AnsiText:=TFileContentsClipboardFormat(Source).Data
{$ENDIF}
...
begin
	...
	if (Dest is TAnsiTextClipboardFormat) then begin
		...
		if (Dest is TRichTextClipboardFormat) then begin
		TRichTextClipboardFormat(Dest).Text:=MakeRTF(Text);
	end else

		if (Dest is TUnicodeTextClipboardFormat) then begin
		TUnicodeTextClipboardFormat(Dest).Text:=UnicodeText;
	end else

		if (Dest is TOEMTextClipboardFormat) then begin
		// Synthesize OEM text
		...
		else TOEMTextClipboardFormat(Dest).Text:='';
	end else

		if (Dest is TLocaleClipboardFormat) then begin
...
{$IFDEF DROPSOURCE_TEXTSCRAP}
		...
		if (Dest is TFileContentsClipboardFormat) then begin
			...
			if (Dest is TFileGroupDescritorClipboardFormat) then begin
			...
			if (Dest is TFileGroupDescritorWClipboardFormat) then begin
...
begin
	if (FNeedAnsi)and(not FNeedUnicode) then begin
		SetLength(FAnsiText,Length(FUnicodeText));
		// Take Locale into account and synthesize ANSI text from Unicode text.
		WideCharToMultiByte(CodePage,0,PWideChar(FUnicodeText),Length(FUnicodeText),PAnsiChar(FAnsiText),
			Length(FAnsiText),nil,nil);
...
begin
	if (FLocale=0)and not(csDesigning in Owner.ComponentState) then Result:=GetThreadLocale
	else Result:=FLocale;
end;
...
begin
	if (FNeedUnicode)and(not FNeedAnsi) then begin
		SetLength(FUnicodeText,Length(FAnsiText));
		// Take Locale into account and synthesize Unicode text from ANSI text.
		MultiByteToWideChar(CodePage,0,PAnsiChar(FAnsiText),Length(FAnsiText),PWideChar(FUnicodeText),Length(FUnicodeText));
...
begin
	...
	FNeedAnsi:=False;
	FNeedUnicode:=True;
	if (FLocale=0)and not(csDesigning in Owner.ComponentState) then FLocale:=GetThreadLocale;
end;
...
begin
	Result:=((not FNeedAnsi)and(FAnsiText<>''))or((not FNeedUnicode)and(FUnicodeText<>''));
end;

function TTextDataFormat.NeedsData:boolean;
begin
	Result:=(FNeedAnsi and(FAnsiText=''))or(FNeedUnicode and(FUnicodeText=''));
end;
...
begin
	...
	RegisterDataConversion(TCSVClipboardFormat);
