...
type
	TUICommandType=(ctUnknown,ctGroup,ctAction,ctAnchor,ctContext,ctCollection,ctCommandCollection,ctDecimal,ctBoolean,
		ctFont,ctRecentItems,ctColorAnchor,ctColorCollection);
	...
	TUIProperty=(upNone=-1,upUnknown,

		{ Common Properties }
		upEnabled=$0001,upLabelDescription,upKeytip,upLabel,upTooltipDescription,upTooltipTitle,upLargeImage,
		upLargeHighContrastImage,upSmallImage,upSmallHighContrastImage,

		{ Collection Properties }
		upCommandId=$0064,upItemsSource,upCategories,upCategoryId,upSelectedItem,upCommandType,upItemImage,

		{ Control Properties }
		upBooleanValue=$00C8,upDecimalValue,upStringValue,upMaxValue,upMinValue,upIncrement,upDecimalPlaces,upFormatString,
		upRepresentativeString,

		{ Font Control Properties }
		upFontProperties=$012C,upFontPropertiesFamily,upFontPropertiesSize,upFontPropertiesBold,upFontPropertiesItalic,
		upFontPropertiesUnderline,upFontPropertiesStrikethrough,upFontPropertiesVerticalPositioning,
		upFontPropertiesForegroundColor,upFontPropertiesBackgroundColor,upFontPropertiesForegroundColorType,
		upFontPropertiesBackgroundColorType,upFontPropertiesChangedProperties,upFontPropertiesDeltaSize,

		{ Application Menu Properties }
		upRecentItems=$015E,upPinned,

		{ Color Picker Properties }
		upColor=$0190,upColorType,upColorMode,upThemeColorsCategoryLabel,upStandardColorsCategoryLabel,
		upRecentColorsCategoryLabel,upAutomaticColorLabel,upNoColorLabel,upMoreColorsLabel,upThemeColors,upStandardColors,
		upThemeColorsTooltips,upStandardColorsTooltips,upPath,upDate,

		{ Ribbon Properties }
		upViewable=$03E8,upMinimized,upQuickAccessToolbarDock,

		{ Contextual Tabset Properties }
		upContextAvailable=$044C,

		{ Color Properties }
		upGlobalBackgroundColor=$07D0,upGlobalHighlightColor,upGlobalTextColor);
...
{$REGION 'Abstract base command'}
	...
	TUIImage=class;
	TUICommandUpdatePropertyEvent=procedure(Sender:TObject;const PropKey:TUIPropertyKey;var NewValue:TPropVariant;
		out Handled:Boolean) of object;
...
{$REGION 'Internal Declarations'}
	...
	strict protected
		...
		FValidProperties:set of (vpCaption,vpKeytip,vpTooltipTitle,vpTooltipDescription,vpLabelDescription,vpMinValue,
			vpMaxValue,vpIncrement,vpDecimalPlaces,vpRepresentativeString,vpFormatString);
	...
	private
		{ IUICommandHandler }
		function Execute(CommandId:UInt32;Verb:_UIExecutionVerb;Key:PUIPropertyKey;CurrentValue:PPropVariant;
			CommandExecutionProperties:IUISimplePropertySet):HRESULT;stdcall;

		function UpdateProperty(CommandId:UInt32;const Key:TUIPropertyKey;CurrentValue:PPropVariant;
			out NewValue:TPropVariant):HRESULT;stdcall;
	...
	strict protected
		procedure DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
			var Result:HRESULT);virtual;abstract;
		procedure DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
...
{$ENDREGION 'Internal Declarations'}
	public
		...
		property OnUpdateProperty:TUICommandUpdatePropertyEvent read FOnUpdateProperty write FOnUpdateProperty;
		...
		property OnUpdateHint:TUICommandUpdateHintEvent read FOnUpdateHint write FOnUpdateHint;
...
{$REGION 'Decimal command'}
	...
	TUICommandDecimalChangeEvent=procedure(const Command:TUICommandDecimal;const Verb:TUICommandVerb;const Value:Double;
...
{$REGION 'Internal Declarations'}
	...
	strict private
		procedure SetLabelDescription(const Value:String);
	private
		procedure DoToggle(const Verb:TUICommandVerb;const CurrentValue:PPropVariant);
	strict protected
		procedure DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
			var Result:HRESULT);override;
		procedure DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
...
{$ENDREGION 'Internal Declarations'}
	public
		...
		property StandardColorsCategoryLabel:String read GetStandardColorsCategoryLabel
			write SetStandardColorsCategoryLabel;
...
{$ENDREGION 'Internal Declarations'}
	...
	TUICollectionChangeEvent=procedure(const Collection:TUICollection;const Immediate:Boolean) of object;
...
{$ENDREGION 'Internal Declarations'}
	public
		constructor Create(const pLabel:string;const pPath:string='';pDescription:string='';pDate:string='';const pPinned:Boolean=False);
		...
		property Datex:String read FDate write FDate;
...
{$REGION 'Recent Items command'}
	...
	TUICommandRecentItemsSelectEvent=procedure(const Command:TUICommandRecentItems;const Verb:TUICommandVerb;
		const ItemIndex:Integer;const Properties:TUICommandExecutionProperties) of object;
...
{$ENDREGION 'Internal Declarations'}
	public
		...
		procedure Add(const pCaption:string;const pPath:string='';const pDescription:string='';const pDate:string='');
		class function CommandType:TUICommandType;override;
...
{$ENDREGION 'Internal Declarations'}
	public
		...
		constructor Create(const Filename:String;const HighContrast:Boolean=False);overload;
		...
		procedure Draw(const Target:TCanvas;const XTarget,YTarget,WTarget,HTarget:Integer);overload;
...
const
	UI_COMMAND_CLASSES: array [TUICommandType] of TUICommandClass=(nil, // ctUnknown,
...
const
	UI_PROPERTIES: array [TUIProperty] of PUIPropertyKey=(nil,nil,

		@UI_PKEY_Enabled,@UI_PKEY_LabelDescription,@UI_PKEY_Keytip,@UI_PKEY_Label,@UI_PKEY_TooltipDescription,
		@UI_PKEY_TooltipTitle,@UI_PKEY_LargeImage,@UI_PKEY_LargeHighContrastImage,@UI_PKEY_SmallImage,
		...
		@UI_PKEY_CommandId,@UI_PKEY_ItemsSource,@UI_PKEY_Categories,@UI_PKEY_CategoryId,@UI_PKEY_SelectedItem,
		@UI_PKEY_CommandType,@UI_PKEY_ItemImage,
		...
		@UI_PKEY_BooleanValue,@UI_PKEY_DecimalValue,@UI_PKEY_StringValue,@UI_PKEY_MaxValue,@UI_PKEY_MinValue,
		@UI_PKEY_Increment,@UI_PKEY_DecimalPlaces,@UI_PKEY_FormatString,@UI_PKEY_RepresentativeString,
		...
		@UI_PKEY_FontProperties,@UI_PKEY_FontProperties_Family,@UI_PKEY_FontProperties_Size,@UI_PKEY_FontProperties_Bold,
		@UI_PKEY_FontProperties_Italic,@UI_PKEY_FontProperties_Underline,@UI_PKEY_FontProperties_Strikethrough,
		@UI_PKEY_FontProperties_VerticalPositioning,@UI_PKEY_FontProperties_ForegroundColor,
		@UI_PKEY_FontProperties_BackgroundColor,@UI_PKEY_FontProperties_ForegroundColorType,
		@UI_PKEY_FontProperties_BackgroundColorType,@UI_PKEY_FontProperties_ChangedProperties,
		@UI_PKEY_FontProperties_DeltaSize,

		nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,
		nil,nil,nil,nil,nil,nil,nil,

		@UI_PKEY_RecentItems,@UI_PKEY_Pinned,

		nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,
		nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,

		@UI_PKEY_Color,@UI_PKEY_ColorType,@UI_PKEY_ColorMode,@UI_PKEY_ThemeColorsCategoryLabel,
		@UI_PKEY_StandardColorsCategoryLabel,@UI_PKEY_RecentColorsCategoryLabel,@UI_PKEY_AutomaticColorLabel,
		@UI_PKEY_NoColorLabel,@UI_PKEY_MoreColorsLabel,@UI_PKEY_ThemeColors,@UI_PKEY_StandardColors,
		...
		@UI_PKEY_Viewable,@UI_PKEY_Minimized,@UI_PKEY_QuickAccessToolbarDock,
...
begin
	...
	for Prop:=Low(TUIProperty) to High(TUIProperty) do begin
		Key:=UI_PROPERTIES[Prop];
		if Assigned(Key) then begin
			if (Key.FmtId.D1<>Cardinal(Ord(Prop))) then
					Assert(False,Format('Property map error. Prop=%.4x, Key=%.4x',[Ord(Prop),Key.FmtId.D1]));
		end;
	end;
end;
{$ENDIF}
...
function GetProperty(const Key:TUIPropertyKey):TUIProperty;
begin
	if (Key.FmtId.D1<=Cardinal(Ord(High(TUIProperty)))) then Result:=TUIProperty(Key.FmtId.D1)
	else Result:=upUnknown;
end;
...
type
	...
	type
		TClients=TDictionary<Pointer,Cardinal>;
	strict private
...
begin
	if (FNotificationWindow<>0) then begin
...
procedure TNotifier.Notification(const Command:TUICommand;const Flag:TNotifierFlag);
var
	Flags:Cardinal;
begin
	{ Don't send this notification if it has already been sent. }
	if (not(FClients.TryGetValue(Command,Flags))) then Flags:=0;

	if ((Flags and(1 shl Flag))=0) then begin
...
var
	Flags:Cardinal;
begin
	if (FClients.TryGetValue(Command,Flags)) then begin
		Command.Notify(Flag);
...
begin
	if (Msg.Msg=WM_RIBBONCMD_NOTIFY) then Notify(TUICommand(Msg.LPARAM),Msg.WParam)
	else Msg.Result:=DefWindowProc(FNotificationWindow,Msg.Msg,Msg.WParam,Msg.LPARAM);
end;

{ TUICommandExecutionProperties }

function TUICommandExecutionProperties.GetValue(const Prop:TUIProperty;out Value:Cardinal):Boolean;
var
	Key:PUIPropertyKey;
	PropVar:TPropVariant;
begin
	Key:=UI_PROPERTIES[Prop];
	Result:=Assigned(FPropertySet)and Assigned(Key)and Succeeded(FPropertySet.GetValue(Key^,PropVar));
	if Result then UIPropertyToUInt32(Key^,PropVar,Value)
end;

function TUICommandExecutionProperties.GetValue(const Prop:TUIProperty;out Value:String):Boolean;
var
	Key:PUIPropertyKey;
	PropVar:TPropVariant;
begin
	...
	if Result then begin
		...
		if Result then Value:=PropVar.bstrVal;
...
procedure TUICommand.CachePropertyValue(const Key:TUIPropertyKey;const Value:TValue);
begin
	if (FCachedProperties=nil) then FCachedProperties:=TDictionary<TUIPropertyKey,TValue>.Create;
	FCachedProperties.AddOrSetValue(Key,Value);
end;

constructor TUICommand.Create(const Ribbon:TObject;const pAction:TCustomAction);
begin
	Create(Ribbon,0);
...
constructor TUICommand.Create(const Ribbon:TObject;const CommandId:Cardinal);
begin
	if (not(Ribbon is TUIRibbon)) then raise EArgumentException.Create('Invalid Ribbon parameter');
...
begin
	if Assigned(Notifier) then Notifier.Remove(Self);
...
procedure TUICommand.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
	var Result:HRESULT);
...
var
	...
	var
		Handle:IUIImage;
	begin
		if ((Image=nil)or(Image.Handle=nil))and Assigned(CurrentValue) then begin
			UIPropertyToImage(PropKey,CurrentValue^,Handle);
			if (Image=nil) then begin
				Image:=TUIImage.Create(Handle);
				Image.OnChanged:=ImageChanged;
			end
			else Image.FHandle:=Handle;
		end else if Assigned(Image) then UIInitPropertyFromImage(PropKey,Image.Handle,NewValue);
	end;
...
begin
	...
	case Prop of
		upKeytip:begin
				if (vpKeytip in FValidProperties) then UIInitPropertyFromString(UI_PKEY_Keytip,FKeytip,NewValue);
			end;
		upLabel:begin
				if (vpCaption in FValidProperties) then UIInitPropertyFromString(UI_PKEY_Label,FCaption,NewValue);
			end;

		upTooltipTitle:begin
				if (vpTooltipTitle in FValidProperties) then
						UIInitPropertyFromString(UI_PKEY_TooltipTitle,FTooltipTitle,NewValue);
			end;

		upTooltipDescription:begin
		...
		upLargeImage:UpdateImage(FLargeImage,UI_PKEY_LargeImage);

		upSmallImage:UpdateImage(FSmallImage,UI_PKEY_SmallImage);

		upLargeHighContrastImage:UpdateImage(FLargeHighContrastImage,UI_PKEY_LargeHighContrastImage);

		upSmallHighContrastImage:UpdateImage(FSmallHighContrastImage,UI_PKEY_SmallHighContrastImage);
	end;
end;

function TUICommand.Execute(CommandId:UInt32;Verb:_UIExecutionVerb;Key:PUIPropertyKey;CurrentValue:PPropVariant;
...
var
	...
	Pinned:Boolean;
	psa:PSafeArray;
	pv:IUICollectionItem;
	RecentFile:UInt32;
	Value:TPropVariant;
	SimplePropertySet:IUISimplePropertySet;
begin
	try
		if (Key=nil) then Prop:=upNone
		...
		// Process otherwise completely unhandled exceptions in this scenario, see #24311
	end; // CmdRecentItems
	if ((CommandId=57)and Assigned(Key)and(Key^=UI_PKEY_RecentItems)and Assigned(CurrentValue)and
		(CurrentValue^.vt=VT_ARRAY+VT_UNKNOWN)) then begin
		psa:=nil;
		...
		if (Succeeded(Result)) then begin
			...
			for I:=0 to Count do
				if (Succeeded(Result)) then begin
					...
					if (Succeeded(Result)and Assigned(pv)) then begin
						Result:=pv.QueryInterface(IUISimplePropertySet,SimplePropertySet);
						if (Succeeded(Result)) then Result:=SimplePropertySet.GetValue(UI_PKEY_Pinned,Value);
						Result:=UIPropertyToBoolean(UI_PKEY_Pinned,Value,Pinned);
						if (Succeeded(Result)) then
							((FRibbon AS TUIRibbon).RecentItems.Items[I] as TUIRecentItem).Pinned:=Pinned;
					end;
				end;
			SafeArrayDestroy(psa);
		end;
	end;
	if (Assigned(Key)and(Key^=UI_PKEY_Minimized)and Assigned(CurrentValue)and(CurrentValue^.vt=VT_BOOL)) then begin
		Result:=UIPropertyToBoolean(UI_PKEY_Minimized,CurrentValue^,Pinned);

	end;
	if (Assigned(Key)and(Key^=UI_PKEY_Viewable)and Assigned(CurrentValue)and(CurrentValue^.vt=VT_BOOL)) then begin
		Result:=UIPropertyToBoolean(UI_PKEY_Viewable,CurrentValue^,Pinned);
...
begin
	if (FActionLink=nil) then FActionLink:=CreateActionLink;
	Result:=FActionLink;
end;
...
procedure TUICommand.GetPropertyValue(const Key:TUIPropertyKey;out Value:Cardinal);
var
	Val:TPropVariant;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,Key,Val)) then UIPropertyToUInt32(Key,Val,Value)
	else Value:=0;
end;

procedure TUICommand.GetPropertyValue(const Key:TUIPropertyKey;out Value:Boolean);
var
	Val:TPropVariant;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,Key,Val)) then UIPropertyToBoolean(Key,Val,Value)
	else Value:=False;
end;

procedure TUICommand.GetPropertyValue(const Key:TUIPropertyKey;out Value:Integer);
var
	Val:TPropVariant;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,Key,Val)) then UIPropertyToUInt32(Key,Val,Cardinal(Value))
	else Value:=-1;
end;

procedure TUICommand.GetPropertyValue(const Key:TUIPropertyKey;out Value:String);
var
	Val:TPropVariant;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,Key,Val)) then UIPropertyToString(Key,Val,Value)
	else Value:='';
end;

procedure TUICommand.GetPropertyValue(const Key:TUIPropertyKey;out Value:Double);
var
	Val:TPropVariant;
	Dec:TDecimal;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,Key,Val)) then begin
...
begin
	if (Sender=FLargeImage) then FFramework.InvalidateUICommand(FCommandId,[UIInvalidationsProperty],@UI_PKEY_LargeImage)
...
var
	...
	Dec:TDecimal;
begin
	if (Flag=NOTIFY_CACHED_PROPERTIES)and Assigned(FCachedProperties) then begin
		for Prop in FCachedProperties do begin
			case Prop.Key.PId of
				VT_BOOL:UIInitPropertyFromBoolean(Prop.Key,Prop.Value.AsBoolean,Val);

				VT_I4,VT_UI4:UIInitPropertyFromUInt32(Prop.Key,UInt32(Prop.Value.AsInteger),Val);

				VT_LPWSTR:UIInitPropertyFromString(Prop.Key,Prop.Value.AsString,Val);
...
begin
	if (Value<>FAlive) then begin
		FAlive:=Value;
		if (FAlive)and Assigned(FCachedProperties) then Notifier.Notification(Self,NOTIFY_CACHED_PROPERTIES);
	end;
end;
...
procedure TUICommand.SetPropertyValue(const Key:TUIPropertyKey;const Value:Cardinal);
var
	Val:TPropVariant;
begin
	if (FAlive) then begin
		UIInitPropertyFromUInt32(Key,Value,Val);
		FFramework.SetUICommandProperty(FCommandId,Key,Val);
	end
	else CachePropertyValue(Key,Value);
end;

procedure TUICommand.SetPropertyValue(const Key:TUIPropertyKey;const Value:Boolean);
var
	Val:TPropVariant;
begin
	if (FAlive) then begin
...
procedure TUICommand.SetPropertyValue(const Key:TUIPropertyKey;const Value:Integer);
var
	Val:TPropVariant;
begin
	if (FAlive) then begin
...
procedure TUICommand.SetPropertyValue(const Key:TUIPropertyKey;const Value:String);
var
	Val:TPropVariant;
begin
	if (FAlive) then begin
...
procedure TUICommand.SetPropertyValue(const Key:TUIPropertyKey;const Value:Double);
var
	Val:TPropVariant;
	Dec:TDecimal;
begin
...
function TUICommand.UpdateProperty(CommandId:UInt32;const Key:TUIPropertyKey;CurrentValue:PPropVariant;
...
procedure TUICommandGroup.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
begin
	{ Nothing }
end;
...
procedure TUICommandAction.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
var
...
procedure TUICommandAction.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
	var Result:HRESULT);
begin
	inherited;
...
begin
	...
	if Result and Assigned(FOnExecute) then begin
		Args.Command:=Self;
		Args.Verb:=cvExecute;
		Args.Properties:=FProperties;
		FOnExecute(Args);
...
procedure TUICommandCollection.CategoriesChange(const Collection:TUICollection;const Immediate:Boolean);
begin
	if (Immediate and Alive) then Notify(NOTIFY_CATEGORIES)
	else
...
constructor TUICommandCollection.Create(const Ribbon:TObject;const CommandId:Cardinal);
begin
	inherited;
...
procedure TUICommandCollection.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;
	const CurrentValue:PPropVariant;var Result:HRESULT);
var
...
procedure TUICommandCollection.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;
	out NewValue:TPropVariant;var Result:HRESULT);
begin
	inherited;
	if (Prop=upRepresentativeString) then begin
...
function TUICommandCollection.HandleShortCut(const ShortCut:TShortCut):Boolean;
var
	Args:TUICommandCollectionEventArgs;
begin
	Result:=(ShortCut=FShortCut);
	if Result and Enabled and Assigned(FOnSelect) then begin
		Args.Command:=Self;
		Args.Verb:=cvExecute;
...
begin
	if (Immediate and Alive) then Notify(NOTIFY_ITEMS)
	else
		{ Don't change collection immediately. Postpone until next message pump in
			case multiple items in the collection are changed at once. }
...
procedure TUICommandDecimal.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
var
	Dec:TDecimal;
	Value:Double;
begin
	if Assigned(FOnChange) then begin
		if (CurrentValue=nil)or Failed(UIPropertyToDecimal(UI_PKEY_DecimalValue,CurrentValue^,Dec)) then Value:=0
		else VarR8FromDec(@Dec,Value);
		FOnChange(Self,Verb,Value,FProperties);
	end;
end;

procedure TUICommandDecimal.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
	var Result:HRESULT);
var
	Dec:TDecimal;
begin
	...
	case Prop of
		upMinValue:begin
				if (vpMinValue in FValidProperties) then begin
		...
		upMaxValue:begin
				if (vpMaxValue in FValidProperties) then begin
...
begin
	Result:=(ShortCut=FShortCut);
	if (Result)and Enabled and Assigned(FOnChange) then FOnChange(Self,cvExecute,GetValue,FProperties);
end;
...
procedure TUICommandBoolean.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
begin
	case Prop of
		upBooleanValue:DoToggle(Verb,CurrentValue);
	end;
end;

procedure TUICommandBoolean.DoToggle(const Verb:TUICommandVerb;const CurrentValue:PPropVariant);
var
...
procedure TUICommandBoolean.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
	var Result:HRESULT);
begin
	inherited;
	if (Prop=upLabelDescription)and(vpLabelDescription in FValidProperties) then
...
begin
	Result:=(ShortCut=FShortCut);
	if (Result)and Enabled and Assigned(FOnToggle) then begin
		Args.Command:=Self;
		Args.Verb:=cvExecute;
...
procedure TUICommandAnchor.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
begin
	{ Nothing }
end;
...
procedure TUICommandContext.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
begin
	{ Nothing }
end;
...
const
	Flags: array [Boolean,Boolean] of TUIFontProperty=((fpNotAvailable,fpNotAvailable),(fpOff,fpOn));
var
...
begin
	...
	if (caFace in Attrs) then SetFamily(Source.Name)
	else SetFamily('');

	if (caSize in Attrs) then SetSize(Source.Size/TWIPS_PER_POINT)
	else SetSize(0);
	...
	if (caColor in Attrs) then SetForegroundColor(Source.Color)
	else SetForegroundColor(clDefault);
end;
...
begin
	...
	if ((Source.dwMask and CFE_SUBSCRIPT)<>0) then begin
		if ((Source.dwMask and CFM_SUBSCRIPT)<>0)and((Source.dwEffects and CFE_SUBSCRIPT)<>0) then
				SetVerticalPositioning(vpSubscript)
		else if ((Source.dwEffects and CFE_SUPERSCRIPT)<>0) then SetVerticalPositioning(vpSuperscript)
		else SetVerticalPositioning(vpDefault);
	end else if ((Source.dwMask and CFM_OFFSET)<>0) then begin
		if (Source.yOffset>0) then SetVerticalPositioning(vpSuperscript)
		else if (Source.yOffset<0) then SetVerticalPositioning(vpSubscript)
		else SetVerticalPositioning(vpNotAvailable)
	end
	else SetVerticalPositioning(vpDefault);

	if ((Source.dwMask and CFM_FACE)<>0) then SetFamily(Source.szFaceName)
	else SetFamily('');

	if ((Source.dwMask and CFM_SIZE)<>0) then SetSize(Source.yHeight/TWIPS_PER_POINT)
	else SetSize(0);
	...
	else SetBackgroundColor(clNone);
end;
...
begin
	...
	PropVariantInit(PropVar);
	if (FSize<>0) then VarDecFromR8(FSize,DecSize)
	else
	...
	if (FForegroundColor=clDefault) then begin
		PropVariantInit(PropVar);
		...
		PropVariantClear(PropVar);
	end else if (FForegroundColor<>clNone) then begin
		PropVariantInit(PropVar);
	...
	if (FBackgroundColor=clNone) then begin
		...
		PropVariantClear(PropVar);
	end else if (FBackgroundColor<>clDefault) then begin
		PropVariantInit(PropVar);
...
var
	...
	begin
		...
		PropVariantInit(PropVar);
		if Succeeded(Source.GetValue(TPropertyKey(PropKey),PropVar)) then begin
...
begin
	...
	PropVariantInit(PropVar);
	if Succeeded(Source.GetValue(TPropertyKey(UI_PKEY_FontProperties_VerticalPositioning),PropVar)) then begin
	...
	PropVariantInit(PropVar);
	if Succeeded(Source.GetValue(TPropertyKey(UI_PKEY_FontProperties_Size),PropVar)) then begin
	...
	PropVariantInit(PropVar);
	if Succeeded(Source.GetValue(TPropertyKey(UI_PKEY_FontProperties_ForegroundColorType),PropVar)) then begin
		UIPropertyToUInt32(UI_PKEY_FontProperties_ForegroundColorType,PropVar,Value);
		if (Integer(Value)=Ord(UISwatchColorTypeAutomatic)) then FForegroundColor:=clDefault;
	end;
	PropVariantClear(PropVar);
	...
	PropVariantInit(PropVar);
	if Succeeded(Source.GetValue(TPropertyKey(UI_PKEY_FontProperties_BackgroundColorType),PropVar)) then begin
		UIPropertyToUInt32(UI_PKEY_FontProperties_BackgroundColorType,PropVar,Cardinal(Value));
		if (Integer(Value)=Ord(UISwatchColorTypeNoColor)) then FBackgroundColor:=clNone;
	end;
	PropVariantClear(PropVar);
...
procedure TUIFont.AssignTo(out Dest:TCharFormat2);
	...
	begin
		if (Prop<>fpNotAvailable) then begin
			Dest.dwMask:=Dest.dwMask or Mask;
			if (Prop=fpOn) then Dest.dwEffects:=Dest.dwEffects or Effect;
		end;
	end;
...
begin
	...
	if (FVerticalPositioning<>vpNotAvailable) then begin
		Dest.dwMask:=Dest.dwMask or(CFM_SUBSCRIPT or CFM_SUPERSCRIPT);
		if (FVerticalPositioning=vpSuperscript) then Dest.dwEffects:=Dest.dwEffects or CFE_SUPERSCRIPT
		else if (FVerticalPositioning=vpSubscript) then Dest.dwEffects:=Dest.dwEffects or CFE_SUBSCRIPT;
	end;

	if (FFamily<>'') then begin
...
begin
	...
	if (FFamily<>'') then Dest.Name:=FFamily;

	if (FSize<>0) then Dest.Size:=Trunc(FSize);
	...
	if (FForegroundColor<>clDefault)and(FForegroundColor<>clNone) then Dest.Color:=FForegroundColor;
end;
...
begin
	if (Value<>FForegroundColor) then begin
...
begin
	...
	if (NewTwips<>CurTwips) then begin
		FSize:=Value;
		Changed;
	end;
end;
...
begin
	if (Value<>FStrikethrough) then begin
...
procedure TUIFont.SetVerticalPositioning(const Value:TUIFontVerticalPositioning);
begin
	if (Value<>FVerticalPositioning) then begin
...
constructor TUICommandFont.Create(const Ribbon:TObject;const CommandId:Cardinal);
begin
	inherited;
...
procedure TUICommandFont.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;const CurrentValue:PPropVariant;
	var Result:HRESULT);
var
...
begin
	if (Prop=upFontProperties)and Assigned(FOnChanged) then begin
		Args.Command:=Self;
		Args.Verb:=Verb;
		Args.Font:=FFont;
		Args.Properties:=FProperties;
		if (Verb=cvCancelPreview) then FOnChanged(Args)
		else begin
			...
			try
				...
				if Succeeded(Result) then begin
					...
					if Succeeded(Result)and Supports(Unk,IPropertyStore,Changes) then begin
			...
			finally NewFont.Free;
			end;
		end;
	end;
end;

procedure TUICommandFont.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;out NewValue:TPropVariant;
	var Result:HRESULT);
var
	Unk:IUnknown;
...
begin
	inherited;
	if (Prop=upFontProperties)and Assigned(CurrentValue) then begin
		Result:=UIPropertyToInterface(UI_PKEY_FontProperties,CurrentValue^,Unk);
		if Supports(Unk,IPropertyStore,PropertyStore) then begin
...
begin
	inherited;
	if (Flag=NOTIFY_FONT) then FFramework.InvalidateUICommand(FCommandId,[UIInvalidationsAllProperties],nil);
end;
...
procedure TUICommandColorAnchor.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;
	const CurrentValue:PPropVariant;var Result:HRESULT);
var
...
begin
	if Assigned(FOnExecute) then begin
		if (CurrentValue=nil)or Failed(UIPropertyToUInt32(UI_PKEY_ColorType,CurrentValue^,Cardinal(ColorType))) then Exit;
...
begin
	case GetColorType of
		ctNoColor:Result:=clNone;

		ctAutomatic:Result:=clDefault;
	else GetPropertyValue(UI_PKEY_Color,Cardinal(Result));
	end;
end;
...
var
	...
	Count:Cardinal;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,UI_PKEY_StandardColors,Val)) then begin
...
var
	...
	I:Integer;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,UI_PKEY_StandardColorsTooltips,Val)) then begin
		Count:=PropVariantGetElementCount(Val);
...
var
	...
	Count:Cardinal;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,UI_PKEY_ThemeColors,Val)) then begin
		Count:=PropVariantGetElementCount(Val);
		SetLength(Result,Count);
		if (Count>0) then begin
			PropVariantToUInt32Vector(Val,@Result[0],Count,Count);
...
var
	...
	I:Integer;
begin
	if Succeeded(FFramework.GetUICommandProperty(FCommandId,UI_PKEY_ThemeColorsTooltips,Val)) then begin
		Count:=PropVariantGetElementCount(Val);
		SetLength(Strings,Count);
		if (Count>0) then begin
			PropVariantToStringVector(Val,@Strings[0],Count,Count);
...
function TUICommandColorAnchor.HandleShortCut(const ShortCut:TShortCut):Boolean;
var
	Args:TUICommandColorEventArgs;
begin
	Result:=(FShortCut=ShortCut);
	if Result and Enabled and Assigned(FOnExecute) then begin
		Args.Command:=Self;
		Args.Verb:=cvExecute;
...
begin
	if (Value=clNone) then SetColorType(ctNoColor)
	else if (Value=clDefault) then SetColorType(ctAutomatic)
	else begin
		SetColorType(ctRgb);
...
procedure TUICommandColorAnchor.SetRecentColorsCategoryLabel(const Value:String);
begin
...
var
	Val:TPropVariant;
begin
	if (Length(Value)=0) then InitPropVariantFromUInt32Vector(nil,0,Val)
	else InitPropVariantFromUInt32Vector(@Value[0],Length(Value),Val);
	FFramework.SetUICommandProperty(FCommandId,UI_PKEY_StandardColors,Val);
end;

procedure TUICommandColorAnchor.SetStandardColorsCategoryLabel(const Value:String);
begin
	SetPropertyValue(UI_PKEY_StandardColorsCategoryLabel,Value);
end;

procedure TUICommandColorAnchor.SetStandardColorTooltips(const Value:TArray<String>);
var
	Val:TPropVariant;
	Strings: array of PWideChar;
	I:Integer;
begin
	if (Length(Value)=0) then InitPropVariantFromStringVector(nil,0,Val)
	else begin
		SetLength(Strings,Length(Value));
		for I:=0 to Length(Value)-1 do Strings[I]:=PWideChar(Value[I]);
...
procedure TUICommandColorAnchor.SetThemeColorsCategoryLabel(const Value:String);
begin
	SetPropertyValue(UI_PKEY_ThemeColorsCategoryLabel,Value);
end;

procedure TUICommandColorAnchor.SetThemeColorTooltips(const Value:TArray<String>);
var
	Val:TPropVariant;
	Strings: array of PWideChar;
	I:Integer;
...
procedure TUICommandRecentItems.Add(const pCaption:string;const pPath:string='';const pDescription:string='';const pDate:string='');
var
	lItem:IUICollectionItem;
begin
	lItem:=TUIRecentItem.Create(pCaption,pDescription,pPath,pDate,False);
	Items.Add(lItem);
...
begin
	Items.Clear();
	for lPath in pPaths do Add(lPath);
end;
...
constructor TUICommandRecentItems.Create(const Ribbon:TObject;const CommandId:Cardinal);
begin
	inherited;
	FItems:=TUICollection.Create;
	FItems.OnChange:=ItemsChange;
...
procedure TUICommandRecentItems.DoExecute(const Prop:TUIProperty;const Verb:TUICommandVerb;
	const CurrentValue:PPropVariant;var Result:HRESULT);
var
	Index:Integer;
begin
	if Assigned(FOnSelect)and(Prop=upSelectedItem) then begin
...
procedure TUICommandRecentItems.DoUpdate(const Prop:TUIProperty;const CurrentValue:PPropVariant;
	out NewValue:TPropVariant;var Result:HRESULT);
var
	I,Count:Integer;
	UnkPtr:Pointer;
	List:PSafeArray;
	Unk:IUICollectionItem;
	Pinned:Boolean;
begin
	inherited;
	if Assigned(ActionLink) then ActionLink.Update;
	if (Prop=upRecentItems) then begin
		Count:=FItems.Count;
		List:=SafeArrayCreateVector(VT_UNKNOWN,0,Count);
		if Assigned(List) then begin
			for I:=0 to Count-1 do begin
				Unk:=(FItems[I] as IUICollectionItem);
				UnkPtr:=Pointer(Unk);
...
procedure TUICommandRecentItems.ItemsChange(const Collection:TUICollection;const Immediate:Boolean);
begin
	if (Immediate and Alive) then Notify(NOTIFY_ITEMS)
	else
		{ Don't change collection immediately. Postpone until next message pump in
...
begin
	inherited;
	if (Flag=NOTIFY_ITEMS) then FFramework.InvalidateUICommand(FCommandId,[UIInvalidationsProperty],@UI_PKEY_RecentItems);
end;

{ TUICollectionItem }

function TUICollectionItem.GetValue(const Key:TUIPropertyKey;out Value:TPropVariant):HRESULT;
var
	Prop:TUIProperty;
	Val:TValue;
	Intf:IInterface;
begin
	Result:=E_FAIL;
	Prop:=GetProperty(Key);
	if (Prop>upUnknown) then begin
		Val:=GetPropertyValue(Prop);
		case Key.PId of
			VT_LPWSTR:Result:=UIInitPropertyFromString(Key,Val.AsString,Value);

			VT_UI4:Result:=UIInitPropertyFromUInt32(Key,UInt32(Val.AsInteger),Value);
			...
			VT_BOOL:Result:=UIInitPropertyFromBoolean(Key,Val.AsBoolean,Value);
		else Assert(False,'Unsupported property type');
		end;
	end;
end;
...
begin
	if Assigned(FOnChange)and(FUpdateCount=0) then FOnChange(Self,Immediate);
end;
...
begin
	Dec(FUpdateCount);
	if (FUpdateCount=0) then Changed(True);
end;
...
begin
	...
	if (Unk=nil)or(not Supports(Unk,IUICollectionItem,Result)) then Result:=nil;
end;

procedure TUICollection.Insert(const Index:Integer;const Item:IUICollectionItem);
begin
	FHandle.Insert(Index,Item);
	Changed;
end;

procedure TUICollection.Replace(const Index:Integer;const Item:IUICollectionItem);
begin
...
constructor TUICollection.TUICollectionImpl.Create(const Items:TList<IUnknown>;const EnumIndex:Integer);
begin
	inherited Create;
	FItems:=Items;
	FOwnsItems:=False;
...
begin
	if (FOwnsItems) then FItems.Free;
	inherited;
end;
...
procedure TUICollection.TUICollectionImpl.Insert(Index:UInt32;const Item:IInterface);
begin
	FItems.Insert(Index,Item);
end;

function TUICollection.TUICollectionImpl.Next(celt:Integer;out elt;pceltFetched:PLongint):HRESULT;
var
	I:Integer;
	Items: array of IUnknown;
begin
	I:=FItems.Count-FEnumIndex;
	if (celt<=I) then Result:=S_OK
	else begin
		celt:=I;
		Result:=S_FALSE;
	end;

	if Assigned(pceltFetched) then pceltFetched^:=celt;
...
procedure TUICollection.TUICollectionImpl.Replace(IndexReplaces:UInt32;const ItemReplaceWith:IInterface);
begin
...
begin
	if ((FEnumIndex+celt)<=FItems.Count) then begin
		Inc(FEnumIndex,celt);
		// Sometimes, `celt` can be -1
		if FEnumIndex<0 then FEnumIndex:=0;
		Result:=S_OK;
	end
	else Result:=S_FALSE;
end;
...
begin
	if (FIndex>=FCollection.Count) then Result:=False
	else begin
		Inc(FIndex);
...
function TUIGalleryCollectionItem.GetPropertyValue(const Prop:TUIProperty):TValue;
begin
	case Prop of
		upLabel:Result:=FLabel;

		upCategoryId:Result:=FCategoryId;

		upCommandId: if Assigned(FCommand) then Result:=FCommand.CommandId
			else Result:=TValue.Empty;

		upCommandType: if Assigned(FCommand) then Result:=Ord(FCommand.CommandType)
			else Result:=TValue.Empty;

		upItemImage: if Assigned(FImage) then Result:=TValue.From<IUIImage>(FImage)
			else Result:=TValue.Empty;
	else Result:=TValue.Empty;
	end;
end;
...
constructor TUIRecentItem.Create(const pLabel:string;const pPath:string='';pDescription:string='';pDate:string='';const pPinned:Boolean=False);
begin
	Inherited Create();
	FLabel:=pLabel;
...
begin
	case Prop of
		...
		upLabelDescription:Result:=FDescription;

		upPath:Result:=FPath;

		upDate:Result:=FDate;

		upPinned:Result:=FPinned;
	end;
end;
...
var
	...
	P:PCardinal;
begin
	if (Bitmap.PixelFormat=pf32Bit)or(Bitmap.Width=0)or(Bitmap.Height=0) then Exit;
	...
	for Y:=0 to Bitmap.Height-1 do begin
		...
		for X:=0 to Bitmap.Width-1 do begin
			if (P^=TransparentColor) then P^:=0
			else P^:=P^or $FF000000;
			Inc(P);
		end;
	end;
end;
...
begin
	...
	try
		...
		if (GetDIBits(DC,Bitmap,0,0,nil,Info,DIB_RGB_COLORS)<>0) then begin
			W:=Info.bmiHeader.biWidth;
			H:=Abs(Info.bmiHeader.biHeight);
			Info.bmiHeader.biHeight:=-H;
			GetMem(Data,W*H*4);
			if (GetDIBits(DC,Bitmap,0,H,Data,Info,DIB_RGB_COLORS)<>0) then begin
				P:=Data;
...
procedure TUIImage.Draw(const Target:TCanvas;const XTarget,YTarget,WTarget,HTarget:Integer);
var
	SrcDC:HDC;
...
begin
	...
	try
		if (FBitsPerPixel=32) then begin
			...
			try
				...
				AlphaBlend(Target.Handle,XTarget,YTarget,WTarget,HTarget,SrcDC,0,0,FWidth,FHeight,BF);
...
begin
	if (FBitmap<>0) then Result:=FBitmap
	else if Assigned(FHandle) then begin
		FBitmap:=FHandle.GetBitmap;
		Result:=FBitmap
	end
	else Result:=0;
...
begin
	if (GetObject(FBitmap,SizeOf(Props),@Props)<>0) then begin
...
begin
	...
	if (Ext='.BMP') then LoadBmp(Filename,HighContrast)
	else if (Ext='.PNG') then LoadPng(Filename,HighContrast)
	else raise EInvalidGraphicOperation.CreateFmt('Unsupported image file extensions (%s)',[Ext]);
...
begin
	FHandle:=nil;
	if Assigned(FImageFactory) then begin
		FBitmap:=LoadImage(Instance,ResourceName,IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION);
		if (FBitmap=0) then raise EInvalidGraphic.Create('Cannot load bitmap');
...
begin
	if Assigned(FImageFactory) then begin
		...
		try
			...
			if (not HighContrast) then ConvertToAlphaBitmap(Bmp);
...
begin
	inherited Create;
	if Assigned(FImageFactory)and Assigned(pImageList)and(pImageList.Count>0)and(pImageIndex>=0) then begin
		lIcon:=TIcon.Create;
		...
		try
			...
			try lIcon.SaveToStream(lStream);
			except
...
if not Succeeded(CoCreateInstance(CLSID_UIRibbonImageFromBitmapFactory,nil,CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER,
	IUnknown,TUIImage.FImageFactory)) then TUIImage.FImageFactory:=nil;
