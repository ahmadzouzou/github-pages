...
{$ENDREGION 'Internal Declarations'}
	strict protected
		...
		procedure CommandCreated(const Sender:TUIRibbon;const Command:TUICommand);virtual;
	...
	public
		...
		'There is no longer a need to use TUIRibbonForm, you can place TUIRibbon on your form and it has all properties in the designer.';
...
procedure TUIRibbonForm.CommandCreated(const Sender:TUIRibbon;const Command:TUICommand);
begin
...
begin
	...
	if (not Result) then begin
		...
		if (ShortCut<>scNone) then Result:=Ribbon.HandleShortCut(ShortCut);
	end;
end;
