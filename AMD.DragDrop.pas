...
{$IF CompilerVersion >= 23.0}
	System.SysUtils,
	System.Classes,{$IFDEF DEBUG}System.Win.ComObj,{$ENDIF}System.Types,
	WinApi.Windows,
	WinApi.ActiveX,
	WinApi.Messages,
	WinApi.MMSystem,
...
{$ELSE}
	SysUtils,
	Classes,{$IFDEF DEBUG}ComObj,{$ENDIF}
	Windows,
	ActiveX,
	Messages,
	MMSystem,
...
type
	...
	protected
		...
		class procedure RegisterDataConversion(DataFormatClass:TDataFormatClass;Priority:integer=0;
	...
	public
		...
		function GetDataFromMedium(const ADataObject:IDataObject;var AMedium:TStgMedium):boolean;virtual;
		// Transfers data to the specified IDataObject
		function SetData(const ADataObject:IDataObject;const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;virtual;
		// Transfers data to the specified medium
		function SetDataToMedium(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
	...
	protected
		...
		class procedure RegisterDataConversion(ClipboardFormatClass:TClipboardFormatClass;Priority:integer=0;
			Capabilities:TConversionCapabilities=[ccWriteToDataFormat,ccReadFromDataFormat]);
	...
	public
		...
		constructor Create(Dummy:pointer);overload; {$IF CompilerVersion >= 17.0}deprecated
{$IF CompilerVersion >= 20.0} 'Use Create(TDataFormatDirection) instead of Create(nil)'{$IFEND}; {$IFEND}
...
{$IFEND}
	...
	public
		constructor Create;
		destructor Destroy;override;
		procedure Add(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass;Priority:integer=0;
			Capabilities:TConversionCapabilities=[ccWriteToDataFormat,ccReadFromDataFormat]);
		procedure Delete(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass);
		...
		class procedure GetClipboardFormats(const DataFormatClass:TDataFormatClass;ClipboardFormats:TClipboardFormats;
			Tier:TDragDropTier);
		function CanMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass):boolean;
		{ Registers the specified format mapping }
		procedure RegisterFormatMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass;
			Priority:integer=0;Capabilities:TConversionCapabilities=[ccWriteToDataFormat,ccReadFromDataFormat]);
		{ Unregisters the specified format mapping }
		procedure UnregisterFormatMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass);
	end;
	...
	public
		...
		destructor Destroy;override;
		property DataFormatClass:TDataFormatClass read FDataFormatClass write SetDataFormatClass;
		property DataFormat:TCustomDataFormat read FDataFormat;
	published
		property DragDropComponent:TDragDropComponent read FDragDropComponent write SetDragDropComponent;
		property DataFormatName:string read GetDataFormatName write SetDataFormatName;
...
function ShiftStateToDropEffect(Shift:TShiftState;AllowedEffects:Longint;Fallback:boolean):Longint;
...
uses
	AMD.DragDropFormats, // Used by TRawClipboardFormat
	AMD.DropSource,
	AMD.DropTarget;
...
resourcestring
	...
	sNoCopyStgMedium=
		'A required system function (URLMON.CopyStgMedium) was not available on this system. Operation aborted.';
...
{$IFDEF DEBUG}
	...
	begin
		...
		if (Result='') then Result:=GUID;
	end;
{$ENDIF}

begin
	if GetInterface(IID,Obj) then Result:=0
	else if (VCLComObject<>nil) then Result:=IVCLComObject(VCLComObject).QueryInterface(IID,Obj)
...
begin
	...
	if (Owner<>nil)and(Owner.GetInterface(IUnknown,Outer)) then Result:=Outer._AddRef
	else begin
		inherited _AddRef;
		Result:=-1;
	end;
...
begin
	...
	if (Owner<>nil)and(Owner.GetInterface(IUnknown,Outer)) then Result:=Outer._Release
	else begin
		inherited _Release;
		Result:=-1;
	end;
...
begin
	...
	Result:=0;
	while (Mask<>0)and(Mask<=FormatEtc.tymed) do begin
		AFormatEtc.tymed:=FormatEtc.tymed and Mask;
		if (AFormatEtc.tymed<>0) then
			if (Succeeded(ADataObject.QueryGetData(AFormatEtc))) then Result:=Result or Mask;
		Mask:=Mask shl 1;
	end;
end;
...
begin
	...
	AFormatEtc:=FormatEtc;
	Result:=False;
	while (not Result)and(Mask<>0)and(Mask<=FormatEtc.tymed) do begin
		AFormatEtc.tymed:=FormatEtc.tymed and Mask;
		if (AFormatEtc.tymed<>0) then Result:=(Succeeded(ADataObject.QueryGetData(AFormatEtc)));
		Mask:=Mask shl 1;
	end;
...
class procedure TClipboardFormat.RegisterDataProvider(DataFormatClass:TDataFormatClass;Priority:integer);
begin
	RegisterDataConversion(DataFormatClass,Priority,[ccReadFromDataFormat]);
end;

class procedure TClipboardFormat.RegisterDataConversion(DataFormatClass:TDataFormatClass;Priority:integer;
...
begin
	Result:=(AFormatEtc.cfFormat=FFormatEtc.cfFormat)and(AFormatEtc.ptd=nil)and(AFormatEtc.dwAspect=FFormatEtc.dwAspect)
		and(AFormatEtc.tymed AND FFormatEtc.tymed<>0);
end;
...
function TClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
begin
	Result:=False;
end;
...
begin
	...
	if (Succeeded(ADataObject.GetData(AFormatEtc,Medium))) then Result:=GetDataFromMedium(ADataObject,Medium)
	else Result:=False;
end;
...
function TClipboardFormat.GetDataFromMedium(const ADataObject:IDataObject;var AMedium:TStgMedium):boolean;
begin
	Result:=False;
	try
		Clear;
		if (AMedium.tymed and FFormatEtc.tymed<>0) then Result:=DoGetData(ADataObject,AMedium);
	finally ReleaseStgMedium(AMedium);
	end;
end;

function TClipboardFormat.SetDataToMedium(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
begin
	Result:=False;
	FillChar(AMedium,SizeOf(AMedium),0);
	if (FormatEtcIn.cfFormat<>FFormatEtc.cfFormat)or(FormatEtcIn.dwAspect<>FFormatEtc.dwAspect)or
		(FormatEtcIn.tymed and FFormatEtc.tymed=0) then exit;
	// Call descendant to allocate medium and transfer data to it
	Result:=DoSetData(FormatEtcIn,AMedium);
end;

function TClipboardFormat.SetData(const ADataObject:IDataObject;const FormatEtcIn:TFormatEtc;
...
begin
	...
	if (Result) then Result:=(Succeeded(ADataObject.SetData(FormatEtc,AMedium,True)));
	...
	if (not Result) then ReleaseStgMedium(AMedium);
end;

class procedure TClipboardFormat.UnregisterClipboardFormat;
begin
	if (not DragDropShutdown) then TDataFormatMap.Instance.DeleteByClipboardFormat(Self);
end;
...
//
/// /////////////////////////////////////////////////////////////////////////////
constructor TClipboardFormats.Create(ADataFormat:TCustomDataFormat;AOwnsObjects:boolean);
begin
	inherited Create;
	FList:=TList.Create;
...
begin
	...
	if (FOwnsObjects)and(DataFormat<>nil) then ClipboardFormat.DataFormat:=DataFormat;
end;
...
begin
	...
	for i:=0 to Count-1 do
		if (Formats[i].InheritsFrom(ClipboardFormatClass)) then begin
			Result:=Formats[i];
			exit;
		end;
	Result:=nil;
...
begin
	// Search list for an object of the specified type
	for i:=0 to Count-1 do
		if (Formats[i].ClipboardFormat=ClipboardFormat) then begin
			Result:=Formats[i];
			exit;
		end;
	Result:=nil;
...
const
	DirectionToScope: array [TDataFormatDirection] of TDragDropTier=(ddtSource,ddtTarget);
begin
	Create(TDragDropComponent(nil));
	CreateCompatibleFormatsList;
	// Populate the compatible formats list
	TDataFormatMap.GetClipboardFormats(TDataFormatClass(Self.ClassType),FCompatibleFormats,DirectionToScope[Direction]);
end;
...
begin
	...
	if (Owner<>nil) then Owner.DataFormats.Remove(Self);
	inherited Destroy;
end;
...
begin
	if (FCompatibleFormats=nil) then FCompatibleFormats:=TClipboardFormats.Create(Self,True);
end;

function TCustomDataFormat.GetCompatibleFormats:TClipboardFormats;
begin
	if (FCompatibleFormats=nil) then begin
...
begin
	...
	// need anymore data.
	while (NeedsData)and(i<CompatibleFormats.Count) do begin
		CompatibleFormats[i].Clear;
		if (CompatibleFormats[i].GetData(DataObject)) then
			if (CompatibleFormats[i].HasData) then begin
				if (Assign(CompatibleFormats[i])) then begin
...
begin
	...
	for i:=0 to CompatibleFormats.Count-1 do
		if (CompatibleFormats[i].HasValidFormats(ADataObject)) then begin
			Result:=True;
			break;
		end;
end;
...
begin
	...
	Result:=False;
	for i:=0 to CompatibleFormats.Count-1 do
		if (CompatibleFormats[i].AcceptFormat(FormatEtc)) then begin
			Result:=True;
			break;
		end;
end;
...
begin
	if (not DragDropShutdown) then begin
...
class procedure TCustomDataFormat.RegisterDataConversion(ClipboardFormatClass:TClipboardFormatClass;Priority:integer;
	Capabilities:TConversionCapabilities);
begin
	// Register format mapping.
	if (not DragDropShutdown) then
			TDataFormatMap.Instance.RegisterFormatMap(Self,ClipboardFormatClass,Priority,Capabilities);
end;
...
begin
	...
	if (Result=-1) then Result:=FList.Add(DataFormat);
end;
...
var
	i:integer;
begin
	for i:=FList.Count-1 downto 0 do Remove(TCustomDataFormat(FList[i]));
	FList.Free;
	inherited Destroy;
end;
...
begin
	...
	if (Result=-1) then Result:=Instance.FList.Add(DataFormat);
end;
...
var
	i:integer;
begin
	for i:=FList.Count-1 downto 0 do Remove(TDataFormatClass(FList[i]));
	FList.Free;
	inherited Destroy;
end;
...
begin
	if (FDataFormatClasses=nil) then begin
		if (DragDropShutdown) then OutputDebugString('DragDrop.TDataFormatClasses recreated during shutdown');
...
begin
	if Assigned(FDataFormatClasses) then Instance.FList.Remove(DataFormat);
end;
{$IF CompilerVersion < 17.0}
...
begin
	...
	for i:=FList.Count-1 downto 0 do Dispose(FList[i]);
	FList.Free;
	inherited Destroy;
end;
...
begin
	...
	i:=FList.Count-2;
	while (i>0) do begin
		if (PFormatMap(FList[i])^.DataFormat=NewMap^.DataFormat) then begin
			// Scan the list for a map with lower priority
			repeat
				if (PFormatMap(FList[i])^.Priority<NewMap^.Priority) then begin
...
procedure TDataFormatMap.Add(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass;
...
begin
	...
	if (ccReadFromDataFormat in Capabilities) then FormatMap^.Priority:=Priority;
	// ...and sort list
	Sort;
end;

function TDataFormatMap.CanMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass):boolean;
begin
	Result:=(FindMap(DataFormatClass,ClipboardFormatClass)<>-1);
end;

procedure TDataFormatMap.Delete(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass);
var
	Index:integer;
begin
...
begin
	...
	for i:=FList.Count-1 downto 0 do
		if (PFormatMap(FList[i])^.ClipboardFormat.InheritsFrom(ClipboardFormatClass)) then begin
			Dispose(FList[i]);
			FList.Delete(i);
		end;
end;
...
begin
	...
	for i:=FList.Count-1 downto 0 do
		if (PFormatMap(FList[i])^.DataFormat.InheritsFrom(DataFormatClass)) then begin
			Dispose(FList[i]);
			FList.Delete(i);
		end;
end;

function TDataFormatMap.FindMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass):integer;
var
	i:integer;
begin
	for i:=0 to FList.Count-1 do
		if (PFormatMap(FList[i])^.DataFormat.InheritsFrom(DataFormatClass))and
			(PFormatMap(FList[i])^.ClipboardFormat.InheritsFrom(ClipboardFormatClass)) then begin
			Result:=i;
			exit;
		end;
	Result:=-1;
...
begin
	...
	for i:=0 to Instance.FList.Count-1 do begin
		FormatMap:=PFormatMap(Instance.FList[i]);
		if (FormatMap.DataFormat.InheritsFrom(DataFormatClass))and
			(((Tier=ddtSource)and((ccReadFromDataFormat in FormatMap.Capabilities)or
			(ddWrite in FormatMap.ClipboardFormat.DataDirection)))or
			((Tier=ddtTarget)and((ccWriteToDataFormat in FormatMap.Capabilities)or
			(ddRead in FormatMap.ClipboardFormat.DataDirection)))) then begin
...
procedure TDataFormatMap.RegisterFormatMap(DataFormatClass:TDataFormatClass;ClipboardFormatClass:TClipboardFormatClass;
	Priority:integer;Capabilities:TConversionCapabilities);
begin
...
begin
	if (FDataFormatMap=nil) then begin
		if (DragDropShutdown) then OutputDebugString('DragDrop.TDataFormatMap recreated during shutdown');
...
begin
	if (csDesigning in ComponentState) then Result:=FDataFormatClassName
	else if Assigned(FDataFormatClass) then Result:=FDataFormatClass.ClassName
	else Result:='';
end;

function TDataFormatAdapter.GetEnabled:boolean;
begin
	if (csDesigning in ComponentState) then Result:=FEnabled
	else Result:=Assigned(FDataFormat)and Assigned(FDataFormatClass);
end;

procedure TDataFormatAdapter.Loaded;
begin
	inherited;
	if (FEnabled) then Enabled:=True;
end;

procedure TDataFormatAdapter.Notification(AComponent:TComponent;Operation:TOperation);
begin
	if (Operation=opRemove)and(AComponent=FDragDropComponent) then DragDropComponent:=nil;
	inherited;
end;

procedure TDataFormatAdapter.SetDataFormatClass(const Value:TDataFormatClass);
begin
	if (Value<>FDataFormatClass) then begin
		if not(csLoading in ComponentState) then Enabled:=False;
...
begin
	if (csDesigning in ComponentState) then FDataFormatClassName:=Value
	else begin
		ADataFormatClass:=nil;
		if (Value<>'') then begin
			for i:=0 to TDataFormatClasses.Count-1 do
				if (AnsiCompareText(TDataFormatClasses.Formats[i].ClassName,Value)=0) then begin
					ADataFormatClass:=TDataFormatClasses.Formats[i];
					break;
				end;
			if (ADataFormatClass=nil) then raise Exception.CreateFmt(sUnregisteredDataFormat,[Value]);
		end;
...
begin
	if (Value<>FDragDropComponent) then begin
		if not(csLoading in ComponentState) then Enabled:=False;
		if (FDragDropComponent<>nil) then FDragDropComponent.RemoveFreeNotification(Self);
...
begin
	if (csLoading in ComponentState) then begin
		...
		FEnabled:=Value and Assigned(FDragDropComponent)and(FDataFormatClassName<>'');
	end else if (Value) then begin
		if (Assigned(FDragDropComponent))and(Assigned(FDataFormatClass))and(not Assigned(FDataFormat)) then
				FDataFormat:=FDataFormatClass.Create(FDragDropComponent);
	end else begin
		if Assigned(FDataFormat) then begin
			if Assigned(FDragDropComponent)and(FDragDropComponent.DataFormats.IndexOf(FDataFormat)<>-1) then FDataFormat.Free;
			FDataFormat:=nil;
		end;
	end;
end;
...
begin
	if (Source is TRawDataFormat) then begin
		Result:=True;
	end
...
begin
	if (Dest is TRawDataFormat) then begin
		Result:=True;
	end
...
function TRawClipboardFormat.DoGetData(const ADataObject:IDataObject;const AMedium:TStgMedium):boolean;
begin
	...
	if (Result)and(FMedium.tymed=TYMED_ISTREAM) then IStream(FMedium.stm).Seek(0,STREAM_SEEK_SET,
{$IF CompilerVersion < 29}PLargeInt{$ELSE}PUInt64{$IFEND}(nil)^);
end;

function TRawClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
begin
...
begin
	with TAnsiStringClipboardFormat.Create do
		try
			...
			if GetDataFromMedium(nil,FMedium) then Result:=Data
			else Result:='';
		finally Free;
		end;
end;
...
begin
	Changing;
	for i:=0 to CompatibleFormats.Count-1 do TRawClipboardFormat(CompatibleFormats[i]).ClearData;
end;
...
begin
	i:=0;
	Result:=False;
	while (not Result)and(i<CompatibleFormats.Count) do begin
...
begin
	Result:=True;
	if ((DropEffect and DROPEFFECT_COPY)<>0) then DragType:=dtCopy
	else if ((DropEffect and DROPEFFECT_MOVE)<>0) then DragType:=dtMove
	else if ((DropEffect and DROPEFFECT_LINK)<>0) then DragType:=dtLink
	else begin
		DragType:=dtCopy;
		Result:=False;
	end;
...
begin
	...
	if (dtCopy in DragTypes) then Result:=Result OR DROPEFFECT_COPY;
	if (dtMove in DragTypes) then Result:=Result OR DROPEFFECT_MOVE;
	if (dtLink in DragTypes) then Result:=Result OR DROPEFFECT_LINK;
end;
...
begin
	...
	if (not PtInRect(DragRect,Position)) then begin
		GetClientRect(Handle,DragRect);
		// ... and inside client coordinates
		if (PtInRect(DragRect,Position)) then
			if (not ClientToScreen(Handle,Position)) then exit;
	end;
	// Check mouse state, and punt if the mouse isn't down.
{$IFNDEF DEBUG_DRAGDETECT}
	if ((GetKeyState(VK_LBUTTON)AND $8000)=0)and((GetKeyState(VK_RBUTTON)AND $8000)=0) then exit;
{$ENDIF DEBUG_DRAGDETECT}
	...
	try
		...
		while (not Result) do begin
			...
			if (PeekMessage(Msg,0,WM_LBUTTONDOWN,WM_LBUTTONUP,PM_NOREMOVE))or
				...
				(PeekMessage(Msg,0,WM_MBUTTONDOWN,WM_MBUTTONUP,PM_NOREMOVE)) then begin
			...
			while (not PeekMessage(Msg,0,WM_LBUTTONDOWN,WM_LBUTTONUP,PM_NOREMOVE))and
				...
				(not PeekMessage(Msg,0,0,0,PM_REMOVE or PM_QS_KEY or PM_QS_MOUSEMOVE))and(GetCapture=Handle) do begin
				// If there are no events for 500mS start drag without further ado.
				if (MsgWaitForMultipleObjects(0,nil^,False,DragDropDragDelayMax,QS_INPUT)=WAIT_TIMEOUT) then
					if (DragDropAutoDrag) then begin
						Result:=True;
						exit;
					end;
			end;
			// Bail out if someone else has captured the mouse.
			if (GetCapture<>Handle) then break;
			case (Msg.message) of
				// Mouse was moved.
				WM_MOUSEMOVE:
					...
					Result:=(not PtInRect(DragRect,Msg.pt))and(Msg.time>=StartTime+DWORD(DragDropDragDelay));
				// [Esc] cancels drag detection.
				WM_KEYDOWN: if (Msg.wParam=VK_ESCAPE) then break;
...
begin
	...
	if (Keys and MK_ALT<>0) then Include(Result,ssAlt);
end;
...
begin
	...
	if (Shift=[ssShift,ssCtrl]) then begin
		if (AllowedEffects AND DROPEFFECT_LINK<>0) then Result:=DROPEFFECT_LINK;
	end else if (Shift=[ssCtrl]) then begin
		if (AllowedEffects AND DROPEFFECT_COPY<>0) then Result:=DROPEFFECT_COPY;
	end else begin
		if (AllowedEffects AND DROPEFFECT_MOVE<>0) then Result:=DROPEFFECT_MOVE;
	end;
	// Fall back to defaults if the shift-states specified an
	// unavailable drop effect.
	if (Result=DROPEFFECT_NONE)and(Fallback) then begin
		if (AllowedEffects AND DROPEFFECT_COPY<>0) then Result:=DROPEFFECT_COPY
		else if (AllowedEffects AND DROPEFFECT_MOVE<>0) then Result:=DROPEFFECT_MOVE
		else if (AllowedEffects AND DROPEFFECT_LINK<>0) then Result:=DROPEFFECT_LINK;
	end;
end;

var
	URLMONDLL:THandle=0;
	_CopyStgMedium: function(const cstgmedSrc:TStgMedium;var stgmedDest:TStgMedium):HRESULT;
stdcall=nil;
...
begin
	...
	// OleDuplicateData.
	if (URLMONDLL=0) then begin
		URLMONDLL:=LoadLibrary('URLMON.DLL');
		if (URLMONDLL<>0) then @_CopyStgMedium:=GetProcAddress(URLMONDLL,'CopyStgMedium');
	end;
	if not Assigned(@_CopyStgMedium) then raise Exception.Create(sNoCopyStgMedium);
...
const
	sClipNames: array [CF_TEXT..CF_MAX-1] of string=('CF_TEXT','CF_BITMAP','CF_METAFILEPICT','CF_SYLK','CF_DIF','CF_TIFF',
		'CF_OEMTEXT','CF_DIB','CF_PALETTE','CF_PENDATA','CF_RIFF','CF_WAVE','CF_UNICODETEXT','CF_ENHMETAFILE','CF_HDROP',
		'CF_LOCALE'{$IF CompilerVersion >= 23.0},'CF_DIBV5'{$IFEND});
begin
	case Value of
...
{$IF CompilerVersion < 23.0}17:Result:='CF_DIBV5'; {$IFEND}
	...
	else Result:=GetNamedClipboardFormatName(Value);
	end;
end;
...
// Avoid GetClipboardFormatNameStr getting eliminated by linker. This is so we can use it in debugger
...
ShellMalloc:=nil;
if (URLMONDLL<>0) then FreeLibrary(URLMONDLL);
if (NeedOleUninitialize) then OleUninitialize;
