...
type
	...
	IChromeTab=interface
		...
		function GetImageIndex: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
		function GetImageIndexOverlay: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
		...
		function GetHControl:TWinCONTROL;
		procedure SetHControl(const Value:TWinCONTROL);
		property HControl:TWinCONTROL read GetHControl write SetHControl;
...
{$ENDIF}
	...
	private
		...
		FCaption:TCaption;
		FData:Pointer;
		FImageIndex: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
		FImageIndexOverlay: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
		FTag:Integer;
		FPinned:Boolean;
		FTabControl:TObject;
		FActive:Boolean;
		...
		procedure SetImageIndex(Value: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex);
		procedure SetTag(const Value:Integer);
		procedure SetPinned(const Value:Boolean);
		procedure SetImageIndexOverlay(const Value: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex);
	...
	published
		property HControl:TWinCONTROL read FControl write FControl default nil;
		...
		property CustomImagesSpinnerUpload:TCustomImageList read GetCustomImagesSpinnerUpload
			write SetCustomImagesSpinnerUpload;
		property CustomImagesSpinnerDownload:TCustomImageList read GetCustomImagesSpinnerDownload
			write SetCustomImagesSpinnerDownload;
		property ImageIndex:
{$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex read GetImageIndex write SetImageIndex;
		property ImageIndexOverlay:
{$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex read GetImageIndexOverlay write SetImageIndexOverlay;
	...
	published
		...
		property ContrainDraggedTabWithinContainer:Boolean read FContrainDraggedTabWithinContainer
			write SetContrainDraggedTabWithinContainer;
	end;
	...
	published
		property DefaultMovementAnimationTimeMS:Cardinal read FDefaultMovementAnimationTimeMS
			write SetMovementAnimationTimeMS;
	...
	public
		constructor Create(AOwner:TPersistent);override;
	published
		property BackgroundDblClickMaximiseRestoreForm:Boolean read FBackgroundDblClickMaximiseRestoreForm
			write SetBackgroundDblClickMaximiseRestoreForm;
		...
		property TabSmartDeleteResizeCancelDelay:Integer read FTabSmartDeleteResizeCancelDelay
			write SetTabSmartDeleteResizeCancelDelay;
		...
		property IgnoreDoubleClicksWhileAnimatingMovement:Boolean read FIgnoreDoubleClicksWhileAnimatingMovement
			write SetIgnoreDoubleClicksWhileAnimatingMovement;
	end;
	...
	published
		...
		property HideImagesWhenSpinnerVisible:Boolean read FHideImagesWhenSpinnerVisible
			write SetHideImagesWhenSpinnerVisible;
	end;
...
begin
	...
	if FTabControl<>nil then FTabControl.Free;
end;
...
var
	LastPinned:Integer;
begin
	if FPinned<>Value then begin
		LastPinned:=GetChromeTabInterface.GetLastPinnedIndex;

		if Value then Index:=LastPinned+1
		else Index:=LastPinned;
...
begin
	if Value<>FSpinnerState then begin
		FSpinnerState:=Value;
	...
	if FActive<>Value then begin
		...
		if Allow then begin
			...
			if FActive then begin
				with (GetOwner as TChromeTabsList) do begin
					for i:=0 to Count-1 do begin
						// Only one can be selected
						if (Items[i]<>Self)and(Items[i].Active) then begin
...
begin
	...
	if (not Value)and(Active) then Active:=FALSE;
...
function TChromeTab.GetHControl:TWinCONTROL;
begin
	Result:=FControl;
end;

procedure TChromeTab.SetHControl(const Value:TWinCONTROL);
begin
	FControl:=Value;
end;
...
function TChromeTab.GetImageIndex: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
begin
	Result:=FImageIndex;
end;

function TChromeTab.GetImageIndexOverlay: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex;
begin
...
begin
	if FCollection<>nil then Supports(FCollection.Owner,IChromeTabs,Result)
	else Result:=nil;
end;
...
begin
	if (GetChromeTabInterface<>nil)and(csDesigning in GetChromeTabInterface.GetComponentState) then begin
		OldCollection:=Collection as TOwnedCollection;

		inherited;

		if (OldCollection<>Value)and(Assigned(OldCollection)) then DoChanged(tcPropertyUpdated);
	end
	else inherited;
end;
...
begin
	if FHideCloseButton<>Value then begin
...
function TChromeTab.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const
{$ENDIF} iid:tguid;out obj):longint;{$IFNDEF WINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
{$ELSE}
...
procedure TChromeTab.SetImageIndex(Value: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex);
begin
...
procedure TChromeTab.SetImageIndexOverlay(const Value: {$IFDEF UNIT_SCOPE_NAMES}System.UITypes.{$ENDIF}TImageIndex);
begin
	if FImageIndexOverlay<>Value then begin
...
begin
	...
	if Items[Index].FMarkedForDeletion then begin
		inherited Delete(Index);
	end else begin
		if not Items[Index].Active then NewIdx:=-1
		else

			if Index<pred(Count) then begin
			NewIdx:=Index;

			if (GetChromeTabInterface.GetOptions.Animation.GetMovementAnimationEaseType
				(GetChromeTabInterface.GetOptions.Animation.MovementAnimations.TabDelete)<>ttNone)and(not DeleteNow) then begin
				if NewIdx<pred(Count) then Inc(NewIdx)
				else Dec(NewIdx);
			end;
		end else if Index>0 then NewIdx:=Index-1
		else NewIdx:=-1;

		GetChromeTabInterface.DoOnChange(Items[Index],tcDeleting);

		if (not DeleteNow)and(GetChromeTabInterface.GetOptions.Animation.GetMovementAnimationEaseType
			(GetChromeTabInterface.GetOptions.Animation.MovementAnimations.TabDelete)<>ttNone) then begin
		...
		if NewIdx>Count-1 then NewIdx:=Count-1;

		while ((NewIdx<>-1))and(not(Items[NewIdx].Visible)) do Dec(NewIdx);

		if NewIdx<>-1 then Items[NewIdx].Active:=TRUE;
	end;
end;
...
begin
	for i:=0 to pred(Count) do
		if (Items[i]=Value)and(Value<>nil) then begin
			Items[i].Active:=TRUE;
			Break;
		end else if (Value=nil) then Items[i].Active:=FALSE;
end;
...
procedure TChromeTabsList.Notify(Item:TCollectionItem;Action:TCollectionNotification);
begin
	inherited;
...
{$IFDEF UNIT_SCOPE_NAMES}TCollectionNotification.{$ENDIF}cnAdded:
...
begin
	if Value<FMinWidth then FMaxWidth:=FMinWidth
...
procedure TChromeTabsOptions.SetTextAlignmentVertical(const Value:TVerticalAlignment);
begin
...
begin
	...
	while Owner<>nil do begin
		if Supports(Owner,IChromeTabs,ChromeTabInterface) then begin
			ChromeTabInterface.DoOnChange(nil,tcPropertyUpdated);

			Break;
		end else if Owner is TChromeTabsPersistent then Owner:=TChromeTabsPersistent(Owner).Owner
		else if Owner is TComponent then Owner:=TChromeTabsPersistent(Owner).Owner
		else Owner:=nil;
	end;
end;
...
begin
	if FOwner is TPersistent then Result:=TPersistent(FOwner)
	else Result:=nil;
end;
...
function TChromeTabsLookAndFeelStyle.GetBrush(ClientRect:TRect):TGPLinearGradientBrush;
begin
	// Make sure thst the last brush wasn't broken
	if (FInvalidated)or((FBrush<>nil)and(FBrush.GetLastStatus<>Ok)) then begin
		FreeAndNil(FBrush);
	...
	if FBrush=nil then
			FBrush:=TGPLinearGradientBrush.Create(MakePoint(0,ClientRect.Top),MakePoint(0,ClientRect.Bottom),
			MakeGDIPColor(StartColor,StartAlpha),MakeGDIPColor(StopColor,StopAlpha));
...
begin
	...
	if FPen=nil then FPen:=TGPPen.Create(MakeGDIPColor(FOutlineColor,FOutlineAlpha),FOutlineSize);
...
procedure TChromeTabsLookAndFeelCloseButton.SetCross(const Value:TChromeTabsLookAndFeelPenItem);
begin
	FCross:=Value;
...
procedure TChromeTabsLookAndFeelAddButton.SetButton(const Value:TChromeTabsLookAndFeelButtonItems);
begin
	FButton.Assign(Value);
end;

procedure TChromeTabsLookAndFeelAddButton.SetPlusSign(const Value:TChromeTabsLookAndFeelButtonItems);
begin
...
procedure TChromeTabsLookAndFeel.SetScrollButtons(const Value:TChromeTabsLookAndFeelScrollButton);
begin
...
procedure TChromeTabsLookAndFeel.SetTabs(const Value:TChromeTabsLookAndFeelTab);
begin
	FTabs.Assign(Value);
...
procedure TChromeTabsLookAndFeel.SetAddButton(const Value:TChromeTabsLookAndFeelAddButton);
begin
...
procedure TChromeTabsAnimationOptions.SetMovementEaseType(const Value:TChromeTabsEaseType);
begin
	FDefaultMovementEaseType:=Value;
end;

procedure TChromeTabsAnimationOptions.SetMovementEasing(const Value:TChromeTabsMovementAnimations);
begin
	FMovementEasing.Assign(Value);
end;

procedure TChromeTabsAnimationOptions.SetStyleEaseType(const Value:TChromeTabsEaseType);
begin
	FDefaultStyleEaseType:=Value;
end;

procedure TChromeTabsAnimationOptions.SetMinimumTabAnimationWidth(const Value:Integer);
begin
...
function TChromeTabsAnimationOptions.GetMovementAnimationEaseType(AnimationTypes:TChromeTabsMovementAnimationTypes)
	:TChromeTabsEaseType;
begin
	if AnimationTypes.UseDefaultEaseType then Result:=FDefaultMovementEaseType
	else Result:=AnimationTypes.EaseType;
end;

function TChromeTabsAnimationOptions.GetMovementAnimationTime
	(AnimationTypes:TChromeTabsMovementAnimationTypes):Cardinal;
begin
	if AnimationTypes.UseDefaultEaseType then Result:=FDefaultMovementAnimationTimeMS
	else Result:=AnimationTypes.FAnimationTimeMS;
end;
...
procedure TChromeTabsDragOptions.SetDragType(const Value:TChromeTabDragType);
begin
	FDragType:=Value;
end;

procedure TChromeTabsDragOptions.SetContrainDraggedTabWithinContainer(const Value:Boolean);
begin
	FContrainDraggedTabWithinContainer:=Value;
end;

procedure TChromeTabsDragOptions.SetDragControlImageResizeFactor(const Value:Real);
begin
...
procedure TChromeTabsDragOptions.SetDragDisplay(const Value:TChromeTabDragDisplay);
begin
	FDragDisplay:=Value;
end;
...
begin
	if FDragForm<>nil then FDragForm.Release;
...
procedure TChromeTabsDisplayOptions.SetAddButton(const Value:TChromeTabsAddButtonProperties);
begin
	FAddButton.Assign(Value);
...
procedure TChromeTabsDisplayOptions.SetScrollButtonLeft(const Value:TChromeTabsControlPosition);
begin
	FScrollButtonLeft.Assign(Value);
end;

procedure TChromeTabsDisplayOptions.SetScrollButtonRight(const Value:TChromeTabsControlPosition);
begin
	FScrollButtonRight.Assign(Value);
end;

procedure TChromeTabsDisplayOptions.SetCloseButton(const Value:TChromeTabsCloseButtonProperties);
begin
	FCloseButton.Assign(Value);
end;

procedure TChromeTabsDisplayOptions.SetTabModifiedGlow(const Value:TChromeTabModifiedDisplayOptions);
begin
...
procedure TChromeTabsDisplayOptions.SetTabMouseGlow(const Value:TChromeTabsControlPropertiesEx);
begin
...
procedure TChromeTabsDisplayOptions.SetTabContainer(const Value:TChromeTabsContainerOptions);
begin
...
procedure TChromeTabsDisplayOptions.SetTabSpinners(const Value:TChromeTabsSpinnersOptions);
begin
...
procedure TChromeTabsBehaviourOptions.SetIgnoreDoubleClicksWhileAnimatingMovement(const Value:Boolean);
begin
	FIgnoreDoubleClicksWhileAnimatingMovement:=Value;
end;

procedure TChromeTabsBehaviourOptions.SetTabRightClickSelect(const Value:Boolean);
begin
	FTabRightClickSelect:=Value;
end;

procedure TChromeTabsBehaviourOptions.SetTabSmartDeleteResizeCancelDelay(const Value:Integer);
begin
	FTabSmartDeleteResizeCancelDelay:=Value;
end;

procedure TChromeTabsBehaviourOptions.SetTabSmartDeleteResizing(const Value:Boolean);
begin
	FTabSmartDeleteResizing:=Value;
end;

procedure TChromeTabsBehaviourOptions.SetBackgroundDblClickMaximiseRestoreForm(const Value:Boolean);
begin
	FBackgroundDblClickMaximiseRestoreForm:=Value;
end;

procedure TChromeTabsBehaviourOptions.SetBackgroundDragMovesForm(const Value:Boolean);
begin
...
procedure TChromeTabsBehaviourOptions.SetUseBuiltInPopupMenu(const Value:Boolean);
begin
...
procedure TChromeTabsScrollOptions.SetScrollButtons(const Value:TChromeTabScrollButtons);
begin
...
procedure TChromeTabsCloseButtonProperties.SetAutoHideWidth(const Value:Integer);
begin
...
procedure TChromeTabsCloseButtonProperties.SetCrossRadialOffset(const Value:Integer);
begin
...
procedure TChromeTabsCloseButtonProperties.SetVisibility(const Value:TCloseButtonVisiblity);
begin
	FVisibility:=Value;
...
procedure TChromeTabsAddButtonProperties.SetHorizontalOffsetFloating(const Value:Integer);
begin
...
procedure TChromeTabsAddButtonProperties.SetShowPlusSign(const Value:Boolean);
begin
	FShowPlusSign:=Value;
...
procedure TChromeTabsAddButtonProperties.SetVisibility(const Value:TAddButtonVisibility);
begin
	FVisibility:=Value;
...
procedure TChromeTabsLookAndFeelCustomStyleItems.SetStyleProperty2(const Value:TChromeTabsLookAndFeelStyle);
begin
...
procedure TChromeTabsLookAndFeelCustomStyleItems.SetStyleProperty3(const Value:TChromeTabsLookAndFeelStyle);
begin
...
procedure TChromeTabsLookAndFeelCustomStyleItems.SetStyleProperty1(const Value:TChromeTabsLookAndFeelStyle);
begin
...
begin
	...
	if FPen=nil then FPen:=TGPPen.Create(MakeGDIPColor(FColor,FAlpha),FThickness);
...
procedure TChromeTabsCustomLookAndFeelPenItem.SetPen1(const Value:TChromeTabsLookAndFeelPen);
begin
	FPen1.Assign(Value);
...
procedure TChromeTabsCustomLookAndFeelPenItem.SetPen2(const Value:TChromeTabsLookAndFeelPen);
begin
	FPen2.Assign(Value);
...
procedure TChromeTabsCustomLookAndFeelPenItem.SetPen3(const Value:TChromeTabsLookAndFeelPen);
begin
	FPen3.Assign(Value);
...
constructor TChromeTabsLookAndFeelDisabledButton.Create(AOwner:TPersistent);
begin
	inherited;
...
procedure TChromeTabsLookAndFeelDisabledButton.SetDisabled(const Value:TChromeTabsLookAndFeelStyle);
begin
...
procedure TChromeTabsLookAndFeelTab.SetBaseLine(const Value:TChromeTabsLookAndFeelPen);
begin
...
procedure TChromeTabsLookAndFeelTab.SetDefaultFont(const Value:TChromeTabsLookAndFeelBaseFont);
begin
...
procedure TChromeTabsLookAndFeelTab.SetModified(const Value:TChromeTabsLookAndFeelGlow);
begin
...
procedure TChromeTabsLookAndFeelTab.SetMouseGlow(const Value:TChromeTabsLookAndFeelGlow);
begin
...
procedure TChromeTabsLookAndFeelTab.SetSpinners(const Value:TChromeTabsLookAndFeelSpinners);
begin
...
function TChromeTabPolygons.AddPolygon(Polygon:TPolygon;Brush:TGPBrush;Pen:TGPPen):IChromeTabPolygons;
var
...
var
	i:Integer;
begin
	for i:=0 to pred(FPolygons.Count) do begin
...
procedure TChromeTabsLookAndFeelScrollButton.SetArrow(const Value:TChromeTabsLookAndFeelDisabledButton);
begin
	FArrow.Assign(Value);
...
procedure TChromeTabsLookAndFeelScrollButton.SetButton(const Value:TChromeTabsLookAndFeelDisabledButton);
begin
	FButton.Assign(Value);
...
procedure TChromeTabModifiedDisplayOptions.SetAnimationUpdateMS(const Value:Integer);
begin
...
procedure TChromeTabModifiedDisplayOptions.SetEaseType(const Value:TChromeTabsEaseType);
begin
	FEaseType:=Value;
...
procedure TChromeTabModifiedDisplayOptions.SetStyle(const Value:TChromeTabModifiedStyle);
begin
	FStyle:=Value;
...
procedure TChromeTabsContainerOptions.SetTransparentBackground(const Value:Boolean);
begin
...
procedure TChromeTabsMovementAnimations.SetTabAdd(const Value:TChromeTabsMovementAnimationTypes);
begin
...
procedure TChromeTabsMovementAnimations.SetTabDelete(const Value:TChromeTabsMovementAnimationTypes);
begin
...
procedure TChromeTabsMovementAnimations.SetTabMove(const Value:TChromeTabsMovementAnimationTypes);
begin
...
procedure TChromeTabsMovementAnimationTypes.SetAnimationTimeMS(const Value:Integer);
begin
	FAnimationTimeMS:=Value;
end;

procedure TChromeTabsMovementAnimationTypes.SetEaseType(const Value:TChromeTabsEaseType);
begin
	FEaseType:=Value;
end;

procedure TChromeTabsMovementAnimationTypes.SetUseDefaultAnimationTime(const Value:Boolean);
begin
	FUseDefaultAnimationTime:=Value;
end;

procedure TChromeTabsMovementAnimationTypes.SetUseDefaultEaseType(const Value:Boolean);
begin
...
procedure TChromeTabsSpinnersOptions.SetAnimationUpdateMS(const Value:Cardinal);
begin
	FAnimationUpdateMS:=Value;
end;

procedure TChromeTabsSpinnersOptions.SetDownload(const Value:TChromeTabsSpinnerOptions);
begin
	FDownload.Assign(Value);
end;

procedure TChromeTabsSpinnersOptions.SetHideImagesWhenSpinnerVisible(const Value:Boolean);
begin
...
procedure TChromeTabsSpinnersOptions.SetUpload(const Value:TChromeTabsSpinnerOptions);
begin
...
procedure TChromeTabsSpinnerOptions.SetPosition(const Value:TChromeTabsControlPosition);
begin
	FPosition.Assign(Value);
end;

procedure TChromeTabsSpinnerOptions.SetRenderedAnimationStep(const Value:Integer);
begin
...
procedure TChromeTabsLookAndFeelSpinners.SetDownload(const Value:TChromeTabsLookAndFeelPen);
begin
	FDownload.Assign(Value);
...
procedure TChromeTabsLookAndFeelSpinners.SetUpload(const Value:TChromeTabsLookAndFeelPen);
begin
	FUpload.Assign(Value);
