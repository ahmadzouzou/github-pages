...
type
	...
	public
		...
		property ChromeTabsMaximizedRightOffset:Integer read FChromeTabsMaximizedRightOffset
			write SetChromeTabsMaximizedRightOffset;
		property ChromeTabsWindowedRightOffset:Integer read FChromeTabsWindowedRightOffset
			write SetChromeTabsWindowedRightOffset;
...
begin
	if Win32MajorVersion>=6 then begin
		if DwmGetWindowAttribute(Form.Handle,DWMWA_CAPTION_BUTTON_BOUNDS,@Result,SizeOf(Result))<>S_OK then
				SetRectEmpty(Result);
	end
...
var
	...
	begin
		...
		if MakeDefaultIfEnabled and Enable then SetMenuDefaultItem(Menu,ID,MF_BYCOMMAND);
	end;

begin
	Menu:=GetSystemMenu(Form.Handle,FALSE);

	if Form.BorderStyle in [bsSingle,bsSizeable,bsToolWindow,bsSizeToolWin] then begin
		...
		UpdateItem(SC_SIZE,(Form.WindowState<>wsMaximized)and(Form.BorderStyle in [bsSizeable,bsSizeToolWin]));

		UpdateItem(SC_MINIMIZE,(biMinimize in Form.BorderIcons)and(Form.BorderStyle in [bsSingle,bsSizeable]));

		UpdateItem(SC_MAXIMIZE,(biMaximize in Form.BorderIcons)and(Form.BorderStyle in [bsSingle,bsSizeable])and
			(Form.WindowState<>wsMaximized),TRUE);
	end;

	if Message.HitTest=HTSYSMENU then SetMenuDefaultItem(Menu,SC_CLOSE,MF_BYCOMMAND);

	Cmd:=WPARAM(TrackPopupMenu(Menu,TPM_RETURNCMD or GetSystemMetrics(SM_MENUDROPALIGNMENT),Message.XCursor,
...
begin
	...
	if ShowTabsInTitleBar then Inc(Rect.Top,GlassFrame.Top);
end;
...
begin
	...
	begin
		...
		with GlassFrame do
				ExcludeClipRect(DC,GlassFrame.Left,GlassFrame.Top,R.Right-GlassFrame.Right,R.Bottom-GlassFrame.Bottom);
	end;
...
begin
	...
	if UpdateFrame then SetWindowPos(Handle,0,Left,Top,Width,Height,SWP_FRAMECHANGED);
end;

procedure TChromeTabsGlassForm.Resize;
begin
	if ShowTabsInTitleBar then begin
...
begin
	if ShowTabsInTitleBar then begin
		if WindowState=wsMaximized then Inc(Message.CalcSize_Params.rgrc[0].Left,FWndFrameSize+1)
		else Inc(Message.CalcSize_Params.rgrc[0].Left,FWndFrameSize);
...
begin
	...
	if ShowTabsInTitleBar then begin
		if Message.Result in [HTMINBUTTON,HTMAXBUTTON,HTCLOSE] then begin
			...
			if ClientPos.Y<=GlassFrame.Top then begin
				if (ClientPos.X<IconRect.Right)and
					((WindowState=wsMaximized)or((ClientPos.Y>=IconRect.Top)and(ClientPos.Y<IconRect.Bottom))) then
						Message.Result:=HTSYSMENU
				else if ClientPos.Y<FWndFrameSize then begin
					if WindowState<>wsMaximized then begin
						if ClientPos.X<FWndFrameSize then Message.Result:=HTTOPLEFT
						else if ClientPos.X>ClientWidth then Message.Result:=HTTOPRIGHT
						else Message.Result:=HTTOP
					end
					else Message.Result:=HTCAPTION
				end else begin
					if WindowState<>wsMaximized then begin
						if ClientPos.X<FWndFrameSize then Message.Result:=HTLEFT
						else if ClientPos.X>ClientWidth then Message.Result:=HTRIGHT
						else Message.Result:=HTCAPTION
					end
					else Message.Result:=HTCAPTION
				end;
...
begin
	if (not ShowTabsInTitleBar)or(not(biSystemMenu in BorderIcons)) then inherited
	else begin
		if Message.HitTest in [HTCAPTION,HTSYSMENU] then ShowSystemMenu(Self,Message)
		else inherited;
	end;
end;
...
begin
	...
	if AeroWasEnabled then DisableTitleTabs;

	if FAeroEnabled then EnableTitleTabs;

	if AeroWasEnabled<>FAeroEnabled then DoOnAeroStatusChanged(FAeroEnabled);
...
begin
	if (FChromeTabs<>nil)and(FTabsInTitleBar) then begin
...
begin
	if Assigned(FOnAeroStatusChanged) then FOnAeroStatusChanged(Self,AeroEnabled);
end;
...
begin
	if not(biSystemMenu in BorderIcons)or not(BorderStyle in [bsSingle,bsSizeable]) then SetRectEmpty(Result)
	else begin
		Result.Left:=0;
		Result.Right:=GetSystemMetrics(SM_CXSMICON);
		Result.Bottom:=GetSystemMetrics(SM_CYSMICON);
		if WindowState=wsMaximized then Result.Top:=GlassFrame.Top-Result.Bottom-2
		else Result.Top:=6;
...
begin
	if ShowTabsInTitleBar then begin
		if Message.WindowPos.Flags and SWP_STATECHANGED=SWP_STATECHANGED then Invalidate;
	end;

	inherited;

	if (ShowTabsInTitleBar)and(Message.WindowPos.Flags and SWP_FRAMECHANGED<>0)and
		(Message.WindowPos.Flags<>SWP_FRAMECHANGED) then RecalcGlassFrameBounds(FALSE);
end;
...
begin
	...
	if (ShowTabsInTitleBar)and(Message.WindowPos.Flags and SWP_FRAMECHANGED<>0) then Realign;
end;

procedure TChromeTabsGlassForm.WndProc(var Message:TMessage);
begin
	if (not ShowTabsInTitleBar)or(not HandleAllocated)or
		(not DwmDefWindowProc(Handle,Message.Msg,Message.WPARAM,Message.LParam,Message.Result)) then inherited;
end;

procedure TChromeTabsGlassForm.SetChromeTabs(const Value:TChromeTabs);
begin
	if (not(csDestroying in ComponentState))and(Value<>FChromeTabs) then begin
		if FChromeTabs<>nil then DisableTitleTabs;

		FChromeTabs:=Value;

		if Value<>nil then EnableTitleTabs
		else begin
			UpdateChromeTabPosition;
...
procedure TChromeTabsGlassForm.SetChromeTabsMaximizedRightOffset(const Value:Integer);
begin
...
procedure TChromeTabsGlassForm.SetChromeTabsMaxmizedTopOffset(const Value:Integer);
begin
...
procedure TChromeTabsGlassForm.SetChromeTabsWindowedRightOffset(const Value:Integer);
begin
...
procedure TChromeTabsGlassForm.SetChromeTabsWindowedTopOffset(const Value:Integer);
begin
...
begin
	Result:=(FChromeTabs<>nil)and(FAeroEnabled);
end;
...
procedure TChromeTabsGlassForm.Notification(AComponent:TComponent;Operation:TOperation);
begin
	inherited Notification(AComponent,Operation);

	if Operation=opRemove then begin
		if AComponent=FChromeTabs then SetChromeTabs(nil);
	end;
end;
