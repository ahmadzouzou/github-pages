...
type
	...
	TDragResult=(drDropCopy,drDropMove,drDropLink,drCancel,drOutMemory,drAsync,drUnknown);

	TDropEvent=procedure(Sender:TObject;DragType:TDragType;var ContinueDrop:Boolean) of object;

	TDResult=record helper for TDragResult
	...
	TAfterDropEvent=procedure(Sender:TObject;DragResult:TDragResult;Optimized:Boolean) of object;

	TFeedbackEvent=procedure(Sender:TObject;Effect:LongInt;var UseDefaultCursors:Boolean) of object;
	...
	TDropDataEvent=procedure(Sender:TObject;const FormatEtc:TFormatEtc;out Medium:TStgMedium;var Handled:Boolean)
		of object;
	...
	TPasteEvent=procedure(Sender:TObject;Action:TDragResult;DeleteOnPaste:Boolean) of object;

	// : TGetDragImageEvent is fired when the drop source initializes the drag
	// image.
	TGetDragImageEvent=procedure(Sender:TObject;const DragSourceHelper:IDragSourceHelper2;var Handled:Boolean) of object;
	...
	/// /////////////////////////////////////////////////////////////////////////////
	TCustomDropSource=class(TDragDropComponent,IDropSource,IDataObject,IDataObjectAsyncCapability)
	private
		...
		FDragSourceHelper:IDragSourceHelper2;
		// Async transfer...
		FAllowAsync:Boolean;
	...
	protected
		...
		function dAdvise(const FormatEtc:TFormatEtc;advf:LongInt;const advsink:IAdviseSink;out dwConnection:LongInt)
			:HRESULT;stdcall;
		...
		property DragSourceHelper:IDragSourceHelper2 read FDragSourceHelper;
	public
...
{$ENDIF}
		...
		property PreferredDropEffect:LongInt read GetPreferredDropEffect write SetPreferredDropEffect;
		property PerformedDropEffect:LongInt read GetPerformedDropEffect write SetPerformedDropEffect;
		property LogicalPerformedDropEffect:LongInt read GetLogicalPerformedDropEffect;
		property InShellDragLoop:Boolean read GetInShellDragLoop write SetInShellDragLoop;
	...
	published
		...
		property Sourcehandle:TWinControl read FSourcehandle write FSourcehandle;
	end;
	...
	protected
		...
		procedure DoOnSetData(DataFormat:TCustomDataFormat;ClipboardFormat:TClipboardFormat);
...
function TDResult.ToString:string;
begin
	case Self of
		drDropCopy:Result:='drDropCopy';
		drDropMove:Result:='drDropMove';
		drDropLink:Result:='drDropLink';
		drCancel:Result:='drCancel';
		drOutMemory:Result:='drOutMemory';
		drAsync:Result:='drAsync';
		drUnknown:Result:='drUnknown';
	else Result:='';
	end;
end;
...
begin
	case DropEffect of
		DROPEFFECT_NONE:Result:=drCancel;
		DROPEFFECT_COPY:Result:=drDropCopy;
		DROPEFFECT_MOVE:Result:=drDropMove;
		DROPEFFECT_LINK:Result:=drDropLink;
	else Result:=drUnknown; // This is probably an error condition
	end;
end;
...
var
	...
	DAD_AutoScroll: function(hWindow:hWnd;var lpSamples:TScrollSamples;var lppt:TPoint):DWORD;stdcall;
...
const
	Shell32='Shell32.dll';
begin
	if (Shell32Handle=THandle(-1)) then begin
		Shell32Handle:=SafeLoadLibrary(Shell32);
		if (Shell32Handle>HINSTANCE_ERROR) then begin
			DAD_AutoScroll:=GetProcAddress(Shell32Handle,PAnsiChar(129));
			if (not Assigned(DAD_AutoScroll)) then begin
				Result:=False;
	...
	else Result:=(Shell32Handle>HINSTANCE_ERROR);
end;
{$ENDIF}
...
begin
	...
	OleCheck(CoMarshalInterThreadInterfaceInStream(IDataObject,FDropSource,IStream(FDataObjectStream)));
	...
	OleCheck(CoMarshalInterThreadInterfaceInStream(IUnknown,FDropSource,IStream(FDropSourceStream)));
end;

destructor TDropSourceThread.Destroy;
begin
	if (FDataObjectStream<>nil) then IUnknown(FDataObjectStream)._Release;
	if (FDropSourceStream<>nil) then IUnknown(FDropSourceStream)._Release;
...
begin
	...
	*)
	...
	try
		...
		if (hwndAttach<>0) then AttachThreadID:=GetWindowThreadProcessId(hwndAttach,nil)
		else
		...
		if (AttachThreadID<>CurrentThreadID) then AttachThreadInput(AttachThreadID,CurrentThreadID,True);
		try
			...
			try
				...
				try
					OleCheck(CoGetInterfaceAndReleaseStream(IStream(FDataObjectStream),IDataObject,FDropSource.FAsyncDataObject));
					FDataObjectStream:=nil;
					OleCheck(CoGetInterfaceAndReleaseStream(IStream(FDropSourceStream),IDataObjectAsyncCapability,
						FDropSource.FAsyncDropSource));
				...
				while (FDropSource.AsyncTransfer)and(not Terminated) do begin
			...
			if (AttachThreadID<>CurrentThreadID) then AttachThreadInput(AttachThreadID,CurrentThreadID,False);
...
begin
	FlushClipboard;
	if (FAsyncTargetEvent<>0) then CloseHandle(FAsyncTargetEvent);
	inherited Destroy;
end;

// -----------------------------------------------------------------------------

function TCustomDropSource.GetCanonicalFormatEtc(const FormatEtc:TFormatEtc;out FormatEtcout:TFormatEtc):HRESULT;
begin
...
function TCustomDropSource.SetData(const FormatEtc:TFormatEtc;var Medium:TStgMedium;fRelease:bool):HRESULT;
begin
	...
	// begin
	try Result:=DoSetData(FormatEtc,Medium);
	finally
...
function TCustomDropSource.dAdvise(const FormatEtc:TFormatEtc;advf:LongInt;const advsink:IAdviseSink;
...
function TCustomDropSource.GetData(const FormatEtcIn:TFormatEtc;out Medium:TStgMedium):HRESULT;stdcall;
var
	Handled:Boolean;
begin
	Handled:=False;
...
function TCustomDropSource.GetDataHere(const FormatEtc:TFormatEtc;out Medium:TStgMedium):HRESULT;stdcall;
begin
	Result:=E_NOTIMPL;
end;
...
begin
	Result:=False;
	if (Assigned(FOnGetDragImage)) then FOnGetDragImage(Self,DragSourceHelper,Result);

	if (not Result) then Result:=GetDragImageFromImageList;
end;
...
{$IFEND}
			ImageHotSpotX:=Images.Width DIV 2;
			ImageHotSpotY:=Images.Height-16;
			// TImageList.GetBitmap uses TImageList.Draw to extract the bitmap so we
			// must clear the destination bitmap before extraction.
			if (FImages.BkColor<>clNone) then DragBitmap.Canvas.Brush.Color:=Images.BkColor;
		...
		if (Succeeded(DragSourceHelper.InitializeFromBitmap(@shDragImage,Self))) then begin
			...
			SHDoDragDrop(Sourcehandle.Handle,Self,nil,Dropimage_copy,GG);
			Result:=True;
		end
		else DeleteObject(shDragImage.hbmpDragImage);
	end;
end;

function TCustomDropSource.QueryGetData(const FormatEtc:TFormatEtc):HRESULT;stdcall;
begin
	if (HasFormat(FormatEtc)) then Result:=S_OK
	else Result:=DV_E_FORMATETC;
end;

function TCustomDropSource.EnumFormatEtc(dwDirection:LongInt;out EnumFormatEtc:IEnumFormatEtc):HRESULT;stdcall;
begin
	EnumFormatEtc:=GetEnumFormatEtc(dwDirection);
	if (EnumFormatEtc<>nil) then Result:=S_OK
	else Result:=E_NOTIMPL;
end;

// Implements IDropSource.QueryContinueDrag
function TCustomDropSource.QueryContinueDrag(fEscapePressed:bool;grfKeyState:LongInt):HRESULT;stdcall;
var
...
begin
	...
	else if (grfKeyState and(MK_LBUTTON or MK_RBUTTON)=0) then begin
		ContinueDrop:=DropEffectToDragType(FeedbackEffect,DragType)and(DragType in DragTypes);
...
begin
	Result:=E_NOTIMPL;
	if (Assigned(FOnSetData)) then begin
		Handled:=False;
...
begin
	if (FAllowAsync<>Value) then begin
		FAllowAsync:=Value;
		if (not FAllowAsync) then begin
			FRequestAsync:=False;
...
function TCustomDropSource.GetAsyncMode(var pfIsOpAsync:bool):HRESULT;
begin
...
begin
	if (FAllowAsync) then begin
		Result:=S_OK;
		if (FRequestAsync<>fDoOpAsync) then begin
...
function TCustomDropSource.InOperation(var pfInAsyncOp:bool):HRESULT;
begin
	pfInAsyncOp:=AsyncTransfer;
	Result:=S_OK;
end;

function TCustomDropSource.StartOperation(pbcReserved:IBindCtx):HRESULT;
begin
	if (FRequestAsync) then begin
		if (FAsyncTargetEvent<>0) then begin
...
function TCustomDropSource.EndOperation(HRESULT:HRESULT;pbcReserved:IBindCtx;dwEffects:DWORD):HRESULT;
var
	DropResult:TDragResult;
begin
	if (AsyncTransfer) then begin
		...
		FAsyncTargetTransfer:=False;
		if (Assigned(FOnAfterDrop)) then begin
			if (Succeeded(HRESULT)) then DropResult:=DropEffectToDragResult(dwEffects and DragTypesToDropEffect(FDragTypes))
			else DropResult:=drUnknown;
			...
			FOnAfterDrop(Self,DropResult,(DropResult<>drDropMove)or(PerformedDropEffect<>DROPEFFECT_MOVE));
		end;
		Result:=S_OK;
	end
	else Result:=E_FAIL;
...
var
	DropResult:HRESULT;
	AllowedEffects,DropEffect:LongInt;
...
begin
	...
	if (FShowImage) then begin
		...
		if (Succeeded(CoCreateInstance(CLSID_DragDropHelper,nil,CLSCTX_INPROC_SERVER,IDragSourceHelper2,FDragSourceHelper)))
			and(FDragSourceHelper<>nil) then begin
			// Create a drag image.
			IsDraggingImage:=GetDragImage;
		end
		else IsDraggingImage:=False;

		if (not IsDraggingImage) then FDragSourceHelper:=nil;
		...
		if (FDragSourceHelper=nil)and(FImages<>nil) then begin
...
{$IFDEF SHELL_DRAGIMAGE}
			...
			if (InitShellDragImage) then begin
				if DAD_SetDragImage(FImages.Handle,FImageHotSpot) then DAD_ShowDragImage(True);
			end;
{$ENDIF}
		end;
	end
	...
	try
		...
		try
			...
			{ if (FAsyncSourceTransfer) then begin
			...
			if (FDragSourceHelper=nil)or not Assigned(FOnGetDragImage) then
					DropResult:=SHDoDragDrop(Sourcehandle.Handle,Self,Self,AllowedEffects,GG);
			DropEffect:=GG;
			if (FAsyncSourceTransfer) then begin
				FAsyncDataObject:=nil;
				FAsyncDropSource:=nil;
		...
		if IsDraggingImage then begin
			if (FDragSourceHelper<>nil) then begin
				FDragSourceHelper:=nil;
			end
			else ImageList_EndDrag;
		end;
	end;
		...
		DRAGDROP_S_DROP:
			...
			*)
			if (LogicalPerformedDropEffect=DROPEFFECT_MOVE)or
				((DropEffect<>DROPEFFECT_MOVE)and(PerformedDropEffect=DROPEFFECT_MOVE)) then Result:=drDropMove
			else Result:=DropEffectToDragResult(DropEffect and AllowedEffects);
	...
	if (PerformedDropEffect=-1) then PerformedDropEffect:=DROPEFFECT_NONE;

	// Abort the async transfer in progress if DoDragDrop returned an error.
	if (AsyncTransfer)and(DropResult<>DRAGDROP_S_DROP) then FAsyncTargetTransfer:=False;

	// Fire OnAfterDrop event unless we are in the middle of an async data
	// transfer.
	if (not AsyncTransfer)and(Assigned(FOnAfterDrop)) then
			FOnAfterDrop(Self,Result,(Result=drDropMove)and((DropEffect<>DROPEFFECT_MOVE)or
			(PerformedDropEffect<>DROPEFFECT_MOVE)));
...
begin
	if (AsyncTransfer) then raise Exception.Create(sDropSourceAsyncBusy);
	if (DragInProgress) then raise Exception.Create(sDropSourceBusy);
	...
	FRequestAsync:=False;
	if (AllowAsyncTransfer) then SetAsyncMode(True);
...
{$IFEND}
		...
		FDragInProgress:=True;
		try Result:=DoExecute;
		finally FDragInProgress:=False;
		end;
	end;
end;
...
begin
	if (Assigned(FOnPaste)) then FOnPaste(Self,Action,DeleteOnPaste);
end;
...
begin
	...
	if (FImages=nil)or(FImageIndex>=FImages.Count) then FImageIndex:=-1;
end;
// -----------------------------------------------------------------------------
...
begin
	...
	if (Value<0)or(FImages=nil)or(FImages.Count=0) then FImageIndex:=-1
	else if (Value<FImages.Count) then FImageIndex:=Value;
end;
// -----------------------------------------------------------------------------

procedure TCustomDropSource.SetPoint(Index:integer;Value:integer);
begin
	if (Index=1) then FImageHotSpot.x:=Value
	else FImageHotSpot.y:=Value;
end;
// -----------------------------------------------------------------------------

function TCustomDropSource.GetPoint(Index:integer):integer;
begin
	if (Index=1) then Result:=FImageHotSpot.x
	else Result:=FImageHotSpot.y;
end;
...
begin
	...
	while (i<Celt)and(FIndex<Formats.Count) do begin
...
begin
	if (FIndex+Celt<=Formats.Count) then begin
		Inc(FIndex,Celt);
		Result:=S_OK;
	end else begin
...
function TCustomDropMultiSource.DoGetData(const FormatEtcIn:TFormatEtc;out Medium:TStgMedium):HRESULT;
var
	i,j:integer;
...
begin
	...
	*)
	for i:=0 to DataFormats.Count-1 do begin
		...
		if (not DF.HasData) then continue;
		...
		(*
			...
			** target.
		*)
		for j:=0 to DF.CompatibleFormats.Count-1 do begin
			...
			*)
			if (CF.AcceptFormat(FormatEtcIn))and(DataFormats[i].AssignTo(CF))and(CF.HasData)and
				(CF.SetDataToMedium(FormatEtcIn,Medium)) then begin
...
function TCustomDropMultiSource.DoSetData(const FormatEtc:TFormatEtc;var Medium:TStgMedium):HRESULT;
var
	i,j:integer;
...
begin
	...
	for i:=0 to DataFormats.Count-1 do
		for j:=0 to DataFormats[i].CompatibleFormats.Count-1 do
			if (DataFormats[i].CompatibleFormats[j].AcceptFormat(FormatEtc))and
				...
				(DataFormats[i].Assign(DataFormats[i].CompatibleFormats[j])) then begin
	...
	if (GenericClipboardFormat.GetDataFromMedium(Self,Medium))and(FRawDataFormat.Assign(GenericClipboardFormat)) then
			Result:=S_OK;
...
procedure TCustomDropMultiSource.GetCompatibleClipboardFormats(const DataFormatClass:TDataFormatClass;
...
begin
	if (dwDirection=DATADIR_GET) then Result:=TEnumFormatEtc.Create(FDataFormats,ddRead)
	else if (dwDirection=DATADIR_SET) then Result:=TEnumFormatEtc.Create(FDataFormats,ddWrite)
	else Result:=nil;
end;
...
begin
	...
	for i:=0 to DataFormats.Count-1 do
		for j:=0 to DataFormats[i].CompatibleFormats.Count-1 do
			if (DataFormats[i].CompatibleFormats[j].AcceptFormat(FormatEtc)) then begin
				Result:=True;
				exit;
			end;
end;
...
procedure TCustomDropMultiSource.DoOnSetData(DataFormat:TCustomDataFormat;ClipboardFormat:TClipboardFormat);
var
	DropEffect:LongInt;
begin
	if (ClipboardFormat is TPasteSucceededClipboardFormat) then begin
		DropEffect:=TPasteSucceededClipboardFormat(ClipboardFormat).Value;
		DoOnPaste(DropEffectToDragResult(DropEffect),(DropEffect=DROPEFFECT_MOVE)and(PerformedDropEffect=DROPEFFECT_MOVE));
	end else if (ClipboardFormat is TPerformedDropEffectClipboardFormat) then begin
...
begin
	...
	// a window property.
	if (DropTarget=0) then DropTarget:=GetProp(Window,sOleDropTargetInterfaceSaved);

	if (DropTarget<>0) then begin
		...
		if (DelphiDropTargets=nil) then DelphiDropTargets:=TList.Create;
...
var
	...
	DropTargetData:PDropTargetData;
begin
	if (DelphiDropTargets<>nil) then begin
		for i:=0 to DelphiDropTargets.Count-1 do begin
...
if (DebugHook<>0) then DisableDelphiDropTargets;

finalization

{$IFDEF SHELL_DRAGIMAGE}
if (Shell32Handle>HINSTANCE_ERROR) then FreeLibrary(Shell32Handle);
{$ENDIF}
if (DebugHook<>0) then RestoreDelphiDropTargets;
