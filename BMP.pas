...
{$ENDIF}
	...
	TPenMode=(pmBlack,pmWhite,pmNop,pmNot,pmCopy,pmNotCopy,pmMergePenNot,pmMaskPenNot,pmMergeNotPen,pmMaskNotPen,pmMerge,
...
{$ENDIF}
	protected
		function GetBitmap:TAMDBMP;
		procedure SetBitmap(Value:TAMDBMP);
		function GetColor:TColor;inline;
		procedure SetColor(Value:TColor);
	...
	public
		...
		destructor Destroy;override;
		procedure Assign(Source:TPersistent);override;
		property Bitmap:TAMDBMP read GetBitmap write SetBitmap;
...
{$ENDIF}
	...
	TTextFormats=(tfBottom,tfCalcRect,tfCenter,tfEditControl,tfEndEllipsis,tfPathEllipsis,tfExpandTabs,tfExternalLeading,
		tfLeft,tfModifyString,tfNoClip,tfNoPrefix,tfRight,tfRtlReading,tfSingleLine,tfTop,tfVerticalCenter,tfWordBreak,
		tfHidePrefix,tfNoFullWidthCharBreak,tfPrefixOnly,tfTabStop,tfWordEllipsis,tfComposited);
...
{$ENDIF}
	...
	public
		...
		procedure BrushCopy(const Dest:TRect;Bitmap:TAMDBMP;const Source:TRect;Color:TColor);virtual;abstract;
...
{$ENDIF}
	...
	public
		...
		procedure BrushCopy(const Dest:TRect;Bitmap:TAMDBMP;const Source:TRect;Color:TColor);override;
	...
	TFindGraphicClassEvent=procedure(Sender:TObject;const Context:TFindGraphicClassContext;var GraphicClass:TGraphicClass)
		of object;
	...
	protected
		...
		procedure Progress(Sender:TObject;Stage:TProgressStage;PercentDone:Byte;RedrawNow:Boolean;const R:TRect;
			const Msg:string);dynamic;
		procedure FindGraphicClass(const Context:TFindGraphicClassContext;var GraphicClass:TGraphicClass);dynamic;
	public
		constructor Create;
		destructor Destroy;override;
...
{$IF DEFINED(CLR)}
		class procedure RegisterFileFormatRes(const AExtension:String;ADescriptionResID:string;AGraphicClass:TGraphicClass);
{$ELSE}
...
{$IF DEFINED(CLR)}
		...
		procedure InternalLoadFromBitmap(Bitmap:System.Drawing.Bitmap;var BMHandle:HBITMAP;var APalette:HPALETTE;
			var DIB:TDIBSection);
		procedure NewImage(NewHandle:HBITMAP;NewPalette:HPALETTE;const NewDIB:TDIBSection;NewImageFormat:TImageFormat=nil;
...
{$ELSE}
		...
		procedure NewImage(NewHandle:HBITMAP;NewPalette:HPALETTE;const NewDIB:TDIBSection;OS2Format:Boolean;
			RLEStream:TStream=nil);
...
{$ENDIF}
		...
		procedure ReadIStream(Stream:IStream;Size:Longint;PosS,PosE:UInt64);
		procedure WriteIStream(Stream:IStream;WriteSize:Boolean);
		procedure ReadIDIB(Stream:IStream;ImageSize:LongWord;PosS,PosE:UInt64;bmf:PBitmapFileHeader=nil);
	protected
		FImage:TBitmapImage;
	...
	public
		...
		procedure SaveToStream(Stream:TStream);override;
		procedure SetSize(AWidth,AHeight:Integer);override;
		procedure LoadFromIStream(Stream:IStream;PosS,PosE:UInt64);
		procedure SaveToIStream(Stream:IStream);
{$IF DEFINED(CLR)}
		function GetHashCode:Integer;override;
		procedure LoadFromBitmap(Bitmap:System.Drawing.Bitmap);
		procedure LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;
			Culture:CultureInfo=nil);overload;
		procedure LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;ResourceSet:System.Type;
			Culture:CultureInfo=nil);overload;
		procedure LoadFromResourceName(const ResName:String;AType:System.Type;Culture:CultureInfo=nil);overload;
...
{$IF NOT DEFINED(CLR)}
	...
	public
		...
		function CreateScaledCopy(ANewWidth,ANewHeight:Integer;
			AInterpolationMode:TWICImageInterpolationMode=wipmHighQualityCubic):TWICImage;
...
{$ELSE}
	...
	DefFontData:TFontData=(Handle:0;Height:0;Orientation:0; { No rotation }
		Pitch:fpDefault;Style:[];Charset:DEFAULT_CHARSET;Name:'Segoe UI');
...
function TransparentStretchBlt(DstDC:HDC;DstX,DstY,DstW,DstH:Integer;SrcDC:HDC;SrcX,SrcY,SrcW,SrcH:Integer;MaskDC:HDC;
...
{$ENDIF}
function AllocPatternBitmap(BkColor,FgColor:TColor):TAMDBMP;

// Alignment must be a power of 2.  Color BMPs require DWORD alignment (32).
function BytesPerScanline(PixelsPerScanline,BitsPerPixel,Alignment:Longint):Longint;

type
	TGraphicIsAnimatingFunc=function(AGraphic:TGraphic):Boolean;

var
	GraphicIsAnimatingFunc:TGraphicIsAnimatingFunc=nil;
...
{$ELSE}
	...
	public
		...
		procedure AssignResource(GraphicsObject:TGraphicsObject;AResource:PResource);
...
var
	...
	LOwner:TThreadID;
begin
	ResHash:=BMP.GetHashCode(ResData,ResDataSize);
	Lock;
	try
		LOwner:=TThread.CurrentThread.ThreadID;
		Result:=ResList;
		while (Result<>nil)and((Result^.Owner<>LOwner)or(Result^.HashCode<>ResHash)or not CompareMem(@Result^.Data,@ResData,
			ResDataSize)) do Result:=Result^.Next;
		...
		with Resource^ do begin
			...
			try
				...
				if DeleteIt then begin
					if Resource=ResList then ResList:=Resource^.Next
					else begin
						P:=ResList;
			...
			if DeleteIt then begin // this is outside the critsect to minimize lock time
...
begin
	...
	try
		...
		if ResData.RefCount=0 then begin
...
begin
	...
	try
		P:=GraphicsObject.FResource;
		if P<>AResource then begin
			if P^.Owner=AResource.Owner then begin
				Inc(AResource^.RefCount);
				GraphicsObject.FResource:=AResource;
				GraphicsObject.Changed;
				FreeResource(P);
			end
			else ChangeResource(GraphicsObject,AResource.Data);
		end;
	finally Unlock;
	end;
end;
...
begin
	if Resource^.Brush.OwnsBitmap then FreeAndNil(Resource^.Brush.Bitmap);
end;
{$ENDIF}
{ TResData }
...
begin
	LHandle:=Handle;
	if LHandle<>0 then DeleteObject(LHandle);
	inherited;
end;
{$ENDIF}
{ TFontData }
...
var
	V:TFontData;
begin
	if Value is TFontData then begin
		V:=TFontData(Value);
		Result:=(Height=V.Height)and(Style=V.Style)and(Charset=V.Charset)and(Name=V.Name)and(Orientation=V.Orientation)and
			(Quality=V.Quality);
	end
	else Result:=False;
end;
...
begin
	Result:=Height xor(Ord(Pitch) shl 1)xor(Integer(Style) shl 2)xor(Ord(Charset) shl 3)xor(Ord(Quality) shl 4)
		xor(Orientation shl 5)xor System.String(Name).GetHashCode;
end;
{$ENDIF}
{ TPenData }
...
var
	V:TPenData;
begin
	if Value is TPenData then begin
		V:=TPenData(Value);
...
var
	V:TBrushData;
begin
	if Value is TBrushData then begin
		V:=TBrushData(Value);
		Result:=(Bitmap=V.Bitmap)and(Color=V.Color)and(Style=V.Style);
	end
	else Result:=False;
end;
...
begin
	Result:=0;
	if Assigned(Bitmap) then Result:=Bitmap.GetHashCode;
	...
	begin
		...
		try
			...
			while Enumerator.MoveNext do begin
				...
				if (ResData<>nil)and(ResData.Handle<>0)and(ResData.Color<0) then begin
					DeleteObject(ResData.Handle);
					ResData.ClearHandle;
				end;
			end;
		finally ResMan.Unlock;
		end;
	end;
{$ELSE}
	procedure ClearColor(ResMan:TResourceManager);
	...
	begin
		...
		try
			...
			while Resource<>nil do begin
				with Resource^ do
					{ Assumes Pen.Color and Brush.Color share the same location }
					if (Handle<>0)and(Pen.Color<0) then begin
						DeleteObject(Handle);
						Handle:=0;
					end;
...
begin
	...
	if Assigned(FResource) then Result:=Result xor(FResource.GetHashCode shl 1);
end;
{$ENDIF}
...
const
	FontCharsets: array [0..17] of TIdentMapEntry=((Value:0;Name:'ANSI_CHARSET'),(Value:1;Name:'DEFAULT_CHARSET'),
		(Value:2;Name:'SYMBOL_CHARSET'),(Value:77;Name:'MAC_CHARSET'),(Value:128;Name:'SHIFTJIS_CHARSET'),(Value:129;
		Name:'HANGEUL_CHARSET'),(Value:130;Name:'JOHAB_CHARSET'),(Value:134;Name:'GB2312_CHARSET'),(Value:136;
		Name:'CHINESEBIG5_CHARSET'),(Value:161;Name:'GREEK_CHARSET'),(Value:162;Name:'TURKISH_CHARSET'),(Value:177;
		Name:'HEBREW_CHARSET'),(Value:178;Name:'ARABIC_CHARSET'),(Value:186;Name:'BALTIC_CHARSET'),(Value:204;
		Name:'RUSSIAN_CHARSET'),(Value:222;Name:'THAI_CHARSET'),(Value:238;Name:'EASTEUROPE_CHARSET'),(Value:255;
		Name:'OEM_CHARSET'));
...
{$IF DEFINED(CLR)}
			...
			try
				...
				Color:=FSource.Color;
				if PixelsPerInch<>FSource.PixelsPerInch then Size:=FSource.Size;
			finally FSource.Unlock;
			end;
{$ELSE}
			TFont(Source).Lock;
			try
				...
				if PixelsPerInch<>TFont(Source).PixelsPerInch then Size:=TFont(Source).Size;
			finally TFont(Source).Unlock;
			end;
{$ENDIF}
		finally Unlock;
		end;
...
begin
	Lock;
	try FontManager.ChangeResource(Self,FontData);
	finally Unlock;
	end;
end;
...
begin
	Result:=(DefFontData.Charset=FontData.Charset)and(DefFontData.Name=FontData.Name);
end;
...
{$IF DEFINED(CLR)}
						...
						else lfFaceName:=Name;
						if (Win32Platform=VER_PLATFORM_WIN32_WINDOWS)and(GetDefFontCharSet=SHIFTJIS_CHARSET)and
							IsDefaultFont(FResource as TFontData) then lfCharSet:=SHIFTJIS_CHARSET;
{$ELSE}
						...
						if (Font.Charset=DEFAULT_CHARSET)and(DefFontData.Charset<>DEFAULT_CHARSET) then
								lfCharSet:=DefFontData.Charset
						else lfCharSet:=Byte(Font.Charset);
						...
						else StrPLCopy(lfFaceName,UTF8ToString(Font.Name),Length(lfFaceName)-1);
						if (Win32Platform=VER_PLATFORM_WIN32_WINDOWS)and(GetDefFontCharSet=SHIFTJIS_CHARSET)and IsDefaultFont(Font)
						then lfCharSet:=SHIFTJIS_CHARSET;
{$ENDIF}
						lfQuality:=Ord(Quality);
						{ Everything else as default }
						{ Only True Type fonts support the angles }
						if lfOrientation<>0 then lfOutPrecision:=OUT_TT_ONLY_PRECIS
...
begin
{$IF DEFINED(CLR)}
	if Handle<>Value then SetFontData(GetHandleFontData(Value));
{$ELSE}
...
{$IF DEFINED(CLR)}
		...
		if FontData.RefCount=0 then FontData.Free;
{$ELSE}
		GetData(FontData);
		FontData.Height:=Value;
		SetData(FontData);
...
const
	DefPenData:TPenData=(Handle:0;Color:clBlack;Width:1;Style:psSolid);
{$ENDIF}
{$IF DEFINED(CLR)}

function GetHandlePenData(Pen:HPen):TPenData;
const
	LogPenStyles: array [0..8] of TPenStyle=(psSolid,psDash,psDot,psDashDot,psDashDotDot,psClear,psInsideFrame,
				...
				begin
					...
					try
						if GetObject(Pen,LPenSize,PExtLogPen)<>0 then
							with Result,ExtLogPen do begin
					...
					finally Marshal.FreeHGlobal(PExtLogPen);
					end;
				end
...
begin
	...
	if Pen<>0 then begin
		...
		end else if LPenSize>=(ExtLogPenSize-4) then // Extended Logical Pen
		begin
			if LPenSize>ExtLogPenSize then // With optional extended dash style
			begin
				...
				try
					if GetObject(Pen,LPenSize,PExtLogPen)<>0 then
						with Result,PExtLogPen^ do begin
							Style:=LogPenStyles[elpPenStyle and PS_STYLE_MASK];
							Width:=elpWidth;
							Color:=elpColor;
							Handle:=Pen;
						end;
				finally FreeMem(Pointer(PExtLogPen));
				end;
			end
			else // Without optional extended dash style
...
begin
	Lock;
	try PenManager.ChangeResource(Self,PenData);
	finally Unlock;
	end;
end;
...
{$IF DEFINED(CLR)}
		...
		PenData.Color:=Value;
		SetPenData(PenData);
		if PenData.RefCount=0 then PenData.Free;
{$ELSE}
		GetData(PenData);
		PenData.Color:=Value;
		SetData(PenData);
...
begin
{$IF DEFINED(CLR)}
	with FResource as TPenData do begin
		if PenHandle=0 then begin
			PenManager.Lock;
			with LogPen do
				try
					if PenHandle=0 then begin
				...
				finally PenManager.Unlock;
				end;
		end;
		Result:=Handle;
	end;
{$ELSE}
	with FResource^ do begin
		if Handle=0 then begin
			PenManager.Lock;
			with LogPen do
				try
...
begin
{$IF DEFINED(CLR)}
	if Value<>Handle then SetPenData(GetHandlePenData(Value));
{$ELSE}
...
var
	PenData:TPenData;
begin
	if (Value>=0)and(Value<>Width) then begin
{$IF DEFINED(CLR)}
		PenData:=GetPenData;
		PenData.Width:=Value;
		SetPenData(PenData);
...
const
	DefBrushData:TBrushData=(Handle:0;Color:clWhite;Bitmap:nil;Style:bsSolid;OwnsBitmap:False);
{$ENDIF}
{$IF DEFINED(CLR)}
...
{$ENDIF}
				...
				else Style:=TBrushStyle(lbHatch+Ord(bsHorizontal));
				end;
				Color:=lbColor;
{$IF DEFINED(CLR)}
				BrushHandle:=Brush;
...
{$IF DEFINED(CLR)}
			...
			LSource.Lock;
			try SetBrushData(LSource.FResource as TBrushData);
			finally LSource.Unlock;
			end;
{$ELSE}
			TBrush(Source).Lock;
			try BrushManager.AssignResource(Self,TBrush(Source).FResource);
...
begin
	Lock;
	try BrushManager.ChangeResource(Self,BrushData);
	finally Unlock;
	end;
end;

function TBrush.GetBitmap:TAMDBMP;
begin
{$IF DEFINED(CLR)}
...
procedure TBrush.SetBitmap(Value:TAMDBMP);
var
...
{$IF DEFINED(CLR)}
	...
	if BrushData.RefCount=0 then BrushData.Free;
{$ELSE}
	SetData(BrushData);
{$ENDIF}
end;
...
var
	BrushData:TBrushData;
begin
	if (Value<>Color)or((Style=bsClear)and(Style<>bsSolid)) then begin
{$IF DEFINED(CLR)}
...
{$ELSE}
				if Handle=0 then begin
					with LogBrush do begin
						if Brush.Bitmap<>nil then begin
							lbStyle:=BS_PATTERN;
...
begin
{$IF DEFINED(CLR)}
	if Value<>Handle then SetBrushData(GetHandleBrushData(Value));
{$ELSE}
...
var
	BrushData:TBrushData;
begin
	if (Value<>Style)or((Value=bsClear)and(Color<>clWhite)) then begin
{$IF DEFINED(CLR)}
		BrushData:=GetBrushData;
...
{$ELSE}
	...
	try
		Result:=FLockCount=0;
		if Result then Lock;
	finally LeaveCriticalSection(CounterLock);
	end;
{$ENDIF}
end;
...
procedure TCanvas.AngleArc(X,Y:Integer;Radius:Cardinal;StartAngle,SweepAngle:Single);
begin
	Changing;
	RequiredState([csHandleValid,csPenValid,csBrushValid]);
...
procedure TCanvas.BrushCopy(const Dest:TRect;Bitmap:TAMDBMP;const Source:TRect;Color:TColor);
const
...
begin
	...
	try
		...
		try
			...
			if Bitmap.TransparentColor=Color then begin
				Mask:=nil;
			...
			try
				...
				if Brush.Style=bsClear then begin
					TransparentStretchBlt(FHandle,Dest.Left,Dest.Top,DstW,DstH,Bitmap.Canvas.FHandle,Source.Left,Source.Top,SrcW,
						SrcH,MaskDC,Source.Left,Source.Top);
				end else begin
					StretchBlt(FHandle,Dest.Left,Dest.Top,DstW,DstH,Bitmap.Canvas.FHandle,Source.Left,Source.Top,SrcW,
						SrcH,SRCCOPY);
					crText:=SetTextColor(Self.FHandle,0);
					crBack:=SetBkColor(Self.FHandle,$FFFFFF);
					StretchBlt(Self.FHandle,Dest.Left,Dest.Top,DstW,DstH,MaskDC,Source.Left,Source.Top,SrcW,SrcH,ROP_DSPDxax);
...
begin
	...
	StretchBlt(FHandle,Dest.Left,Dest.Top,Dest.Right-Dest.Left,Dest.Bottom-Dest.Top,Canvas.FHandle,Source.Left,Source.Top,
...
begin
	if (Graphic<>nil)and not Graphic.Empty then begin
		Changing;
...
begin
	if Graphic<>nil then begin
		...
		if (Graphic.ScaledDrawer<>nil)and Graphic.ScaledDrawer.Initialized then Graphic.ScaledDrawer.Draw(Self,Rect)
		else Graphic.Draw(Self,Rect);
		Changed;
	end;
end;
...
begin
	...
	if (FTextFlags and ETO_RTLREADING)<>0 then begin
...
begin
	...
	if ((FTextFlags and ETO_RTLREADING)<>0)and(CanvasOrientation=coRightToLeft) then Inc(X,TextWidth(Text)+1);
...
{$IF DEFINED(CLR)}
	if tfModifyString in TextFormat then begin
		SB:=StringBuilder.Create(Text,Length(Text)+4);
		DrawTextEx(Handle,SB,Length(Text),Rect,Format,nil);
		Text:=SB.ToString;
	end
	else DrawTextEx(Handle,Text,Length(Text),Rect,Format,nil);
{$ELSE}
	if tfModifyString in TextFormat then Text:=Text+#0#0#0#0;
	DrawTextEx(Handle,PChar(Text),Length(Text),Rect,Format,nil);
	if tfModifyString in TextFormat then SetLength(Text,StrLen(PChar(Text)));
{$ENDIF}
end;
...
begin
	if (FHandle<>0)and(State-[csPenValid,csBrushValid,csFontValid]<>State) then begin
...
begin
	...
	if NeededState<>[] then begin
		if csHandleValid in NeededState then begin
			CreateHandle;
			if FHandle=0 then raise EInvalidOperation.CreateRes({$IFNDEF CLR}@{$ENDIF}SNoCanvasHandle);
		end;
...
const
	PenModes: array [TPenMode] of Word=(R2_BLACK,R2_WHITE,R2_NOP,R2_NOT,R2_COPYPEN,R2_NOTCOPYPEN,R2_MERGEPENNOT,
		R2_MASKPENNOT,R2_MERGENOTPEN,R2_MASKNOTPEN,R2_MERGEPEN,R2_NOTMERGEPEN,R2_MASKPEN,R2_NOTMASKPEN,R2_XORPEN,
		R2_NOTXORPEN);
begin
...
begin
	if csFontValid in State then begin
...
begin
	if csPenValid in State then begin
...
begin
	if csBrushValid in State then begin
...
{$ENDIF}
	...
	if (ErrorCode<>0)and(FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM,nil,ErrorCode,LOCALE_USER_DEFAULT,Buf,BufSize,nil)<>0)
	then
{$IF DEFINED(CLR)}
...
begin
	...
	try
		...
		if Mono then Result:=CreateBitmap(Size.X,Size.Y,1,1,nil)
		else begin
			DC:=GetDC(0);
...
{$ENDIF}
		...
		if SavePal<>0 then SavePal:=SelectPalette(MemDC,SavePal,True)
		else
{$IF DEFINED(CLR)}
...
begin
	...
	for I:=255 downto 1 do // don't move zeroth item
		with P4^[I],P3^[I] do begin // order is significant for last item moved
			rgbRed:=rgbtRed;
			rgbGreen:=rgbtGreen;
			rgbBlue:=rgbtBlue;
			rgbReserved:=0;
...
begin
	...
	for I:=1 to ColorCount-1 do // don't move zeroth item
		with P4^[I],P3^[I] do begin
			rgbtRed:=rgbRed;
			rgbtGreen:=rgbGreen;
			rgbtBlue:=rgbBlue;
		end;
	if ColorCount<256 then begin
...
begin
	...
	while I<Count do begin
		Color:=C^;
		C^:=(Byte(Color shr 16)or(Word(Color) shr 8) shl 8)or(Byte(Word(Color)) shl 16);
		Inc(I);
		Inc(C);
	end;
end;
...
begin
	...
	for I:=0 to 15 do Pal.palPalEntry[I]:=Entries[I];
	DC:=GetDC(0);
	try
		...
		if SysPalSize>=16 then begin
			...
			if TColor(Pal.palPalEntry[7])=clSilver then begin
...
begin
	...
	try
		...
		if SysPalSize>=16 then begin
			...
			else GetSystemPaletteEntries(DC,SysPalSize-8,8,Pal.palPalEntry[Pal.palNumEntries-8]);
		end else begin
		end;
	finally ReleaseDC(0,DC);
	end;
...
function PaletteFromDIBColorTable(DIBHandle:THandle;const ColorTable: array of COLORREF;ColorCount:Integer):HPALETTE;
{$ELSE}
...
{$IF DEFINED(CLR)}
		if Length(ColorTable)>0 then
			for I:=0 to ColorCount-1 do Pal.palPalEntry[I]:=ColorTable[I];
{$ELSE}
		Move(ColorTable^,Pal.palPalEntry,ColorCount*4);
{$ENDIF}
	end;
	if Pal.palNumEntries=0 then Exit;
	if (Pal.palNumEntries<>16)or not SystemPaletteOverride(Pal) then ByteSwapColors(Pal.palPalEntry,Pal.palNumEntries);
{$IF DEFINED(CLR)}
	Result:=CreatePalette(Pal);
{$ELSE}
	Result:=CreatePalette(PLogPalette(@Pal)^);
...
function PaletteToDIBColorTable(Pal:HPALETTE;var ColorTable: array of COLORREF):Integer;
{$ELSE}

function PaletteToDIBColorTable(Pal:HPALETTE;var ColorTable: array of TRGBQuad):Integer;
{$ENDIF}
begin
	Result:=0;
	if (Pal=0)or(GetObject(Pal,SizeOf(Result), {$IFNDEF CLR}@{$ENDIF}Result)=0)or(Result=0) then Exit;
...
procedure TwoBitsFromDIB(var BI:TBitmapInfoHeader;var XorBits,AndBits:HBITMAP;const IconSize:TPoint);
type
...
begin
	...
	try
		...
		try XorBits:=DupBits(Temp,IconSize,False);
		finally DeleteObject(Temp);
		end;
		with BI do begin
		...
		Colors^[1]:=$FFFFFF;
		Temp:=GDICheck(CreateDIBitmap(DC,BI,CBM_INIT,Bits,PBitmapInfo(@BI)^,DIB_RGB_COLORS));
		try AndBits:=DupBits(Temp,IconSize,True);
		finally DeleteObject(Temp);
		end;
	finally ReleaseDC(0,DC);
	end;
...
procedure ReadIcon(Stream:TStream;var Icon:HICON;ImageCount:Integer;StartOffset:Integer;const RequestedSize:TPoint;
	var IconSize:TPoint);
type
...
var
	...
	*)
	...
	begin
		if New.Width=0 then NewX:=256-IconSize.X
		else NewX:=New.Width-IconSize.X;
		if New.Height=0 then NewY:=256-IconSize.Y
		else NewY:=New.Height-IconSize.Y;
		if Old.Width=0 then OldX:=256-IconSize.X
		else OldX:=Old.Width-IconSize.X;
		if Old.Height=0 then OldY:=256-IconSize.Y
		else OldY:=Old.Height-IconSize.Y;
		Result:=(Abs(NewX)<=Abs(OldX))and((NewX<=0)or(NewX<=OldX))and(Abs(NewY)<=Abs(OldY))and((NewY<=0)or(NewY<=OldY));
	end;
...
begin
	...
	try
		...
		for N:=1 to ImageCount-1 do begin
			C1:=AdjustColor(List^[N].Colors);
			if (C1<=Colors)and(C1>=BestColor)and BetterSize(List^[Index],List^[N]) then begin
				Index:=N;
				BestColor:=C1;
			end;
		end;
		...
		*)
		with List^[Index] do begin
			if Width=0 then IconSize.X:=256
			else IconSize.X:=Width;
			if Height=0 then IconSize.Y:=256
			else IconSize.Y:=Height;
			BI:=AllocMem(DIBSize);
			try
				Stream.Seek(DIBOffset-(HeaderLen+StartOffset),1);
				Stream.Read(BI^,DIBSize);

				if CompareMem(BI,@PNGSignature[0],8) then begin
					WICImage:=TWICImage.Create;
					try
						WICImage.LoadFromStream(Stream);
						IconBitmap:=TAMDBMP.Create;
						try
							IconBitmap.Assign(WICImage);
							MaskBitmap:=TAMDBMP.Create;
							try
							...
							finally MaskBitmap.Free;
							end;
						finally IconBitmap.Free;
						end;
					finally WICImage.Free;
					end;
				end else begin
					...
					with AndInfo do AndLen:=bmWidthBytes*bmHeight*bmPlanes;
					with XorInfo do XorLen:=bmWidthBytes*bmHeight*bmPlanes;
					Length:=AndLen+XorLen;
					ResData:=AllocMem(Length);
					try
						AndMem:=ResData;
						with AndInfo do XorMem:=Pointer(PByte(ResData)+AndLen);
						GetBitmapBits(AndBits,AndLen,AndMem);
						GetBitmapBits(XorBits,XorLen,XorMem);
						DeleteObject(XorBits);
						DeleteObject(AndBits);
						Icon:=CreateIcon(HInstance,IconSize.X,IconSize.Y,XorInfo.bmPlanes,XorInfo.bmBitsPixel,AndMem,XorMem);
					finally FreeMem(ResData,Length);
					end;
				end;
			finally FreeMem(BI,DIBSize);
			end;
		end;
	finally FreeMem(List,HeaderLen);
	end;
...
{$ENDIF}
	...
	else if (Bytes>=(SizeOf(DS.dsbm)+SizeOf(DS.dsbmih)))and(DS.dsbmih.biSize>=DWORD(SizeOf(DS.dsbmih))) then BI:=DS.dsbmih
	else begin
{$IF NOT DEFINED(CLR)}
...
{$ENDIF}
	...
	if BI.biSizeImage=0 then BI.biSizeImage:=BytesPerScanline(BI.biWidth,BI.biBitCount,32)*Abs(BI.biHeight);
end;
...
begin
	...
	end else if BI.biClrUsed=0 then InfoHeaderSize:=SizeOf(TBitmapInfoHeader)+SizeOf(TRGBQuad)*(1 shl BI.biBitCount)
	else InfoHeaderSize:=SizeOf(TBitmapInfoHeader)+SizeOf(TRGBQuad)*BI.biClrUsed;
...
function InternalGetDIB(Bitmap:HBITMAP;Palette:HPALETTE;BitmapInfoPtr:IntPtr;var Bits:TBytes;Colors:Integer):Boolean;
{$ELSE}
...
begin
{$IF DEFINED(CLR)}
	BitmapInfo:=TBitmapInfoHeader(Marshal.PtrToStructure(BitmapInfoPtr,TypeOf(TBitmapInfoHeader)));
{$ENDIF}
...
{$IF DEFINED(CLR)}
		...
		Result:=GetDIBits(DC,Bitmap,0,BitmapInfo.biHeight,Bits,BitmapInfoPtr,DIB_RGB_COLORS)<>0;
{$ELSE}
		Result:=GetDIBits(DC,Bitmap,0,TBitmapInfoHeader(BitmapInfo).biHeight,@Bits,TBitmapInfo(BitmapInfo),
			DIB_RGB_COLORS)<>0;
{$ENDIF}
	finally
...
begin
	...
	try
		...
		try
			...
			if WriteLength then begin
				Length:=SizeOf(CI)+SizeOf(List)+ColorInfoSize+ColorBitsSize+MonoBitsSize;
			...
			with List,PBitmapInfoHeader(ColorInfo)^ do begin
				Width:=biWidth;
				Height:=biHeight;
...
var
	IsAnimating:Boolean;
begin
	if Assigned(BMP.GraphicIsAnimatingFunc) then IsAnimating:=BMP.GraphicIsAnimatingFunc(Self)
	else IsAnimating:=False;

	if not IsAnimating then begin
		FModified:=True;
		UpdateScaledDrawer;
	end;
	if Assigned(FOnChange) then FOnChange(Self);
...
{$IFDEF CLR}[SecurityPermission(SecurityAction.Assert,UnmanagedCode=True)]
{$ENDIF}
...
procedure TGraphic.DefineProperties(Filer:TFiler);
	...
	begin
		if Filer.Ancestor<>nil then Result:=not(Filer.Ancestor is TGraphic)or not Equals(TGraphic(Filer.Ancestor))
...
begin
	...
	if Empty or Graphic.Empty then begin
	...
	if Result then begin
		...
		try
			...
			try
				...
				Result:=(MyImage.Size=GraphicsImage.Size)and CompareMem(MyImage.Memory,GraphicsImage.Memory,MyImage.Size);
			finally GraphicsImage.Free;
			end;
		finally MyImage.Free;
		end;
	end;
end;
...
begin
	if Obj is TGraphic then Result:=Equals(TGraphic(Obj))
...
procedure TGraphic.Progress(Sender:TObject;Stage:TProgressStage;PercentDone:Byte;RedrawNow:Boolean;const R:TRect;
	const Msg:string);
begin
...
begin
	if AGraphicScalerClass=nil then raise EArgumentNilException.CreateRes(@SArgumentNil);
...
begin
	if FScaledDrawer<>nil then FScaledDrawer.Initialize;
end;
...
{$ENDIF}
		...
		procedure BuildFilterStrings(AGraphicClass:TGraphicClass;var Descriptions,Filters:string);
	end;
...
procedure TFileFormatsList.Add(const Ext,Desc:String;DescID:string;AClass:TGraphicClass);
{$ELSE}

procedure TFileFormatsList.Add(const Ext,Desc:String;DescID:Integer;AClass:TGraphicClass);
{$ENDIF}
var
...
{$ELSE}
	New(NewFormat);
{$ENDIF}
	with NewFormat{$IFNDEF CLR}^{$ENDIF} do begin
...
begin
	for I:=Count-1 downto 0 do
		with TFileFormatType(Items[I]){$IFNDEF CLR}^{$ENDIF} do
			if GraphicClass.CanLoadFromStream(Stream) then begin
				Result:=GraphicClass;
				Exit;
			end;
	Result:=nil;
...
begin
	for I:=Count-1 downto 0 do begin
		...
		if LFormat.GraphicClass.InheritsFrom(AClass) then begin
{$IF DEFINED(CLR)}
			LFormat.Free;
{$ELSE}
			Dispose(LFormat);
...
procedure TFileFormatsList.BuildFilterStrings(AGraphicClass:TGraphicClass;var Descriptions,Filters:string);
var
	C,I:Integer;
	LFormat:TFileFormatType;
...
begin
	...
	try
		...
		for I:=Count-1 downto 0 do begin
			LFormat:=TFileFormatType(Items[I]);
			with LFormat{$IFNDEF CLR}^{$ENDIF} do begin
				if GraphicClass.InheritsFrom(AGraphicClass)and(Extension<>'') then begin
					if C<>0 then begin
						LDescriptions.Append('|');
						LFilters.Append(';');
					end;
{$IF DEFINED(CLR)}
					if (Description='')and(DescResID<>'') then Description:=LoadResString(DescResID);
{$ELSE}
...
{$ENDIF}
		...
		if C>1 then LDescriptions.Insert(0,Format('%s (%s)|%1:s|',[sAllFilter,LFilters.ToString(True)]));
...
{$IF DEFINED(CLR)}
	...
	try FFormats.Add(TObject(Integer(Fmt)));
	except
		FClasses.Delete(I);
		raise;
	end;
...
begin
	for I:=FFormats.Count-1 downto 0 do
		if Integer(FFormats[I])=Fmt then begin
...
begin
	for I:=FClasses.Count-1 downto 0 do
		if TGraphicClass(FClasses[I]).InheritsFrom(AClass) then begin
			FClasses.Delete(I);
			FFormats.Delete(I);
		end;
end;
...
begin
	inherited Create;
	FGraphic:=AGraphic;
	if AInitialize then Initialize;
end;
...
begin
	if Graphic is Dest.ClassType then Dest.Assign(Graphic)
	else inherited AssignTo(Dest);
end;

procedure TPicture.ForceType(GraphicType:TGraphicClass);
begin
	if not(Graphic is GraphicType) then begin
		FGraphic.Free;
		FGraphic:=nil;
...
begin
	...
	FGraphic:=NewGraphic;
	if FGraphic<>nil then FGraphic.OnChange:=Changed;
	Changed(Self);
end;

function TPicture.GetBitmap:TAMDBMP;
begin
	ForceType(TAMDBMP);
	Result:=TAMDBMP(Graphic);
end;
...
procedure TPicture.SetBitmap(Value:TAMDBMP);
begin
	SetGraphic(Value);
end;
...
var
	GraphicClass:TGraphicClass;
begin
	if Value<>nil then GraphicClass:=TGraphicClass(Value.ClassType)
	else GraphicClass:=nil;

	Load(GraphicClass, procedure(Graphic:TGraphic)
		begin
...
begin
	...
	if GraphicClass=nil then raise EInvalidGraphic.CreateFmt(SUnknownExtension,[Ext]);
...
{$IFDEF CLR}[UIPermission(SecurityAction.LinkDemand,Clipboard=UIPermissionClipboard.AllClipboard)]
{$ENDIF}

procedure TPicture.LoadFromClipboardFormat(AFormat:Word;AData:THandle;APalette:HPALETTE);
var
	GraphicClass:TGraphicClass;
	Context:TFindGraphicClassContext;
begin
	...
	FindGraphicClass(Context,GraphicClass);
	if GraphicClass=nil then InvalidGraphic({$IFNDEF CLR}@{$ENDIF}SUnknownClipboardFormat);
...
procedure TPicture.SaveToClipboardFormat(var AFormat:Word;var AData:THandle;var APalette:HPALETTE);
begin
	if FGraphic<>nil then FGraphic.SaveToClipboardFormat(AFormat,AData,APalette);
end;
...
var
	...
	Context:TFindGraphicClassContext;
begin
	if Stream.Size-Stream.Position=0 then GraphicClass:=TAMDBMP
	else GraphicClass:=FileFormats.FindFormat(Stream);
	Context.FSource:=gsStream;
	Context.FStream:=Stream;
	FindGraphicClass(Context,GraphicClass);
	if GraphicClass=nil then InvalidGraphic({$IFNDEF CLR}@{$ENDIF}SUnknownStreamFormat);
...
begin
	if Source=nil then SetGraphic(nil)
	else if Source is TPicture then SetGraphic(TPicture(Source).Graphic)
	else if Source is TGraphic then SetGraphic(TGraphic(Source))
	else inherited Assign(Source);
end;

class procedure TPicture.RegisterFileFormat(const AExtension,ADescription:string;AGraphicClass:TGraphicClass);
begin
{$IF DEFINED(CLR)}
...
class procedure TPicture.RegisterFileFormatRes(const AExtension:String;ADescriptionResID:string;
AGraphicClass:TGraphicClass);
{$ELSE}

class procedure TPicture.RegisterFileFormatRes(const AExtension:String;ADescriptionResID:Integer;
AGraphicClass:TGraphicClass);
{$ENDIF}
begin
...
class procedure TPicture.RegisterClipboardFormat(AFormat:Word;AGraphicClass:TGraphicClass);
begin
...
procedure TPicture.Progress(Sender:TObject;Stage:TProgressStage;PercentDone:Byte;RedrawNow:Boolean;const R:TRect;
const Msg:string);
begin
	if Assigned(FOnProgress) then FOnProgress(Sender,Stage,PercentDone,RedrawNow,R,Msg);
end;

procedure TPicture.FindGraphicClass(const Context:TFindGraphicClassContext;var GraphicClass:TGraphicClass);
begin
...
begin
	with Stream do begin
		if Graphic<>nil then LClassName:=Graphic.Classname
		else LClassName:='';
		...
		if Graphic<>nil then Graphic.WriteData(Stream);
	end;
end;
	...
	begin
		if Filer.Ancestor<>nil then begin
			...
			if Filer.Ancestor is TPicture then begin
				...
				Result:=not((Graphic=Ancestor.Graphic)or((Graphic<>nil)and(Ancestor.Graphic<>nil)and
...
constructor TMetafileCanvas.CreateWithComment(AMetafile:TMetafile;ReferenceDevice:HDC;
...
begin
	...
	try
		if FMetafile.MMWidth=0 then
			if FMetafile.Width=0 then FMetafile.MMWidth:=GetDeviceCaps(RefDC,HORZSIZE)*100
			else FMetafile.MMWidth:=MulDiv(FMetafile.Width,GetDeviceCaps(RefDC,HORZSIZE)*100,GetDeviceCaps(RefDC,HORZRES));
		if FMetafile.MMHeight=0 then
			if FMetafile.Height=0 then FMetafile.MMHeight:=GetDeviceCaps(RefDC,VERTSIZE)*100
			else FMetafile.MMHeight:=MulDiv(FMetafile.Height,GetDeviceCaps(RefDC,VERTSIZE)*100,GetDeviceCaps(RefDC,VERTRES));
...
{$IF DEFINED(CLR)}
		...
		if FMetafileDC.Handle=0 then GDIError;
...
begin
{$IF DEFINED(CLR)}
	if FMetafileDC.Handle<>0 then begin
		Handle:=0;
...
begin
	...
	if MetaPal<>0 then SelectPalette(ACanvas.Handle,OldPal,True);
end;
...
begin
	...
	if P>0 then begin
		L:=PosEx(#0,S,P+1)-1;
		if L>0 then Result:=Copy(S,P+1,L-P)
	end;
end;
{$ELSE}
...
begin
	if Assigned(FImage) then Result:=FImage.FHandle
	else Result:=0;
end;
...
begin
	...
	with FImage do
		if FInch=0 then
			if FHandle=0 then Result:=FTempHeight
			else begin { convert 0.01mm units to referenceDC device pixels }
{$IF DEFINED(CLR)}
...
begin
	Result:=0;
	if (FImage=nil)or(FImage.FHandle=0) then Exit;
	if FImage.FPalette=0 then begin
		Count:=GetEnhMetaFilePaletteEntries(FImage.FHandle,0,nil);
		if Count=0 then Exit
		else if Count>256 then Count:=Count and $FF;
...
begin
	...
	with FImage do
		if FInch=0 then
			if FHandle=0 then Result:=FTempWidth
			else begin { convert 0.01mm units to referenceDC device pixels }
{$IF DEFINED(CLR)}
				GetEnhMetaFileHeader(FHandle,Marshal.SizeOf(TypeOf(EMFHeader)),EMFHeader);
{$ELSE}
...
begin
	...
	try
		...
		if not Result then
				Result:=(Stream.Read(WMF,SizeOf(WMF))=SizeOf(WMF))and(WMF.Key=WMFKey)and
				(ComputeAldusChecksum(WMF)=WMF.CheckSum);
...
begin
	if TestEMF(Stream) then ReadEMFStream(Stream)
	else ReadWMFStream(Stream,Stream.Size-Stream.Position);
...
begin
	...
	if Length<=4 then Assign(nil)
	else if TestEMF(Stream) then ReadEMFStream(Stream)
	else ReadWMFStream(Stream,Length-SizeOf(Length));
	PaletteModified:=Palette<>0;
	Changed(Self);
end;
...
begin
	...
	with FImage do begin
		...
		FInch:=0;
		with EnhHeader.rclFrame do begin
...
begin
	...
	if EnhHeader.dSignature<>ENHMETA_SIGNATURE then InvalidMetafile;
	if EnhHeader.nBytes<SizeOf(EnhHeader) then InvalidMetafile;
	GetMem(Buf,EnhHeader.nBytes);
	with FImage do
		try
			Move(EnhHeader,Buf^,SizeOf(EnhHeader));
			Stream.ReadBuffer(PByte(Buf+SizeOf(EnhHeader))^,EnhHeader.nBytes-SizeOf(EnhHeader));
			...
			Enhanced:=True;
		finally FreeMem(Buf,EnhHeader.nBytes);
		end;
end;
{$ENDIF}
...
{$ENDIF}
	if (WMF.Key<>WMFKey)or(ComputeAldusChecksum(WMF)<>WMF.CheckSum) then InvalidMetafile;
...
{$ELSE}
			GetEnhMetaFileHeader(FHandle,SizeOf(EMFHeader),@EMFHeader);
{$ENDIF}
			with MFP,EMFHeader.rclFrame do begin
				MM:=MM_ANISOTROPIC;
				xExt:=Right;
				yExt:=Bottom;
				hmf:=0;
...
begin
	SaveEnh:=Enhanced;
	try
		if AnsiLowerCase(ExtractFileExt(Filename))='.wmf' then Enhanced:=False; { For 16 bit compatibility }
		inherited SaveToFile(Filename);
	finally Enhanced:=SaveEnh;
	end;
end;
...
begin
	if FImage<>nil then
		if Enhanced then WriteEMFStream(Stream)
		else WriteWMFStream(Stream);
end;
...
begin
	...
	with FImage do
		if FInch=0 then
			if FHandle=0 then FTempHeight:=Value
...
{$ENDIF}
		...
		else MMHeight:=MulDiv(Value,HundredthMMPerInch,ScreenLogPixels);
end;

procedure TMetafile.SetInch(Value:Word);
begin
	if FImage=nil then NewImage;
	if FImage.FInch<>Value then begin
		UniqueImage;
		FImage.FInch:=Value;
		Changed(Self);
	end;
...
begin
	if FImage=nil then NewImage;
	FImage.FTempHeight:=0;
	if FImage.FHeight<>Value then begin
		UniqueImage;
...
begin
	if FImage=nil then NewImage;
	FImage.FTempWidth:=0;
	if FImage.FWidth<>Value then begin
		UniqueImage;
		FImage.FWidth:=Value;
		Changed(Self);
	end;
...
begin
	...
	with FImage do
		if FInch=0 then
			if FHandle=0 then FTempWidth:=Value
			else begin { convert device pixels to 0.01mm units }
{$IF DEFINED(CLR)}
				GetEnhMetaFileHeader(FHandle,Marshal.SizeOf(TypeOf(EMFHeader)),EMFHeader);
{$ELSE}
...
{$ENDIF}
		...
		else MMWidth:=MulDiv(Value,HundredthMMPerInch,ScreenLogPixels);
end;
...
begin
	...
	Result:=(Size>SizeOf(Header))and(Header.iType=EMR_HEADER)and(Header.dSignature=ENHMETA_SIGNATURE);
end;
{$ELSE}
...
begin
	Size:=Stream.Size-Stream.Position;
	if Size>SizeOf(Header) then begin
...
begin
	if FImage=nil then Self.NewImage
	else if FImage.FRefCount>1 then begin
		NewImage:=TMetafileImage.Create;
		if FImage.FHandle<>0 then NewImage.FHandle:=CopyEnhMetafile(FImage.FHandle,nil);
...
{$ENDIF}
	with FImage do begin
		with WMF do begin
			Key:=WMFKey;
			if FInch=0 then Inch:=96 { WMF defaults to 96 units per inch }
			else Inch:=FInch;
			with Box do begin
...
{$IF DEFINED(CLR)}
			...
			if GetWinMetaFileBits(FHandle,Length,Bits,MM_ANISOTROPIC,RefDC)<Length then GDIError;
			Stream.WriteBuffer(StructureToBytes(TObject(WMF)),Marshal.SizeOf(TypeOf(TMetafileHeader)));
...
{$ELSE}
			...
			try
				...
				Stream.WriteBuffer(Bits^,Length);
			finally FreeMem(Bits,Length);
			end;
{$ENDIF}
		finally ReleaseDC(0,RefDC);
		end;
	end;
end;
...
begin
	...
	if AData=0 then InvalidGraphic({$IFNDEF CLR}@{$ENDIF}SUnknownClipboardFormat);
	NewImage;
	with FImage do begin
	...
	with FImage do
		if FInch=0 then
			if FHandle=0 then begin
				FTempWidth:=AWidth;
				FTempHeight:=AHeight;
			end else begin { convert device pixels to 0.01mm units }
{$IF DEFINED(CLR)}
				GetEnhMetaFileHeader(FHandle,Marshal.SizeOf(TypeOf(EMFHeader)),EMFHeader);
{$ELSE}
				GetEnhMetaFileHeader(FHandle,SizeOf(EMFHeader),@EMFHeader);
...
{$ENDIF}
	...
	public
		constructor Create(ABitmap:TAMDBMP);
		destructor Destroy;override;
{$IF DEFINED(CLR)}
		function GetHashCode:Integer;override;
{$ENDIF}
...
begin
	with BitmapCanvasList.LockList do
		...
		finally BitmapCanvasList.UnlockList;
		end;
end;
...
constructor TBitmapCanvas.Create(ABitmap:TAMDBMP);
begin
	inherited Create;
{$IF DEFINED(CLR)}
...
{$IF DEFINED(CLR)}
begin
	if FBitmapDC.Handle<>0 then begin
		Lock;
		try
			if FBitmapDC.OldBitmap<>0 then SelectObject(FBitmapDC.Handle,FBitmapDC.OldBitmap);
			if FBitmapDC.OldPalette<>0 then SelectPalette(FBitmapDC.Handle,FBitmapDC.OldPalette,True);
			Self.Handle:=0;
...
begin
	...
	if Assigned(Brush) then Result:=Result xor(Brush.GetHashCode shl 2);
	if Assigned(Font) then Result:=Result xor(Font.GetHashCode shl 3);
	if Assigned(Pen) then Result:=Result xor(Pen.GetHashCode shl 4);
end;
{$ENDIF}
...
{$IF DEFINED(CLR)}
			...
			if FBitmap.FImage.FHandle<>0 then FBitmapDC.OldBitmap:=SelectObject(FBitmapDC.Handle,FBitmap.FImage.FHandle)
			else FBitmapDC.OldBitmap:=0;
			Handle:=FBitmapDC.Handle;
{$ELSE}
			H:=CreateCompatibleDC(0);
			if FBitmap.FImage.FHandle<>0 then FOldBitmap:=SelectObject(H,FBitmap.FImage.FHandle)
			else FOldBitmap:=0;
			if FBitmap.FImage.FPalette<>0 then begin
...
begin
{$IF DEFINED(CLR)}
	if (FDIBHandle<>0)and(FDIBHandle<>FHandle) then begin
		DeselectBitmap(FDIBHandle);
		DeleteObject(FDIBHandle);
		FDIBHandle:=0;
	end;
...
begin
	with FDIB.dsbm do
			Result:=inherited GetHashCode xor(bmWidth shl 1)xor(bmHeight shl 2)xor(bmWidthBytes shl 3)xor(bmPlanes shl 4)
			xor(bmBitsPixel shl 5);
end;
{$ENDIF}
{ TBitmap }
...
begin
				...
				if DC<>0 then
					with Bitmap^ do begin
						...
						if Result<>0 then begin
							...
							StretchDIBits(DC,0,0,biWidth,biHeight,0,0,biWidth,biHeight,Bits,PBitmapInfo(Bitmap)^,
	...
	finally FreeMem(Bitmap,BitmapInfoSize+BitmapBitsSize);
	end;
end;
{$ENDIF}
...
begin
	...
	try
		...
		if Bitmap<>nil then
			try
				if (Bitmap^.biBitCount*Bitmap^.biPlanes)<=8 then begin
					ColorCount:=1 shl (Bitmap^.biBitCount);
						...
						try
							...
							if DC<>0 then
								with BitmapInfo^ do begin
									...
									if Result<>0 then begin
										Save:=SelectObject(DC,Result);
										StretchDIBits(DC,0,0,biWidth,biHeight,0,0,biWidth,biHeight,Bits,PBitmapInfo(BitmapInfo)^,
											DIB_RGB_COLORS,SRCCOPY);
										SelectObject(DC,Save);
									end;
								end;
					...
					finally FreeMem(BitmapInfo,BitmapInfoSize);
					end;
				end else begin
					Temp:=TAMDBMP.Create;
					try
...
begin
	if (DIB.dsbmih.biCompression and BI_BITFIELDS<>0)and(DIB.dsBitFields[0]=0) then
		if DIB.dsbmih.biBitCount=16 then begin
			// fix buggy 16 bit color drivers
			DIB.dsBitFields[0]:=$F800;
			DIB.dsBitFields[1]:=$07E0;
			DIB.dsBitFields[2]:=$001F;
		end else if DIB.dsbmih.biBitCount=32 then begin
...
function CopyBitmap(Handle:HBITMAP;OldPalette,NewPalette:HPALETTE;var DIB:TDIBSection;Canvas:TCanvas):HBITMAP;
var
...
{$ENDIF}
				with DIB.dsbmih do
					try
						...
						biPlanes:=1;
{$IF DEFINED(CLR)}
						if bmBitsPixel=0 then biBitCount:=GetDeviceCaps(ScreenDC,BITSPIXEL)*GetDeviceCaps(ScreenDC,PLANES);
...
{$ENDIF}
						if (biBitCount<=8) then begin
							if (biBitCount=1)and((Handle=0)or(SrcDIB.dsbm.bmBits=nil)) then begin
...
{$IF DEFINED(CLR)}
								if SrcDIB.dsbm.bmBits<>nil then biClrUsed:=GetDIBColorTable(NewImageDC,0,1 shl biBitCount,BI.bmiColors)
...
{$ENDIF}
						...
						end else if ((biBitCount=16)or(biBitCount=32))and((biCompression and BI_BITFIELDS)<>0) then begin
							FixupBitFields(DIB);
{$IF DEFINED(CLR)}
...
{$ENDIF}
						...
						if (Handle<>0)and(SrcDIB.dsbm.bmWidth=biWidth)and(SrcDIB.dsbm.bmHeight=biHeight)and(biBitCount>8) then begin
							// shortcut bitblt steps
				...
				try
					...
					try
						if Canvas<>nil then begin
							FillRect(NewImageDC,Rect(0,0,bmWidth,bmHeight),Canvas.Brush.Handle);
							SetTextColor(NewImageDC,ColorToRGB(Canvas.Font.Color));
							SetBkColor(NewImageDC,ColorToRGB(Canvas.Brush.Color));
							if (DIB.dsbmih.biBitCount=1)and(DIB.dsbm.bmBits<>nil) then begin
						...
						if Handle<>0 then begin
							...
							try
								...
								if OldPalette<>0 then SelectPalette(OldImageDC,Pal2,True);
								GDICheck(SelectObject(OldImageDC,OldScr));
							finally DeleteDC(OldImageDC);
							end;
						end;
					finally
						if NewPalette<>0 then SelectPalette(NewImageDC,Pal1,True);
					end;
				finally SelectObject(NewImageDC,NewScr);
				end;
			except
				DeleteObject(Result);
				raise;
...
begin
	...
	PaletteSize:=0;
	if GetObject(Palette,SizeOf(PaletteSize), {$IFNDEF CLR}@{$ENDIF}PaletteSize)=0 then Exit;
		...
		try
			...
			with DIB,dsbm do begin
				...
				if Result<>0 then begin
					...
					if TransparentColor=TColorRef(clNone) then PatBlt(MonoDC,0,0,bmWidth,bmHeight,BLACKNESS)
					else begin
...
constructor TAMDBMP.Create(AWidth,AHeight:Integer);
begin
	Create;
	SetSize(AWidth,AHeight);
end;

destructor TAMDBMP.Destroy;
begin
	FreeContext;
	FImage.Release;
	FCanvas.Free;
...
procedure TAMDBMP.Assign(Source:TPersistent);
var
	DIB:TDIBSection;
{$IF DEFINED(CLR)}
	NewHandle:HBITMAP;
...
{$ENDIF}
begin
	if (Source=nil)or(Source is TAMDBMP) then begin
		FreeContext;
{$IF DEFINED(CLR)}
...
{$ENDIF}
		try
			if Source<>nil then begin
				TAMDBMP(Source).FImage.reference;
				FImage.Release;
				FImage:=TAMDBMP(Source).FImage;
				FTransparent:=TAMDBMP(Source).FTransparent;
				FTransparentColor:=TAMDBMP(Source).FTransparentColor;
				FTransparentMode:=TAMDBMP(Source).FTransparentMode;
				PixelFormat:=TAMDBMP(Source).PixelFormat;
				FAlphaFormat:=TAMDBMP(Source).AlphaFormat;
			end
			else
{$IF DEFINED(CLR)}
					NewImage(0,0,DIB);
...
{$IF DEFINED(CLR)}
		if Source is System.Drawing.Bitmap then begin
			FreeContext;
...
procedure TAMDBMP.CopyImage(AHandle:HBITMAP;APalette:HPALETTE;DIB:TDIBSection);
var
...
{$ENDIF}
			NewPalette:=APalette
		else NewPalette:=CopyPalette(APalette);
...
{ Called by the FCanvas whenever an operation is going to be performed on the
	...
	than one TBitmap }
procedure TAMDBMP.Changing(Sender:TObject);
begin
	FreeImage;
...
procedure TAMDBMP.Changed(Sender:TObject);
begin
	FMaskBitsValid:=False;
	inherited Changed(Sender);
end;

procedure TAMDBMP.Dormant;
var
	S:TMemoryStream;
	DIB:TDIBSection;
begin
...
procedure TAMDBMP.Draw(ACanvas:TCanvas;const Rect:TRect);
var
	OldPalette:HPALETTE;
...
begin
	with Rect do begin
		...
		BPP:=GetDeviceCaps(ACanvas.FHandle,BITSPIXEL)*GetDeviceCaps(ACanvas.FHandle,PLANES);
		...
		end else if not Monochrome then SetStretchBltMode(ACanvas.Handle,STRETCH_DELETESCANS);
		try
			...
			if Transparent then begin
				...
				try
					...
					TransparentStretchBlt(ACanvas.FHandle,Left,Top,Right-Left,Bottom-Top,Canvas.FHandle,0,0,
						FImage.FDIB.dsbm.bmWidth,FImage.FDIB.dsbm.bmHeight,MaskDC,0,0);
				finally
					if Save<>0 then SelectObject(MaskDC,Save);
					if MaskDC<>0 then DeleteDC(MaskDC);
				end;
			end else if SupportsPartialTransparency then begin
				...
				Winapi.Windows.AlphaBlend(ACanvas.Handle,Left,Top,Right-Left,Bottom-Top,Canvas.FHandle,0,0,
					FImage.FDIB.dsbm.bmWidth,FImage.FDIB.dsbm.bmHeight,BF);
			end
			else StretchBlt(ACanvas.FHandle,Left,Top,Right-Left,Bottom-Top,Canvas.FHandle,0,0,FImage.FDIB.dsbm.bmWidth,
					FImage.FDIB.dsbm.bmHeight,ACanvas.CopyMode);
		finally
			if RestorePalette then SelectPalette(ACanvas.FHandle,OldPalette,True);
			SetStretchBltMode(ACanvas.FHandle,OldStretchBltMode);
		end;
	end;
end;

procedure TAMDBMP.DrawTransparent(ACanvas:TCanvas;const Rect:TRect;Opacity:Byte);
var
	OldPalette:HPALETTE;
	RestorePalette:Boolean;
	DoHalftone:Boolean;
...
begin
	with Rect do begin
		...
		try
			...
			end else if FImage.FDIB.dsbmih.biBitCount=32 then begin
...
begin
	with FImage do
		if FRefCount>1 then begin
			HandleNeeded;
			if FHalftone then P:=0
			else P:=FPalette;
			CopyImage(FHandle,P,FDIB)
		end else if (FHandle<>0)and(FHandle<>FDIBHandle) then begin
...
function TAMDBMP.GetHashCode:Integer;
begin
	Result:=inherited GetHashCode xor(FImage.GetHashCode shl 1);
	if Assigned(FCanvas) then Result:=Result xor(FCanvas.GetHashCode shl 2);
end;
{$ENDIF}

function TAMDBMP.GetEmpty:Boolean;
begin
	with FImage do Result:=(FHandle=0)and(FDIBHandle=0)and(FSaveStream=nil);
end;

function TAMDBMP.GetCanvas:TCanvas;
begin
...
{ Since the user might modify the contents of the HBITMAP it must not be
	...
	into a DC. }
function TAMDBMP.GetHandle:HBITMAP;
begin
	FreeContext;
	HandleNeeded;
	Changing(Self);
...
function TAMDBMP.HandleAllocated:Boolean;
begin
	Result:=Assigned(FImage)and(FImage.FHandle<>0);
end;

function TAMDBMP.GetHandleType:TBitmapHandleType;
begin
	with FImage do begin
		if (FHandle=0)or(FHandle=FDIBHandle) then
			if FDIBHandle=0 then
				if FDIB.dsbmih.biSize=0 then Result:=bmDDB
				else Result:=bmDIB
			else Result:=bmDIB
		else Result:=bmDDB;
	end;
...
function TAMDBMP.GetHeight:Integer;
begin
...
function TAMDBMP.GetImageFormat:TImageFormat;
begin
	Result:=FImage.FImageFormat;
end;
{$ENDIF}

function TAMDBMP.GetMaskHandle:HBITMAP;
begin
	MaskHandleNeeded;
	Result:=FImage.FMaskHandle;
end;

function TAMDBMP.GetMonochrome:Boolean;
begin
	with FImage.FDIB.dsbm do Result:=(bmPlanes=1)and(bmBitsPixel=1);
end;

function TAMDBMP.GetPalette:HPALETTE;
begin
	PaletteNeeded;
	Result:=FImage.FPalette;
end;

function TAMDBMP.GetPixelFormat:TPixelFormat;
begin
	Result:=pfCustom;
	if HandleType=bmDDB then Result:=pfDevice
	else
{$IF DEFINED(CLR)}
...
{$ENDIF}
				...
				16: case biCompression of
						...
						BI_BITFIELDS: if dsBitFields[1]=$7E0 then Result:=pf16bit
							else if dsBitFields[1]=$3E0 then Result:=pf15bit;
					end;
				24:Result:=pf24bit;
...
function TAMDBMP.GetScanline(Row:Integer):IntPtr;
begin
	Changing(Self);
	with FImage.FDIB.dsbm do begin
		if (Row<0)or(Row>=bmHeight) then InvalidOperation(SScanLine);
		DIBNeeded;
		GDIFlush;
		Row:=bmHeight-Row-1; // Always bottom-up DIB
		Result:=IntPtr.Create(bmBits.ToInt64+Row*BytesPerScanline(bmWidth,bmBitsPixel,32));
	end;
end;
{$ELSE}

function TAMDBMP.GetScanline(Row:Integer):Pointer;
begin
	Changing(Self);
	with FImage.FDIB,dsbm,dsbmih do begin
		if (Row<0)or(Row>=bmHeight) then InvalidOperation(@SScanLine);
		DIBNeeded;
		GDIFlush;
		if biHeight>0 then // bottom-up DIB
				Row:=biHeight-Row-1;
		Result:=PByte(bmBits)+Row*BytesPerScanline(biWidth,biBitCount,32);
	end;
end;
{$ENDIF}

function TAMDBMP.GetSupportsPartialTransparency:Boolean;
begin
	Result:=(FImage.FDIB.dsbmih.biBitCount=32)and(FAlphaFormat in [afDefined,afPremultiplied]);
end;

procedure TAMDBMP.PreMultiplyAlpha;
var
	Alpha:Word;
{$IFNDEF CLR}
	ImageData:PByte;
...
begin
	...
	begin
		...
		if FImage.FDIB.dsbm.bmBits=nil then Exit;
		ImageData:=FImage.FDIB.dsbm.bmBits;
		while UIntPtr(ImageData)<UIntPtr(FImage.FDIB.dsbm.bmBits)+FImage.FDIB.dsbmih.biSizeImage do begin
{$IFNDEF CLR}
...
procedure TAMDBMP.UnPreMultiplyAlpha;
var
	Alpha:Word;
{$IFNDEF CLR}
	ImageData:PByte;
...
begin
	...
	begin
		...
		while UIntPtr(ImageData)<UIntPtr(FImage.FDIB.dsbm.bmBits)+FImage.FDIB.dsbmih.biSizeImage do begin
{$IFNDEF CLR}
			if (PByte(ImageData+3)^)=0 then begin
				PByte(ImageData)^:=0;
				PByte(ImageData+1)^:=0;
				PByte(ImageData+2)^:=0;
			end else begin
...
procedure TAMDBMP.SetAlphaFormat(Value:TAlphaFormat);
begin
	if FAlphaFormat<>Value then begin
		if FImage.FRefCount>1 then
			with FImage do begin
				HandleNeeded;
				CopyImage(FHandle,FPalette,FDIB);
...
function TAMDBMP.GetTransparentColor:TColor;
begin
	if FTransparentColor=clDefault then begin
		if Monochrome then Result:=clWhite
		else Result:=Canvas.Pixels[0,Height-1];
	end
	else Result:=ColorToRGB(FTransparentColor);
	Result:=Result or $02000000;
end;

function TAMDBMP.GetWidth:Integer;
begin
	Result:=FImage.FDIB.dsbm.bmWidth;
end;

procedure TAMDBMP.DIBNeeded;
begin
	with FImage do begin
		if (FHandle=0)or(FDIBHandle<>0) then Exit;
		PaletteNeeded;
		if FDIB.dsbmih.biSize=0 then begin
{$IF DEFINED(CLR)}
			GetObject(FHandle,Marshal.SizeOf(TypeOf(FDIB)),FDIB);
{$ELSE}
			GetObject(FHandle,SizeOf(FDIB),@FDIB);
{$ENDIF}
			with FDIB,dsbm,dsbmih do begin
...
procedure TAMDBMP.Mask(TransparentColor:TColor);
var
	NewHandle,NewPalette:THandle;
	DIB:TDIBSection;
begin
	...
	try
		FreeContext;
		HandleNeeded;
		NewHandle:=CopyBitmapAsMask(FImage.FHandle,FImage.FPalette,ColorToRGB(TransparentColor));
{$IF DEFINED(CLR)}
...
{$ENDIF}
		...
		else NewPalette:=CopyPalette(FImage.FPalette);
{$IF DEFINED(CLR)}
...
procedure TAMDBMP.MaskHandleNeeded;
begin
...
procedure TAMDBMP.PaletteNeeded;
var
	DC:HDC;
begin
	with FImage do begin
...
{$ENDIF}
		...
		DC:=GDICheck(GetDC(0));
		FHalftone:=FHalftone or((GetDeviceCaps(DC,BITSPIXEL)*GetDeviceCaps(DC,PLANES))<
...
procedure TAMDBMP.InternalLoadFromBitmap(Bitmap:System.Drawing.Bitmap;var BMHandle:HBITMAP;var APalette:HPALETTE;
var DIB:TDIBSection);
var
...
begin
	...
	BmData:=Bitmap.LockBits(System.Drawing.Rectangle.FromLTRB(0,0,Bitmap.Width,Bitmap.Height),ImageLockMode.ReadOnly,
		Bitmap.PixelFormat);
	with BI,bmiHeader,Bitmap do begin
		try
			biSize:=Marshal.SizeOf(TypeOf(TBitmapInfoHeader));
			biPlanes:=1;

			biBitCount:=Abs(BmData.Stride)div Width;
			if biBitCount>0 then biBitCount:=biBitCount*8
			else begin
				if BmData.PixelFormat=System.Drawing.Imaging.PixelFormat.Format1bppIndexed then biBitCount:=1
				else if BmData.PixelFormat=System.Drawing.Imaging.PixelFormat.Format4bppIndexed then biBitCount:=4
				else if BmData.PixelFormat=System.Drawing.Imaging.PixelFormat.Format8bppIndexed then biBitCount:=8
				else InvalidGraphic({$IFNDEF CLR}@{$ENDIF}SInvalidPixelFormat);
			end;
		...
		if ColorCount>0 then
			for I:=0 to ColorCount-1 do bmiColors[I]:=LPalette.Entries[I].ToArgb xor Longint($FF000000);
	end;
...
procedure TAMDBMP.LoadFromBitmap(Bitmap:System.Drawing.Bitmap);
var
	DIB:TDIBSection;
	BMHandle:HBITMAP;
	APalette:HPALETTE;
...
procedure TAMDBMP.LoadFromClipboardFormat(AFormat:Word;AData:THandle;APalette:HPALETTE);
var
	DIB:TDIBSection;
begin
...
procedure TAMDBMP.LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;Culture:CultureInfo);
var
...
procedure TAMDBMP.LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;ResourceSet:System.Type;
Culture:CultureInfo=nil);
var
	ResMgr:ResourceManager;
begin
...
procedure TAMDBMP.LoadFromResourceName(const ResName:String;AType:System.Type;Culture:CultureInfo);
var
	ResMgr:ResourceManager;
begin
...
procedure TAMDBMP.LoadFromResourceName(Instance:THandle;const ResName:string);
{$IF DEFINED(CLR)}
var
	DIB:TDIBSection;
	NewHandle:HBITMAP;
begin
	NewHandle:=LoadImage(Instance,ResName,IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION or LR_DEFAULTSIZE);
	GetObject(NewHandle,Marshal.SizeOf(TypeOf(DIB)),DIB);
...
begin
	...
	try ReadDIB(Stream,Stream.Size);
	finally Stream.Free;
	end;
end;
{$ENDIF}
...
procedure TAMDBMP.LoadFromResourceID(Instance:THandle;ResID:Integer);
{$IF DEFINED(CLR)}
var
	DIB:TDIBSection;
	NewHandle:HBITMAP;
begin
	NewHandle:=LoadImage(Instance,ResID,IMAGE_BITMAP,0,0,LR_CREATEDIBSECTION or LR_DEFAULTSIZE);
	GetObject(NewHandle,Marshal.SizeOf(TypeOf(DIB)),DIB);
	NewImage(NewHandle,0,DIB);
	Changed(Self);
end;
...
class function TAMDBMP.CanLoadFromStream(Stream:TStream):Boolean;
var
	bmf:TBitmapFileHeader;
	P:Int64;
begin
	P:=Stream.Position;
	try Result:=(Stream.Size-Stream.Position=0)or(Stream.Read(bmf,SizeOf(bmf))=SizeOf(bmf))and(bmf.bfType=$4D42);
	finally Stream.Position:=P;
	end;
end;

procedure TAMDBMP.LoadFromStream(Stream:TStream);
begin
...
procedure TAMDBMP.NewImage(NewHandle:HBITMAP;NewPalette:HPALETTE;const NewDIB:TDIBSection;
NewImageFormat:TImageFormat=nil;NewSaveStream:TMemoryStream=nil);
{$ELSE}

procedure TAMDBMP.NewImage(NewHandle:HBITMAP;NewPalette:HPALETTE;const NewDIB:TDIBSection;OS2Format:Boolean;
RLEStream:TStream=nil);
{$ENDIF}
var
	Image:TBitmapImage;
...
procedure TAMDBMP.ReadData(Stream:TStream);
var
	Size:Longint;
begin
...
procedure TAMDBMP.ReadDIB(Stream:TStream;ImageSize:LongWord);
var
	BMHandle:HBITMAP;
	APalette:HPALETTE;
	DIB:TDIBSection;
...
procedure TAMDBMP.ReadDIB(Stream:TStream;ImageSize:LongWord;bmf:PBitmapFileHeader);
const
	...
	with BitmapInfo^ do
		try
			try
				...
				begin
					Stream.Read(Pointer(PByte(@OS2Header)+SizeOf(HeaderSize))^,SizeOf(OS2Header)-SizeOf(HeaderSize));
					FillChar(bmiHeader,SizeOf(bmiHeader),0);
					with bmiHeader,OS2Header do begin
						biWidth:=bcWidth;
						biHeight:=bcHeight;
						biPlanes:=bcPlanes;
				...
				end else begin // support bitmap headers larger than TBitmapInfoHeader
					Stream.Read(Pointer(PByte(BitmapInfo)+SizeOf(HeaderSize))^,HeaderSize-SizeOf(HeaderSize));
					DEC(ImageSize,HeaderSize);

					if (bmiHeader.biCompression<>BI_BITFIELDS)and(bmiHeader.biCompression<>BI_RGB) then begin
						...
						RLEStream.Write(Pointer(PByte(BitmapInfo)+SizeOf(HeaderSize))^,HeaderSize-SizeOf(HeaderSize));
				...
				with bmiHeader do begin
					...
					if (HeaderSize=SizeOf(TBitmapInfoHeader))and((biBitCount=16)or(biBitCount=32))and(biCompression=BI_BITFIELDS)
					then begin
					...
					if biClrUsed=0 then biClrUsed:=GetDInColors(biBitCount);
				...
				DC:=GDICheck(GetDC(0));
				try
					if ((bmiHeader.biCompression<>BI_RGB)and(bmiHeader.biCompression<>BI_BITFIELDS))or DDBsOnly then begin
						MemDC:=0;
						...
						try
							...
							OldPal:=0;
							if bmiHeader.biClrUsed>0 then begin
							...
							try
								...
								if (BMHandle=0) then
									if GetLastError=0 then InvalidBitmap
...
{$IFDEF LINUX}
							...
							for I:=1 to (ImageSize+4095)div 4096 do PByteArray(BitsMem)^[(I-1)*4096]:=0;
{$ENDIF}
							Stream.ReadBuffer(BitsMem^,ImageSize);
						except
				...
				if ((bmiHeader.biBitCount>8)and(bmiHeader.biBitCount<=16)or(bmiHeader.biBitCount=32))and(bmiHeader.biClrUsed>0)
					and(Pal=0) then Pal:=PaletteFromDIBColorTable(0,ColorTable,bmiHeader.biClrUsed);
				FillChar(DIB,SizeOf(DIB),0);
				GetObject(BMHandle,SizeOf(DIB),@DIB);
				// GetObject / CreateDIBSection don't preserve these info values
				if Abs(DIB.dsbmih.biHeight)=Abs(bmiHeader.biHeight) then DIB.dsbmih.biHeight:=bmiHeader.biHeight;
		...
		finally FreeMem(BitmapInfo);
		end;
	NewImage(BMHandle,Pal,DIB,OS2Format,RLEStream);

	if (FImage.FDIB.dsbmih.biBitCount=32)and(FAlphaFormat=afDefined) then PreMultiplyAlpha;
...
procedure TAMDBMP.ReadStream(Stream:TStream;Size:Longint);
{$IF DEFINED(CLR)}
var
	DIB:TDIBSection;
...
begin
	FreeContext;
	if Size=0 then NewImage(0,0,DIB)
	else begin
		...
		if Stream.Position=0 then ReadDIB(Stream,Size)
		else begin
...
procedure TAMDBMP.SetHandle(Value:HBITMAP);
var
	DIB:TDIBSection;
	APalette:HPALETTE;
begin
...
{$IF DEFINED(CLR)}
				if THandle(FPalette)=SystemPalette16.Handle then APalette:=SystemPalette16.Handle
{$ELSE}
			if FPalette=SystemPalette16 then APalette:=SystemPalette16
{$ENDIF}
			else APalette:=CopyPalette(FPalette);
			try
{$IF DEFINED(CLR)}
...
procedure TAMDBMP.SetHandleType(Value:TBitmapHandleType);
var
	DIB:TDIBSection;
	AHandle:HBITMAP;
	NewPalette:HPALETTE;
...
begin
	...
	with FImage do begin
		if (FHandle=0)and(FDIBHandle=0) then
			if Value=bmDDB then FDIB.dsbmih.biSize:=0
			else FDIB.dsbmih.biSize:=SizeOf(FDIB.dsbmih)
		else begin
			if Value=bmDIB then begin
...
{$ENDIF}
				...
				else CopyImage(FDIBHandle,FPalette,FDIB);
			end else begin
				...
				DoCopy:=FRefCount=1;
				if DoCopy then NewPalette:=FPalette
				else NewPalette:=CopyPalette(FPalette);
				AHandle:=CopyBitmap(FDIBHandle,FPalette,NewPalette,DIB,nil);
				if DoCopy then FHandle:=AHandle
				else
{$IF DEFINED(CLR)}
...
procedure TAMDBMP.SetHeight(Value:Integer);
begin
	SetSize(FImage.FDIB.dsbm.bmWidth,Value);
end;

procedure TAMDBMP.SetMaskHandle(Value:HBITMAP);
begin
	with FImage do
		if FMaskHandle<>Value then begin
			FMaskHandle:=Value;
			FMaskValid:=True;
			FMaskBitsValid:=True;
		end;
...
procedure TAMDBMP.SetMonochrome(Value:Boolean);
var
	DIB:TDIBSection;
begin
	with FImage,FDIB.dsbmih do
		if Value<>((biPlanes=1)and(biBitCount=1)) then begin
			HandleNeeded;
			DIB:=FDIB;
			with DIB.dsbmih,DIB.dsbm do begin
...
procedure TAMDBMP.SetPalette(Value:HPALETTE);
var
	AHandle:HBITMAP;
	DIB:TDIBSection;
begin
	if FImage.FPalette<>Value then begin
		with FImage do
			if (Value=0)and(FRefCount=1) then begin
				InternalDeletePalette(FPalette);
				FPalette:=0;
			end else begin
				FreeContext;
...
procedure TAMDBMP.SetPixelFormat(Value:TPixelFormat);
const
...
procedure TAMDBMP.SetImageFormat(Value:TImageFormat);
begin
	with FImage do
		if Value<>FImageFormat then begin
			FImageFormat:=Value;
			FreeAndNil(FImage.FSaveStream);
		end;
end;
...
procedure TAMDBMP.SetTransparentColor(Value:TColor);
begin
	if Value<>FTransparentColor then begin
		if Value=clDefault then FTransparentMode:=tmAuto
...
procedure TAMDBMP.SetTransparentMode(Value:TTransparentMode);
begin
	if Value<>FTransparentMode then begin
		if Value=tmAuto then SetTransparentColor(clDefault)
		else SetTransparentColor(GetTransparentColor);
	end;
end;

procedure TAMDBMP.SetWidth(Value:Integer);
begin
	SetSize(Value,FImage.FDIB.dsbm.bmHeight);
end;

procedure TAMDBMP.WriteData(Stream:TStream);
begin
	WriteStream(Stream,True);
end;

procedure TAMDBMP.WriteStream(Stream:TStream;WriteSize:Boolean);
{$IF DEFINED(CLR)}
var
	Size:DWORD;
	MemStream:TMemoryStream;
...
begin
	...
	DIBNeeded;
	if FImage.FDIBHandle<>0 then begin
		Bitmap:=System.Drawing.Bitmap.FromHBitmap(FImage.FDIBHandle);

		if FImage.FImageFormat=nil then Format:=System.Drawing.Imaging.ImageFormat.BMP
		else Format:=FImage.FImageFormat;
		...
		else Bitmap.Save(Stream,Format);
	end;
end;
{$ELSE}
...
begin
	...
	DIBNeeded;
	if (FImage.FDIB.dsbmih.biBitCount=32)and(FAlphaFormat=afDefined) then UnPreMultiplyAlpha;
	with FImage do begin
		Size:=0;
		if FDIBHandle<>0 then begin
			InternalGetDIBSizes(FDIBHandle,HeaderSize,Size,FDIB.dsbmih.biClrUsed);
			if FOS2Format then begin // OS2 format cannot have partial palette
				HeaderSize:=SizeOf(BC);
				if FDIB.dsbmih.biBitCount<=8 then Inc(HeaderSize,SizeOf(TRGBTriple)*(1 shl FDIB.dsbmih.biBitCount));
			end;
			...
			if (0<FDIB.dsbmih.biClrUsed)and(FDIB.dsbmih.biClrUsed<ColorCount) then ColorCount:=FDIB.dsbmih.biClrUsed;
			if (not FOS2Format)and(ColorCount=0)and(FPalette<>0)and not FHalftone then begin
				ColorCount:=PaletteToDIBColorTable(FPalette,Colors);
				if FDIB.dsbmih.biBitCount>8 then begin // optional color palette for hicolor images (non OS2)
		...
		if Size<>0 then begin
			FixupBitFields(FDIB);
			if (ColorCount<>0) then begin
				if (FDIB.dsbmih.biClrUsed=0)or(FDIB.dsbmih.biClrUsed<>ColorCount) then FDIB.dsbmih.biClrUsed:=ColorCount;
				if FOS2Format then RGBQuadToTriple(Colors,Integer(ColorCount));
			end;
			if FOS2Format then begin
				with BC,FDIB.dsbmih do begin
					bcSize:=SizeOf(BC);
					bcWidth:=biWidth;
					bcHeight:=biHeight;
					bcPlanes:=1;
...
function TAMDBMP.ReleaseHandle:HBITMAP;
begin
	HandleNeeded;
	Changing(Self);
	with FImage do begin
		Result:=FHandle;
		if FHandle=FDIBHandle then begin
			FDIBHandle:=0;
			FDIB.dsbm.bmBits:=nil;
		end;
		FHandle:=0;
...
function TAMDBMP.ReleaseMaskHandle:HBITMAP;
begin
	Result:=GetMaskHandle;
	FImage.FMaskHandle:=0;
end;

function TAMDBMP.ReleasePalette:HPALETTE;
begin
	FreeContext;
	HandleNeeded;
	Changing(Self);
...
procedure TAMDBMP.SaveToStream(Stream:TStream);
begin
	WriteStream(Stream,False);
end;

procedure TAMDBMP.SaveToClipboardFormat(var Format:Word;var Data:THandle;var APalette:HPALETTE);
var
	DIB:TDIBSection;
begin
	Format:=CF_BITMAP;
	...
	try APalette:=CopyPalette(FImage.FPalette);
	except
		DeleteObject(Data);
		raise;
	end;
...
procedure TAMDBMP.SetSize(AWidth,AHeight:Integer);
var
	DIB:TDIBSection;
begin
	HandleNeeded;
	with FImage do
		if (FDIB.dsbm.bmWidth<>AWidth)or(FDIB.dsbm.bmHeight<>AHeight) then begin
			DIB:=FDIB;
...
function TAMDBMP.TransparentColorStored:Boolean;
begin
...
procedure TAMDBMP.LoadFromIStream(Stream:IStream;PosS,PosE:UInt64);
begin
	Stream.Seek(PosS,STREAM_SEEK_SET,PUINT64(nil)^);
	ReadIStream(Stream,PosE-PosS,PosS,PosE);
end;

procedure TAMDBMP.SaveToIStream(Stream:IStream);
begin
	WriteIStream(Stream,True);
end;

procedure TAMDBMP.WriteIStream(Stream:IStream;WriteSize:Boolean);
const
	PalSize: array [Boolean] of Byte=(SizeOf(TRGBQuad),SizeOf(TRGBTriple));
var
	Size,ColorCount:DWORD;
...
begin
	...
	if (FImage.FSaveStream<>nil) then begin
		Size:=FImage.FSaveStream.Size;
		F:=0;
		if WriteSize then Stream.Write(@Size,SizeOf(Size),@F);
		F:=0;
		Stream.Write(FImage.FSaveStream.Memory,FImage.FSaveStream.Size,@F);
		Exit;
	end;
	DIBNeeded;
	if (FImage.FDIB.dsbmih.biBitCount=32)and(FAlphaFormat=afDefined) then UnPreMultiplyAlpha;
	with FImage do begin
		...
		if Size<>0 then begin
			...
			if FOS2Format then begin
				...
				F:=0;
				Stream.Write(@bmf,SizeOf(bmf),@F);
				F:=0;
				Stream.Write(@BC,SizeOf(BC),@F);
			end else begin
				F:=0;
				Stream.Write(@bmf,SizeOf(bmf),@F);
				F:=0;
				Stream.Write(@FDIB.dsbmih,SizeOf(FDIB.dsbmih),@F);
				F:=0;
				if (FDIB.dsbmih.biBitCount>8)and((FDIB.dsbmih.biCompression and BI_BITFIELDS)<>0) then
						Stream.Write(@FDIB.dsBitFields,12,@F);
			end;
			F:=0;
			Stream.Write(@Colors,ColorCount*PalSize[FOS2Format],@F);
			F:=0;
			Stream.Write(FDIB.dsbm.bmBits,FDIB.dsbmih.biSizeImage,@F);
		end;
	end;
end;

procedure TAMDBMP.ReadIStream(Stream:IStream;Size:Longint;PosS,PosE:UInt64);
var
	bmf:TBitmapFileHeader;
	DIB:TDIBSection;
	F:UInt64;
begin
	...
	if Size=0 then begin
		...
		Stream.Read(@bmf,SizeOf(bmf),@F);
		if bmf.bfType<>$4D42 then InvalidBitmap;
		ReadIDIB(Stream,Size-SizeOf(bmf),PosS,PosE,@bmf);
	end;
end;

procedure TAMDBMP.ReadIDIB(Stream:IStream;ImageSize:LongWord;PosS,PosE:UInt64;bmf:PBitmapFileHeader=nil);
const
	DIBPalSizes: array [Boolean] of Byte=(SizeOf(TRGBQuad),SizeOf(TRGBTriple));
var
	DC,MemDC:HDC;
...
begin
	...
	RLEStream:=nil;
	F:=0;
	Stream.Read(@HeaderSize,SizeOf(HeaderSize),@F);
	OS2Format:=HeaderSize=SizeOf(OS2Header);
	if OS2Format then HeaderSize:=SizeOf(TBitmapInfoHeader);
	GetMem(BitmapInfo,HeaderSize+12+256*SizeOf(TRGBQuad));
	with BitmapInfo^ do
		try
			try
				...
				begin
					F:=0;
					Stream.Read(Pointer(PByte(@OS2Header)+SizeOf(HeaderSize)),SizeOf(OS2Header)-SizeOf(HeaderSize),@F);
					FillChar(bmiHeader,SizeOf(bmiHeader),0);
					with bmiHeader,OS2Header do begin
						biWidth:=bcWidth;
						biHeight:=bcHeight;
					...
					F:=0;
					Stream.Read(Pointer(PByte(BitmapInfo)+SizeOf(HeaderSize)),HeaderSize-SizeOf(HeaderSize),@F);
					DEC(ImageSize,HeaderSize);
					...
					if (bmiHeader.biCompression<>BI_BITFIELDS)and(bmiHeader.biCompression<>BI_RGB) then begin
						...
						Stream.Seek(PosS,STREAM_SEEK_SET,PUINT64(nil)^);
						PP:=CoTaskMemAlloc(ImageSize);
						F:=0;
						Stream.Read(PP,ImageSize,@F);
						RLEStream.Write(PP^,ImageSize);
						// RLEStream.CopyFrom(Stream,ImageSize);
						CoTaskMemFree(PP);
						{ Cast ImageSize (long word) to integer to avoid integer overflow when negating. }
						RLEStream.Seek(-Integer(ImageSize),soFromEnd);
				...
				with bmiHeader do begin
					...
					then begin
						F:=0;
						Stream.Read(ColorTable,3*SizeOf(DWORD),@F);
						Inc(PByte(ColorTable),3*SizeOf(DWORD));
						DEC(ImageSize,3*SizeOf(DWORD));
					end;
					...
					if (biClrUsed*DIBPalSizes[OS2Format])>(256*SizeOf(TRGBQuad)) then
							InvalidGraphic({$IFNDEF CLR}@{$ENDIF}SInvalidBitmap);
					F:=0;
					Stream.Read(ColorTable,biClrUsed*DIBPalSizes[OS2Format],@F);
					DEC(ImageSize,biClrUsed*DIBPalSizes[OS2Format]);
				...
				try
					if ((bmiHeader.biCompression<>BI_RGB)and(bmiHeader.biCompression<>BI_BITFIELDS))or DDBsOnly then begin
						...
						try
							F:=0;
							Stream.Read(BitsMem,ImageSize,@F);
							MemDC:=GDICheck(CreateCompatibleDC(DC));
							OldBMP:=SelectObject(MemDC,CreateCompatibleBitmap(DC,1,1));
							OldPal:=0;
							if bmiHeader.biClrUsed>0 then begin
...
procedure TIcon.AssignTo(Dest:TPersistent);
	...
	begin
		...
		for Y:=0 to CY-1 do begin
			for X:=0 to CY-1 do begin
				if (DWORD(Buf^)and $FF000000)<>0 then begin
					Result:=True;
					Exit;
				end;
				Inc(Buf,1);
...
begin
	if Dest is TAMDBMP then begin
		...
		try
			...
			hBmp:=BMP.Handle;
			if (GetDIBits(BMP.Canvas.Handle,hBmp,0,Height,pBitmap,Bmi,DIB_RGB_COLORS)=Height)and
				not HasAlpha(pBitmap,Width,Height) then begin
				pMask:=AllocMem(Bmi.bmiHeader.biWidth*4*Bmi.bmiHeader.biHeight);
				try
					GetIconInfo(Handle,Info);
					try
						if GetDIBits(BMP.Canvas.Handle,Info.hbmMask,0,Height,pMask,Bmi,DIB_RGB_COLORS)=Height then begin
							pBitmapBit:=pBitmap;
							pMaskBit:=pMask;
							for Y:=0 to Bmi.bmiHeader.biHeight-1 do begin
								for X:=0 to Bmi.bmiHeader.biWidth-1 do begin
									if Cardinal(pMaskBit^)<>0 then Cardinal(pBitmapBit^):=0
									else pBitmapBit.rgbReserved:=255;
									Inc(pBitmapBit,1);
									Inc(pMaskBit,1);
								end;
							end;
							{ dc parameter not used since Usage = DIB_RGB_COLORS }
							SetDIBits(0,BMP.Handle,0,Height,pBitmap,Bmi,DIB_RGB_COLORS);
						end;
					finally
						DeleteObject(Info.hbmMask);
						DeleteObject(Info.hbmColor);
					end;
				finally FreeMem(pMask);
				end;
			end;
		finally FreeMem(pBitmap);
		end;
	end
	else inherited;
...
begin
	if (Source=nil)or(Source is TIcon) then begin
		if Source<>nil then begin
...
begin
	HandleNeeded;
{$IF DEFINED(CLR)}
	if Assigned(FImage.FIcon) then Result:=HICON(FImage.FIcon.Handle)
	else Result:=0;
{$ELSE}
	Result:=FImage.FHandle;
{$ENDIF}
...
begin
{$IF DEFINED(CLR)}
	Result:=Assigned(FImage)and Assigned(FImage.FIcon)and(FImage.FIcon.Handle<>nil);
{$ELSE}
	Result:=Assigned(FImage)and(FImage.FHandle<>0);
{$ENDIF}
end;
...
begin
	...
	if Result=0 then begin
		if FImage.FMemoryImage<>nil then begin
			HandleNeeded;
			Result:=FImage.FSize.Y;
		end
		else Result:=GetSystemMetrics(SM_CYICON);
	end;
end;
...
begin
	with FImage do begin
		...
		case CI.wType of
			...
			rc3_Icon:ReadIcon(FMemoryImage,NewHandle,CI.Count,SizeOf(CI),FRequestedSize,FSize);
		else InvalidIcon;
		end;
		FHandle:=NewHandle;
	end;
...
begin
	with FImage do begin
		if FMemoryImage<>nil then Exit;
		if (not Assigned(FIcon))or(Assigned(FIcon)and(FIcon.Handle=nil)) then InvalidIcon;
		Image:=TMemoryStream.Create;
		try
			if GetHandle=StockIcon then begin
...
begin
	with FImage do begin
		...
		try
			...
			else WriteIcon(Image,Handle,False);
		except
			Image.Free;
			raise;
		end;
...
procedure TIcon.LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;Culture:CultureInfo);
var
	ResMgr:ResourceManager;
begin
	ResMgr:=ResourceManager.Create(BaseName,ResourceAssembly);
...
procedure TIcon.LoadFromResourceName(const ResName,BaseName:String;ResourceAssembly:Assembly;ResourceSet:System.Type;
Culture:CultureInfo=nil);
var
	ResMgr:ResourceManager;
begin
...
procedure TIcon.LoadFromResourceName(const ResName:String;AType:System.Type;Culture:CultureInfo);
var
	ResMgr:ResourceManager;
begin
	ResMgr:=ResourceManager.Create(AType);
...
begin
	P:=Stream.Position;
	try Result:=(Stream.Read(CI,SizeOf(CI))=SizeOf(CI))and(CI.Reserved=0)and(CI.wType in [rc3_StockIcon,rc3_Icon]);
	finally Stream.Position:=P;
	end;
end;
...
{$IF DEFINED(CLR)}
		...
		if not(IconType in [rc3_StockIcon,rc3_Icon]) then InvalidIcon;
		NewImage(nil,Image);
{$ELSE}
		Stream.ReadBuffer(Image.Memory^,Image.Size);
...
begin
{$IF DEFINED(CLR)}
	if Value=0 then NewImage(nil,nil)
	else NewImage(System.Drawing.Icon.FromHandle(Value),nil);
{$ELSE}
	NewImage(Value,nil);
	if (Value<>0)and GetIconInfo(Value,IconInfo) then
		try
			if GetObject(IconInfo.hbmColor,SizeOf(BitmapInfo),@BitmapInfo)<>0 then begin
...
{$ENDIF}
	...
	else InvalidOperation({$IFNDEF CLR}@{$ENDIF}SChangeIconSize);
end;
...
begin
	ImageNeeded;
	with FImage.FMemoryImage do Stream.WriteBuffer(Memory{$IFNDEF CLR}^{$ENDIF},Size);
end;
...
var
	LImage:TWICImage;
begin
	if Source is TWICImage then begin
		...
		if Assigned(LImage.FBitmap) then begin
			if FBitmap=nil then FBitmap:=TAMDBMP.Create;

			try FBitmap.Assign(LImage.FBitmap);
			except
				FreeAndNil(FBitmap);
				raise;
			end;
	...
	end else if Source is TAMDBMP then begin
		FWicBitmap:=nil;
		var
			LReleaseBitmap:Boolean:=False;
		if FBitmap=nil then begin
			LReleaseBitmap:=True;
			FBitmap:=TAMDBMP.Create;
		end;
		try
			...
			if LReleaseBitmap then FreeAndNil(FBitmap);
		end;
	end
	else inherited;
end;
...
begin
	...
	try
		if FImagingFactory=nil then begin
			LResult:=CoCreateInstance(CLSID_WICImagingFactory,nil,CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER,IUnknown,
				FImagingFactory);
			if Failed(LResult) then
					raise EInvalidGraphicOperation.CreateFmt(SWinRTInstanceError+' (%X)',['CLSID_WICImagingFactory',LResult]);
		end
		else FImagingFactory._AddRef;
	finally LeaveCriticalSection(WicImageLock);
	end;
...
begin
	...
	EnterCriticalSection(WicImageLock);
	try
		if (FImagingFactory<>nil)and(FImagingFactory._Release=0) then Pointer(FImagingFactory):=nil;
	finally LeaveCriticalSection(WicImageLock);
	end;
...
begin
	...
	if FBitmap.FAlphaFormat in [afDefined,afPremultiplied] then PixelFormat:=GUID_WICPixelFormat32bppPBGRA
	else PixelFormat:=GUID_WICPixelFormat32bppBGR;
	...
	try
		// Forces evaluation of Bitmap.Handle before Bitmap.Canvas.Handle
		hBmp:=FBitmap.Handle;
		GetDIBits(FBitmap.Canvas.Handle,hBmp,0,FHeight,@Buffer[0],BitmapInfo,DIB_RGB_COLORS);

		FImagingFactory.CreateBitmapFromMemory(FWidth,FHeight,PixelFormat,FWidth*4,Length(Buffer),@Buffer[0],FWicBitmap);
	finally FBitmap.Canvas.Unlock;
	end;
end;

function TWICImage.CreateScaledCopy(ANewWidth,ANewHeight:Integer;
AInterpolationMode:TWICImageInterpolationMode=wipmHighQualityCubic):TWICImage;
const
	IMode: array [TWICImageInterpolationMode] of Integer=(-1,WICBitmapInterpolationModeHighQualityCubic,
		WICBitmapInterpolationModeFant,WICBitmapInterpolationModeCubic,WICBitmapInterpolationModeLinear,
...
begin
	...
	if (AInterpolationMode=wipmNone)or Empty then begin
		Result.Assign(Self);
...
begin
	...
	if (FInterpolationMode<>wipmNone)and((Rect.Width<>Width)or(Rect.Height<>Height)) then begin
		if (FScaledBuffer=nil)or(FScaledBuffer.Width<>Rect.Width)or(FScaledBuffer.Height<>Rect.Height) then begin
			FreeAndNil(FScaledBuffer);
			FScaledBuffer:=CreateScaledCopy(Rect.Width,Rect.Height,FInterpolationMode);
		end;
		if FScaledBuffer<>nil then FScaledBuffer.Draw(ACanvas,Rect);
	end else begin
		RequireBitmap;
		if FBitmap<>nil then ACanvas.StretchDraw(Rect,FBitmap);
	end;
end;
...
var
	...
	begin
		if Failed(Result) then raise EInvalidGraphic.Create(SInvalidImage);
	end;
...
procedure TWICImage.LoadFromClipboardFormat(AFormat:Word;AData:THandle;APalette:HPALETTE);
begin
	FWicBitmap:=nil;
	FreeAndNil(FScaledBuffer);
	if FBitmap=nil then FBitmap:=TAMDBMP.Create;
...
procedure TWICImage.SaveToClipboardFormat(var AFormat:Word;var AData:THandle;var APalette:HPALETTE);
begin
	RequireBitmap;
...
begin
	if not IsEqualGUID(Value,FEncoderContainerFormat) then begin
		FFormatChanged:=True;
...
begin
	...
	try
		...
		SetDIBits(0,FBitmap.Handle,0,FHeight,@Buffer[0],BitmapInfo,DIB_RGB_COLORS);
...
begin
	if FScaledBuffer<>Graphic then FreeAndNil(FScaledBuffer)
	else if FScaledBuffer<>nil then FScaledBuffer.InterpolationMode:=wipmNone;
	inherited;
end;

procedure TWICScaledGraphicDrawer.SetInterpolationMode(AValue:TWICImageInterpolationMode);
begin
	if FInterpolationMode<>AValue then begin
...
begin
	if FScaledBuffer<>Graphic then FreeAndNil(FScaledBuffer)
	else FScaledBuffer:=nil;

	if not Assigned(Graphic)or Graphic.Empty then Exit;

	if Graphic is TWICImage then FScaledBuffer:=TWICImage(Graphic)
	else begin
		FScaledBuffer:=TWICImage.Create;
		if Graphic is TAMDBMP then FScaledBuffer.Assign(Graphic)
		else begin
			LStream:=TMemoryStream.Create;
			try
				Graphic.SaveToStream(LStream);
				LStream.Seek(0,TSeekOrigin.soBeginning);
				try FScaledBuffer.LoadFromStream(LStream);
				except
					FreeAndNil(FScaledBuffer);
					raise;
				end;
	...
	if FScaledBuffer<>nil then FScaledBuffer.InterpolationMode:=FInterpolationMode;
end;

procedure TWICScaledGraphicDrawer.Draw(ACanvas:TCanvas;const ARect:TRect);
begin
	if Initialized and not ARect.IsEmpty then FScaledBuffer.Draw(ACanvas,ARect);
end;
...
begin
	for I:=GetFileFormats.Count-1 downto 0 do
		if TFileFormatType(FileFormats[I]).GraphicClass.Classname=GraphicClass.Classname then begin
...
const
	Pal16: array [0..15] of TColor=(clBlack,clMaroon,clGreen,clOlive,clNavy,clPurple,clTeal,clDkGray,clLtGray,clRed,
		clLime,clYellow,clBlue,clFuchsia,clAqua,clWhite);
var
	DC:HDC;
begin
	DC:=GetDC(0);
	try ScreenLogPixels:=GetDeviceCaps(DC,LOGPIXELSY);
	finally ReleaseDC(0,DC);
	end;
{$IFDEF CLR}
...
begin
	...
	if (DisplayDC<>0) then begin
		if (SelectObject(DisplayDC,StockFont)<>0) then
			if (GetTextMetrics(DisplayDC,TxtMetric)) then Result:=TxtMetric.tmCharSet;
		ReleaseDC(0,DisplayDC);
	end;
end;

{$IFDEF CLR}[RegistryPermission(SecurityAction.Assert,Read='HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\FontSubstitutes\')]
{$ENDIF}
...
begin
	...
	if (Win32Platform=VER_PLATFORM_WIN32_WINDOWS)and(GetDefFontCharSet=SHIFTJIS_CHARSET) then Name:='Tahoma';
	Key:=Microsoft.Win32.Registry.LocalMachine.OpenSubKey(sFontSubstitutes,False);
	if Assigned(Key) then Name:=string(Key.GetValue('MS Shell Dlg 2')); { do not localize }
	if Name<>'' then DefFontData.Name:=Name
	else DefFontData.Name:='MS Sans Serif'; { do not localize }
...
begin
	...
	if Name<>'' then DefFontData.Name:=UTF8EncodeToShortString(Name);
{$ENDIF}
end;
...
{$IF NOT DEFINED(CLR)}
		FLock:TRTLCriticalSection;
{$ENDIF}
		function CreateBitmap(BkColor,FgColor:TColor):TAMDBMP;
	public
...
begin
	...
	try
		Result:=List;
		while (Result<>nil)and((Result.BkColorRef<>BkColor)or(Result.FgColorRef<>FgColor)) do Result:=Result.Next;
		if Result=nil then begin
{$IF DEFINED(CLR)}
...
function TPatternManager.CreateBitmap(BkColor,FgColor:TColor):TAMDBMP;
var
	X,Y:Integer;
begin
	Result:=TAMDBMP.Create;
	try
		with Result do begin
			Width:=8;
			Height:=8;
...
function AllocPatternBitmap(BkColor,FgColor:TColor):TAMDBMP;
begin
	if PatternManager<>nil then Result:=PatternManager.AllocPattern(ColorToRGB(BkColor),ColorToRGB(FgColor)).Bitmap
	else Result:=nil;
end;
...
const
	CFlags: array [TTextFormats] of Cardinal=(DT_BOTTOM,DT_CALCRECT,DT_CENTER,DT_EDITCONTROL,DT_END_ELLIPSIS,
		DT_PATH_ELLIPSIS,DT_EXPANDTABS,DT_EXTERNALLEADING,DT_LEFT,DT_MODIFYSTRING,DT_NOCLIP,DT_NOPREFIX,DT_RIGHT,
		DT_RTLREADING,DT_SINGLELINE,DT_TOP,DT_VCENTER,DT_WORDBREAK,DT_HIDEPREFIX,DT_NOFULLWIDTHCHARBREAK,DT_PREFIXONLY,
		DT_TABSTOP,DT_WORD_ELLIPSIS,MASK_TF_COMPOSITED { tfComposited } );
var
	LDrawTextFlag:TTextFormats;
begin
	Result:=0;
	for LDrawTextFlag:=Low(TTextFormats) to High(TTextFormats) do
		if (LDrawTextFlag in Value.FValue) then Result:=Result or CFlags[LDrawTextFlag];
end;
...
begin
	Result.FValue:=[];
	if (Value and DT_BOTTOM)=DT_BOTTOM then Include(Result.FValue,tfBottom);
	if (Value and DT_CALCRECT)=DT_CALCRECT then Include(Result.FValue,tfCalcRect);
	if (Value and DT_CENTER)=DT_CENTER then Include(Result.FValue,tfCenter);
	if (Value and DT_EDITCONTROL)=DT_EDITCONTROL then Include(Result.FValue,tfEditControl);
	if (Value and DT_END_ELLIPSIS)=DT_END_ELLIPSIS then Include(Result.FValue,tfEndEllipsis);
	if (Value and DT_PATH_ELLIPSIS)=DT_PATH_ELLIPSIS then Include(Result.FValue,tfPathEllipsis);
	if (Value and DT_EXPANDTABS)=DT_EXPANDTABS then Include(Result.FValue,tfExpandTabs);
	if (Value and DT_EXTERNALLEADING)=DT_EXTERNALLEADING then Include(Result.FValue,tfExternalLeading);
	if (Value and DT_LEFT)=DT_LEFT then Include(Result.FValue,tfLeft);
	if (Value and DT_MODIFYSTRING)=DT_MODIFYSTRING then Include(Result.FValue,tfModifyString);
	if (Value and DT_NOCLIP)=DT_NOCLIP then Include(Result.FValue,tfNoClip);
	if (Value and DT_NOPREFIX)=DT_NOPREFIX then Include(Result.FValue,tfNoPrefix);
	if (Value and DT_RIGHT)=DT_RIGHT then Include(Result.FValue,tfRight);
	if (Value and DT_RTLREADING)=DT_RTLREADING then Include(Result.FValue,tfRtlReading);
	if (Value and DT_SINGLELINE)=DT_SINGLELINE then Include(Result.FValue,tfSingleLine);
	if (Value and DT_TOP)=DT_TOP then Include(Result.FValue,tfTop);
	if (Value and DT_VCENTER)=DT_VCENTER then Include(Result.FValue,tfVerticalCenter);
	if (Value and DT_WORDBREAK)=DT_WORDBREAK then Include(Result.FValue,tfWordBreak);
	if (Value and DT_HIDEPREFIX)=DT_HIDEPREFIX then Include(Result.FValue,tfHidePrefix);
	if (Value and DT_NOFULLWIDTHCHARBREAK)=DT_NOFULLWIDTHCHARBREAK then Include(Result.FValue,tfNoFullWidthCharBreak);
	if (Value and DT_PREFIXONLY)=DT_PREFIXONLY then Include(Result.FValue,tfPrefixOnly);
	if (Value and DT_TABSTOP)=DT_TABSTOP then Include(Result.FValue,tfTabStop);
	if (Value and DT_WORD_ELLIPSIS)=DT_WORD_ELLIPSIS then Include(Result.FValue,tfWordEllipsis);
	if (Value and MASK_TF_COMPOSITED)=MASK_TF_COMPOSITED then Include(Result.FValue,tfComposited);
end;
