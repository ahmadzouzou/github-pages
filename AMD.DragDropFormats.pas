...
type
	...
	/// /////////////////////////////////////////////////////////////////////////////
type
	TStreamProgressEvent=procedure(Sender:TObject;Count,MaxCount:integer;COMMENT:STRING) of object;
	...
	public
		...
		property WriteMAX:UInt64 read fsz write fsz default 0;
		property wread:UInt64 read fwz write fwz default 0;
	...
	protected
		...
		function DoGetDataSized(const ADataObject:IDataObject;const AMedium:TStgMedium;Size:integer):boolean;virtual;
		...
		function ReadDataInto(const ADataObject:IDataObject;const AMedium:TStgMedium;Buffer:Pointer;Size:integer)
			:boolean;virtual;
	...
	public
		...
		function HasData:boolean;override;
		function NeedsData:boolean;override;
		property PreferredDropEffect:Longint read FPreferredDropEffect write SetPreferredDropEffect;
		property PerformedDropEffect:Longint read FPerformedDropEffect write SetPerformedDropEffect;
		property LogicalPerformedDropEffect:Longint read FLogicalPerformedDropEffect write SetLogicalPerformedDropEffect;
		property PasteSucceeded:Longint read FPasteSucceeded write SetPasteSucceeded;
		property InShellDragLoop:boolean read FInShellDragLoop write SetInShellDragLoop;
...
begin
	...
	if (Medium.tymed=TYMED_HGLOBAL) then begin
		...
		if (Medium.tymed=TYMED_ISTREAM) then begin
			Stream:=IStream(Medium.stm);
			if (Stream<>nil)and(Succeeded(Stream.Stat(statstg,STATFLAG_NONAME))) then Result:=statstg.cbSize;
		end
		else
...
begin
	...
	try
		...
		try
			...
			OleCheck(StgCreateDocfileOnILockBytes(LockBytes,STGM_CREATE or STGM_READWRITE or STGM_SHARE_EXCLUSIVE or
				STGM_DIRECT,0,NewStorage));
			try
...
begin
	...
	try
		...
		// i.e. it will free it.
		try OleCheck(CreateILockBytesOnHGlobal(GlobalDest,True,LockBytesDest));
		except
		...
		OleCheck(StgCreateDocfileOnILockBytes(LockBytesSource,STGM_CREATE or STGM_READWRITE or
			STGM_SHARE_EXCLUSIVE { or STGM_DIRECT } ,0,StorageSource));
		...
		// IStorage is freed.
		OleCheck(StgCreateDocfileOnILockBytes(LockBytesDest,STGM_CREATE or STGM_READWRITE or
			STGM_SHARE_EXCLUSIVE { or STGM_DIRECT } ,0,Result));
...
begin
	...
	for i:=0 to FStreams.Count-1 do
		if (FStreams.Objects[i]<>nil) then FStreams.Objects[i].Free;
	FStreams.Clear;
end;
...
begin
	Changing;
	for i:=0 to Value.Count-1 do AddNamed(Value.Items[i],Value.Names[i]);
end;
...
begin
	...
	FHasSeeked:=True;
	// if Assigned(FProgress) then FProgress(Self,dlibMove,Stream.Size);
end;
...
var
	...
	posn:UInt64;
begin
	if (not FHasSeeked) then Seek(0,STREAM_SEEK_SET,PLargeUInt(nil)^);
	Result:=inherited Read(pv,cb,pcbRead);
	if Assigned(FProgress) then FProgress(Self,cb,Stream.Size,'READ');
	Seek(0,STREAM_SEEK_CUR,posn);
	fwz:=fwz+1;
	if (posn=Stream.Size) then
		if DeleteFile(PChar(Filename)) then ShowMessage(Filename+#13+fwz.ToString+#13+'end read file');
end;
...
begin
	...
	if fsz>0 then
		if Assigned(FProgress) then FProgress(Self,cb,fsz,'WRITE');
end;
...
begin
	...
	try
		...
		if cb>MaxBufSize then BufSize:=MaxBufSize
		else BufSize:=integer(cb);
		GetMem(Buffer,BufSize);
		try
			while cb>0 do begin
				if cb>BufSize then BurstReadSize:=BufSize
				else BurstReadSize:=cb;

				BurstWriteSize:=Stream.Read(Buffer^,BurstReadSize);
				if (BurstWriteSize=0) then break;
				...
				if (Succeeded(Result))and(integer(BurstWritten)<>BurstWriteSize) then Result:=E_FAIL;
...
function TCustomSimpleClipboardFormat.DoGetData(const ADataObject:IDataObject;const AMedium:TStgMedium):boolean;
var
	Stream:IStream;
	statstg:TStatStg;
	Size:integer;
...
begin
	// Get size from HGlobal.
	if (AMedium.tymed=TYMED_HGLOBAL) then begin
		Size:=GlobalSize(AMedium.HGLOBAL);
		Result:=True;
		// Read the given amount of data.
		if (Size>0) then Result:=DoGetDataSized(ADataObject,AMedium,Size);
	end
	else
		// Get size from IStream.
		if (AMedium.tymed=TYMED_ISTREAM) then begin
			Stream:=IStream(AMedium.stm);
			Result:=(Stream<>nil)and(Succeeded(Stream.Stat(statstg,STATFLAG_NONAME)));
			Stream:=nil;
			Size:=statstg.cbSize;
			// Read the given amount of data.
			if (Result)and(Size>0) then Result:=DoGetDataSized(ADataObject,AMedium,Size);
		end
		else
			// Get size and stream from IStorage.
			if (AMedium.tymed=TYMED_ISTORAGE) then begin
...
function TCustomSimpleClipboardFormat.DoGetDataSized(const ADataObject:IDataObject;const AMedium:TStgMedium;
...
begin
	if (Size>0) then begin
		...
		if (AMedium.tymed=TYMED_HGLOBAL) then begin
			...
			finally GlobalUnlock(AMedium.HGLOBAL);
			end;
		end
		else
				...
				try
					...
					if (Stream<>nil) then begin
						...
						try
							...
							try
								if (ChunkBuffer=nil) then begin
									Result:=False;
									Exit;
								end;

								while (Result)and(Remaining>0) do begin
									if (Remaining>MaxChunk) then Chunk:=MaxChunk
...
function TCustomSimpleClipboardFormat.ReadDataInto(const ADataObject:IDataObject;const AMedium:TStgMedium;
...
begin
	...
	if (Result) then begin
		...
		if (AMedium.tymed=TYMED_HGLOBAL) then begin
			...
			try
				Result:=(p<>nil);
				if (Result) then Move(p^,Buffer^,Size);
			finally GlobalUnlock(AMedium.HGLOBAL);
			end;
		end
		else
...
function TCustomSimpleClipboardFormat.DoSetData(const FormatEtcIn:TFormatEtc;var AMedium:TStgMedium):boolean;
var
	p:Pointer;
	Size:integer;
	Global:HGLOBAL;
...
begin
	...
	*)
	...
	if (FormatEtcIn.tymed<>-1)and(FormatEtc.tymed and FormatEtcIn.tymed and TYMED_ISTREAM<>0) then begin
		...
		if (Global=0) then Exit;
		try
			p:=GlobalLock(Global);
			try Result:=WriteData(p,Size);
			finally GlobalUnlock(Global);
			end;

			if (not Result)or(Failed(CreateStreamOnHGlobal(Global,True,Stream))) then begin
				GlobalFree(Global);
				Exit;
			end;

			Stream.Seek(0,STREAM_SEEK_END,PUInt64(nil)^);
			...
			*)
			...
			// showmessage('TCustomSimpleClipboardFormat.DoSetData');
		except Result:=False;
		end;

		if (not Result) then IStream(AMedium.stm):=nil
		else AMedium.tymed:=TYMED_ISTREAM;

	end else if (FormatEtc.tymed and FormatEtcIn.tymed and TYMED_HGLOBAL<>0) then begin

		AMedium.HGLOBAL:=GlobalAlloc(GMEM_MOVEABLE or GMEM_ZEROINIT,Size);
		if (AMedium.HGLOBAL=0) then Exit;

		try
			p:=GlobalLock(AMedium.HGLOBAL);
			try Result:=(p<>nil)and WriteData(p,Size);
			finally GlobalUnlock(AMedium.HGLOBAL);
			end;
		except Result:=False;
		end;
		...
		else AMedium.tymed:=TYMED_HGLOBAL;

	end else

		if (FormatEtcIn.tymed and TYMED_ISTORAGE<>0) then begin
		...
		try
			...
			finally GlobalFree(Global);
			end;
			if (Result) then AMedium.tymed:=TYMED_ISTORAGE;
		except
...
function TCustomAnsiStringClipboardFormat.ReadData(Value:Pointer;Size:integer):boolean;
begin
	...
	if (FTrimZeroes) then SetLength(FData,Length(PAnsiChar(FData)));

	Result:=True;
end;

function TCustomAnsiStringClipboardFormat.WriteData(Value:Pointer;Size:integer):boolean;
begin
	// Transfer string including terminating zero if requested.
	Result:=(Size<=Length(FData)+1);
	if (Result) then Move(PAnsiChar(FData)^,Value^,Size);
end;
...
function TCustomStringListClipboardFormat.ReadData(Value:Pointer;Size:integer):boolean;
var
	s:AnsiString;
begin
	SetLength(s,Size+1);
...
function TCustomStringListClipboardFormat.WriteData(Value:Pointer;Size:integer):boolean;
var
	s:AnsiString;
begin
	s:=AnsiString(FLines.Text);
	Result:=(Size=Length(s)+1);
	if (Result) then Move(PAnsiChar(s)^,Value^,Size);
end;
...
//
/// /////////////////////////////////////////////////////////////////////////////
function TCustomDWORDClipboardFormat.ReadData(Value:Pointer;Size:integer):boolean;
begin
	FValue:=PDWORD(Value)^;
	Result:=True;
end;

function TCustomDWORDClipboardFormat.WriteData(Value:Pointer;Size:integer):boolean;
begin
	Result:=(Size=SizeOf(DWORD));
	if (Result) then PDWORD(Value)^:=FValue;
end;
...
begin
	if (CF_PREFERREDDROPEFFECT=0) then CF_PREFERREDDROPEFFECT:=RegisterClipboardFormat(CFSTR_PREFERREDDROPEFFECT);
...
begin
	if (CF_PERFORMEDDROPEFFECT=0) then CF_PERFORMEDDROPEFFECT:=RegisterClipboardFormat(CFSTR_PERFORMEDDROPEFFECT);
...
begin
	if (CF_PASTESUCCEEDED=0) then CF_PASTESUCCEEDED:=RegisterClipboardFormat(CFSTR_PASTESUCCEEDED);
...
begin
	if (CF_InDragLoop=0) then CF_InDragLoop:=RegisterClipboardFormat(CFSTR_InDragLoop);
...
begin
	if (CF_TargetCLSID=0) then CF_TargetCLSID:=RegisterClipboardFormat('TargetCLSID'); // *** DO NOT LOCALIZE ***
...
function TTargetCLSIDClipboardFormat.ReadData(Value:Pointer;Size:integer):boolean;
begin
	// Validate size.
	Result:=(Size=SizeOf(TCLSID));
	if (Result) then FCLSID:=PCLSID(Value)^;
end;

function TTargetCLSIDClipboardFormat.WriteData(Value:Pointer;Size:integer):boolean;
begin
	// Validate size.
	Result:=(Size=SizeOf(TCLSID));
	if (Result) then PCLSID(Value)^:=FCLSID;
end;
...
begin
	...
	else if (Source is TPasteSucceededClipboardFormat) then FPasteSucceeded:=TPasteSucceededClipboardFormat(Source).Value

	else if (Source is TTargetCLSIDClipboardFormat) then FTargetCLSID:=TTargetCLSIDClipboardFormat(Source).CLSID

	else if (Source is TInShellDragLoopClipboardFormat) then begin
...
begin
	...
	else if (Dest is TPasteSucceededClipboardFormat) then TPasteSucceededClipboardFormat(Dest).Value:=FPasteSucceeded

	else if (Dest is TTargetCLSIDClipboardFormat) then TTargetCLSIDClipboardFormat(Dest).CLSID:=FTargetCLSID
...
procedure TFeedbackDataFormat.SetPerformedDropEffect(const Value:Longint);
begin
	Changing;
	FPerformedDropEffect:=Value;
end;

procedure TFeedbackDataFormat.SetLogicalPerformedDropEffect(const Value:Longint);
begin
	Changing;
	FLogicalPerformedDropEffect:=Value;
end;

procedure TFeedbackDataFormat.SetPreferredDropEffect(const Value:Longint);
begin
	Changing;
...
begin
	Result:=(FPreferredDropEffect<>DROPEFFECT_NONE)or(FPerformedDropEffect<>DROPEFFECT_NONE)or
		(FPasteSucceeded<>DROPEFFECT_NONE)or(FGotInShellDragLoop);
end;

function TFeedbackDataFormat.NeedsData:boolean;
begin
	Result:=(FPreferredDropEffect=DROPEFFECT_NONE)or(FPerformedDropEffect=DROPEFFECT_NONE)or
		(FPasteSucceeded=DROPEFFECT_NONE)or(not FGotInShellDragLoop);
end;
...
begin
	FFormat:=Value;
	if (FFormat<>'') then ClipboardFormat:=RegisterClipboardFormat(PChar(FFormat));
end;

function TGenericClipboardFormat.GetClipboardFormat:TClipFormat;
begin
	if (FFormatEtc.cfFormat=0)and(FFormat<>'') then FFormatEtc.cfFormat:=RegisterClipboardFormat(PChar(FFormat));
	Result:=FFormatEtc.cfFormat;
end;
...
begin
	if (Source is TGenericDataFormat) then begin
...
begin
	if (Dest is TGenericDataFormat) then begin
