...
type
	...
	public
		constructor Create(ComServer:TComServerObject;ComponentClass:TComponentClass;const ClassID:TGUID;
	...
	public
		constructor Create(ComServer:TComServerObject;ComponentClass:TComponentClass;const ClassID:TGUID;
			const ClassName,Description,AFileClass,AFileExtension:string;Instancing:TClassInstancing);
		procedure UpdateRegistry(Register:Boolean);override;
...
begin
	if (RegOpenKey(HKEY_CLASSES_ROOT,PChar(Key),SubKey)=ERROR_SUCCESS) then
		try RegDeleteValue(SubKey,nil);
		finally RegCloseKey(SubKey);
		end;
end;
	...
	repeat
		...
		if (RegOpenKey(HKEY_CLASSES_ROOT,PChar(Key),SubKey)=ERROR_SUCCESS) then
			try
				NumSubKeys:=nil;
				NumValues:=nil;
				Result:=(RegQueryInfoKey(SubKey,nil,nil,nil,NumSubKeys,nil,nil,NumValues,nil,nil,nil,nil)=ERROR_SUCCESS);
				// Only delete key if it doesn't contain values or sub keys.
				if (NumSubKeys<>nil)and(NumValues<>nil) then Result:=Result and(NumSubKeys^=0)and(NumValues^=0);
			finally RegCloseKey(SubKey);
			end;
		...
		if (Result) then begin
			...
			if (Result and DeleteTree) then begin
				// Move to parent key.
...
//
/// /////////////////////////////////////////////////////////////////////////////
constructor TVCLComObject.Create(Factory:TComObjectFactory;Component:TComponent);
begin
...
function TVCLComObject.GetIDsOfNames(const IID:TGUID;Names:Pointer;NameCount,LocaleID:Integer;DispIDs:Pointer):HResult;
begin
	Result:=E_NOTIMPL;
end;

function TVCLComObject.GetTypeInfo(Index,LocaleID:Integer;out TypeInfo):HResult;
begin
...
function TVCLComObject.Invoke(DispID:Integer;const IID:TGUID;LocaleID:Integer;Flags:Word;var Params;
	VarResult,ExcepInfo,ArgErr:Pointer):HResult;
begin
	Result:=E_NOTIMPL;
end;

function TVCLComObject.ObjQueryInterface(const IID:TGUID;out Obj):HResult;
begin
...
constructor TApartmentThread.Create(Factory:IClassFactory2;UnkOuter:IUnknown;IID:TGUID);
begin
	FFactory:=Factory;
	FUnkOuter:=UnkOuter;
	FIID:=IID;
...
begin
	try
		...
		try
			...
			FUnkOuter:=nil;
			FFactory:=nil;
			if FCreateResult=S_OK then CoMarshalInterThreadInterfaceInStream(FIID,Unk,IStream(FStream));
...
//
/// /////////////////////////////////////////////////////////////////////////////
constructor TVCLComObjectFactory.Create(ComServer:TComServerObject;ComponentClass:TComponentClass;const ClassID:TGUID;
	const ClassName,Description:string;Instancing:TClassInstancing);
begin
	inherited Create(ComServer,TComClass(ComponentClass),ClassID,ClassName,Description,Instancing,tmApartment);
end;
...
function TVCLComObjectFactory.CreateInstance(const UnkOuter:IUnknown;const IID:TGUID;out Obj):HResult;
begin
	if not IsLibrary then begin
		LockServer(True);
		try
			with TApartmentThread.Create(Self,UnkOuter,IID) do begin
				if WaitForSingleObject(Semaphore,INFINITE)=WAIT_OBJECT_0 then begin
					Result:=CreateResult;
		...
		finally LockServer(False);
		end;
	end
	else Result:=inherited CreateInstance(UnkOuter,IID,Obj);
end;
...
//
/// /////////////////////////////////////////////////////////////////////////////
constructor TShellExtFactory.Create(ComServer:TComServerObject;ComponentClass:TComponentClass;const ClassID:TGUID;
	const ClassName,Description,AFileClass,AFileExtension:string;Instancing:TClassInstancing);
begin
	inherited Create(ComServer,ComponentClass,ClassID,ClassName,Description,Instancing);
...
begin
	if Register then begin
		...
		if (FileExtension<>'') then CreateRegKey(FileExtension,'',FileClass);
	end else begin
		if (FileExtension<>'') then DeleteDefaultRegValue(FileExtension);
		inherited UpdateRegistry(Register);
	end;
end;
