...
{$DEFINE GRAPHUTILS_HAS_SCALE_FUNCTION}
{$IFEND}
{$ENDIF}
...
procedure CopyControlToBitmap(AWinControl:TWinControl;Bitmap:
	{$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap;X,Y:Integer);
...
function BitmapToGPBitmap(Bitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap):TGPBitmap;
function GeneratePolygon(ControlRect:TRect;const PolygonPoints: Array of TPoint;Orientation:TTabOrientation):TPolygon;
function CreateAlphaBlendForm(AOwner:TComponent;Bitmap:
	{$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap;Alpha:Byte):TForm;
procedure SetTabClipRegionFromPolygon(GPGraphics:TGPGraphics;Polygon:TPolygon;CombineMode:TCombineMode);
procedure ClearBitmap(Bitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap);
procedure ScaleImage(Bitmap,ScaledBitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap;ScaleFactor:Real);
...
procedure BitmapTo32BitBitmap(Bitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap);
procedure SetColorAlpha(Bitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap;AColor:TColor;NewAlpha:Byte);
...
begin
	case EaseType of
		...
		ttEaseInOutQuad:begin
				CurrentTime:=CurrentTime/(Duration/2);

				if CurrentTime<1 then Result:=ChangeInValue/2*CurrentTime*CurrentTime+StartValue
				else begin
		...
		ttEaseOutCubic:begin
				CurrentTime:=(CurrentTime/Duration)-1;

				Result:=ChangeInValue*(CurrentTime*CurrentTime*CurrentTime+1)+StartValue;
			end;

		ttEaseInOutCubic:begin
				CurrentTime:=CurrentTime/(Duration/2);

				if CurrentTime<1 then Result:=ChangeInValue/2*CurrentTime*CurrentTime*CurrentTime+StartValue
				else begin
		...
		ttEaseOutQuart:begin
				CurrentTime:=(CurrentTime/Duration)-1;

				Result:=-ChangeInValue*(CurrentTime*CurrentTime*CurrentTime*CurrentTime-1)+StartValue;
			end;

		ttEaseInOutQuart:begin
				CurrentTime:=CurrentTime/(Duration/2);

				if CurrentTime<1 then Result:=ChangeInValue/2*CurrentTime*CurrentTime*CurrentTime*CurrentTime+StartValue
				else begin
					CurrentTime:=CurrentTime-2;
		...
		ttEaseInOutExpo:begin
				CurrentTime:=CurrentTime/(Duration/2);

				if CurrentTime<1 then Result:=ChangeInValue/2*Power(2,10*(CurrentTime-1))+StartValue
				else begin
					CurrentTime:=CurrentTime-1;
		...
		ttEaseInOutCirc:begin
				CurrentTime:=CurrentTime/(Duration/2);

				if CurrentTime<1 then Result:=-ChangeInValue/2*(Sqrt(1-CurrentTime*CurrentTime)-1)+StartValue
				else begin
					CurrentTime:=CurrentTime-2;
...
function HorzFlipRect(ParentRect,ChildRect:TRect):TRect;
begin
	Result:=Rect(ParentRect.Left+ParentRect.Right-ChildRect.Right,ChildRect.Top,
...
begin
	...
	for i:=Low(Polygon) to High(Polygon) do Polygon[i].X:=ParentRect.Left+ParentRect.Right-Polygon[i].X;
end;
...
begin
	...
	Result:=(Color+$000080)and $0000FF+(Color+$008000)and $00FF00+(Color+$800000)and $FF0000;
end;
...
begin
	...
	try
		...
		if CodeStrings.Count<=2 then Result:=''
		else begin
			...
			for i:=0 to pred(CodeStrings.Count) do
					CodeStrings[i]:=ComponentName+'.'+StringReplace(Trim(CodeStrings[i]),' = ',' := ',
...
begin
	...
	try
		Brush:=TGPLinearGradientBrush.Create(MakePoint(0,0),MakePoint(0,Bitmap.Height),MakeGDIPColor(clBlack,0),
			MakeGDIPColor(clBlack,0));
		try Graphics.FillRectangle(Brush,0,0,Bitmap.Width,Bitmap.Height);
		finally FreeAndNil(Brush);
		end;
	finally FreeAndNil(Graphics);
	end;
end;

procedure ScaleImage(Bitmap,ScaledBitmap: {$IFDEF UNIT_SCOPE_NAMES}Vcl.Graphics.{$ENDIF}TBitmap;ScaleFactor:Real);

{$IFNDEF GRAPHUTILS_HAS_SCALE_FUNCTION}
var
	NewHeight,NewWidth:Integer;
{$ENDIF}
begin
{$IFDEF GRAPHUTILS_HAS_SCALE_FUNCTION} // {$IFDEF DELPHI2006_UP}
{$IFDEF UNIT_SCOPE_NAMES}Vcl.{$ENDIF}GraphUtil.ScaleImage(Bitmap,ScaledBitmap,ScaleFactor);
{$ELSE}
...
begin
	...
	for i:=low(PolygonPoints) to High(PolygonPoints) do begin
		case Orientation of
			toTop:Result[i]:=Point(PolygonPoints[i].X+ControlRect.Left,PolygonPoints[i].Y+ControlRect.Top);

			toBottom:Result[i]:=Point(PolygonPoints[i].X+ControlRect.Left,(RectHeight(ControlRect)-PolygonPoints[i].Y)+
					ControlRect.Top-1);
		end;
	end;
end;
...
function RectInflate(ARect:TRect;Value:Integer):TRect;
begin
	Result:=Rect(ARect.Left+Value,ARect.Top+Value,ARect.Right-Value,ARect.Bottom-Value);
end;
...
begin
	...
	for i:=0 to Control.ControlCount-1 do
		if Control.Controls[i] is TWinControl then EnableControlAndChildren(Control.Controls[i] as TWinControl,DoEnable);
end;
...
begin
	...
	try BitBlt(Bitmap.Canvas.Handle,X,Y,AWinControl.ClientWidth,AWinControl.ClientHeight,SrcDC,0,0,SRCCOPY);
	finally ReleaseDC(AWinControl.Handle,SrcDC);
	end;
end;
...
var
	...
	Position:TPoint;
begin
	if SrcControl.Parent<>nil then begin
...
begin
	...
	Result:=((DWORD(GetBValue(tmpRGB)) shl BlueShift)or(DWORD(GetGValue(tmpRGB)) shl GreenShift)or
		(DWORD(GetRValue(tmpRGB)) shl RedShift)or(DWORD(Alpha) shl AlphaShift));
end;
...
begin
	...
	for i:=0 to pred(Count) do begin
		if ((Polygon[i].Y<=Y)and(Y<Polygon[n].Y))or((Polygon[n].Y<=Y)and(Y<Polygon[i].Y)) then begin
			if (X<(Polygon[n].X-Polygon[i].X)*(Y-Polygon[i].Y)/(Polygon[n].Y-Polygon[i].Y)+Polygon[i].X) then
					Result:=not Result;
		end;
...
begin
	...
	try
		...
		try
			...
			finally FreeAndNil(PNGBitmap);
			end;
		finally StreamAdapter:=nil;
		end;
...
begin
	...
	for Row:=0 to 255 do
		for Col:=Row to 255 do begin
			PreMult[Row,Col]:=Row*Col div 255;

			if (Row<>Col) then PreMult[Col,Row]:=PreMult[Row,Col]; // a*b = b*a
		end;
	...
	for Row:=0 to pred(Bitmap.Height) do begin
		...
		while (Col>0) do begin
			if (GetRed(AColor)=p.rgbRed)and(GetBlue(AColor)=p.rgbBlue)and(GetGreen(AColor)=p.rgbGreen) then begin
...
begin
	...
	if (exStyle and WS_EX_LAYERED=0) then SetWindowLong(Result.Handle,GWL_EXSTYLE,exStyle or WS_EX_LAYERED);
	...
	UpdateLayeredWindow(Result.Handle,0,nil,@BitmapSize,Bitmap.Canvas.Handle,@BitmapPos,0,@BlendFunction,ULW_ALPHA);
end;
